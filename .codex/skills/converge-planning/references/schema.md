...
- **`mode: spawner`** — body calls `converge spawn <id> <template> --var key=value...` directly; framework expands templates and applies post-body. One-shot fan-out.
...
Current Converge uses one primary dynamic-work mechanism: **template invocation** via the `converge spawn` CLI. Reusable child contracts live in `templates/<name>/` and bodies invoke them via `converge spawn <id> <template> --var key=value...`.
...
**Invocation pattern:**
```bash
converge spawn <child-id> <template-name> \
  --var key1=value1 \
  --var key2=value2
```

The child id is passed as an argument; there is no `id:` field. There is no `outputs:`, no `checks:`, no body. Templates own all of those; bodies only invoke.
...
jq -c '.sprints[]' state/sprint-plan.json | while read -r S; do
  ...
  converge spawn "$ID" sprint \
    --var "wave=$(echo "$S" | jq -r '.wave')" \
    --var "focus=$(echo "$S" | jq -r '.focus')" \
    --var "goals=$(echo "$S" | jq -r '.goals | join(",")')"
done
```

Per-child outcomes surface in `$CONVERGE_SPAWN_DIR/STATUS.md` — one `- [x]` or `- [ ]` row per invocation. Failed rows carry a `fix:` block telling the AI which template or params to adjust. Failure codes: `template-not-found`, `missing-required-param`, `unknown-param`, `param-type-mismatch`, `invalid-yaml`, `unknown-field`, `duplicate-id`, plus the anti-goal locks `SPAWN_TASKMD_AUTHORED_BY_BODY` and `SPAWN_MANIFEST_AUTHORED_BY_BODY`.
