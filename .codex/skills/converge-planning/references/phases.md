...
- **Same shape repeats N times** → use a runtime template plus `mode: spawner` with a body that calls `converge spawn <id> <template> --var key=value...` per child (see `references/task-modes.md`).
