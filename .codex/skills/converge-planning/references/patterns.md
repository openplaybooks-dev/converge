...
1. **Diverge**: the `mode: converger` root evaluates goals, picks the first unsatisfied goal, calls `converge spawn <id> <template> --var key=value...` per implement+verify child; the framework expands and applies
