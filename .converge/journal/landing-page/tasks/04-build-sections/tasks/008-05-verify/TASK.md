...
description: Build the site and confirm the Get started section renders + passes section-specific assertions.
dependencies:
  - 008-04-integrate
tags:
  - verify
...
checks:
  ...
  - id: section-id-rendered
    description: "<section id=cta> is in the rendered HTML"
    cmd: "test -f apps/landing/dist/client/index.html && grep -qE 'id=\"cta\"' apps/landing/dist/client/index.html"
  - id: passed-marker
    description: PASSED marker file written (signals next section can start)
    cmd: test -f apps/landing/.content/sections/cta-banner/PASSED
vars:
  prefix: 008
...
Build the site and confirm `cta-banner` renders correctly. After this
...
grep -qE 'id="cta"' apps/landing/dist/client/index.html

# 3. Section-specific assertion (varies — see below)
# 4. Write the PASSED marker
mkdir -p apps/landing/.content/sections/cta-banner
echo "PASSED at $(date -u +%Y-%m-%dT%H:%M:%SZ)" > apps/landing/.content/sections/cta-banner/PASSED
```

## Section-specific assertion

Pick the assertion that matches `cta-banner`:
...
- Catching only `id="cta-banner"`. That confirms the section was mounted but not that its content is correct. The section-specific assertion is what makes verification real.
