---
id: 004-04-integrate
title: "Integrate: Six differentiators"
description: Mount the FeatureGrid component in src/pages/index.astro at the correct position.
dependencies:
  - 004-03-build
tags:
  - integrate
...
checks:
  ...
  - id: component-imported
    ...
    cmd: "test -f apps/landing/src/pages/index.astro && grep -qE \"import\\s+FeatureGrid\\s+from\" apps/landing/src/pages/index.astro"
  - id: component-rendered
    description: "<FeatureGrid> is rendered in index.astro"
    cmd: "test -f apps/landing/src/pages/index.astro && grep -qE '<FeatureGrid\\b' apps/landing/src/pages/index.astro"
  - id: build-clean
    description: astro check still passes after integration
    cmd: "test -f apps/landing/package.json && pnpm --filter @converge/landing check"
vars:
  prefix: 004
...
# Integrate: Six differentiators

Add the import + render of `<FeatureGrid>` to
`apps/landing/src/pages/index.astro` at the right position.

The "right position" is the order defined in
`apps/landing/.content/sections.json`. Section #004 renders in
position #004.
...
3. Add the import: `import FeatureGrid from '@/components/sections/FeatureGrid.astro';` (insert in alphabetical order with other section imports).
4. Add the render: `<FeatureGrid />` in the page body, in the position dictated by sections.json. If earlier sections (`<Hero />`, `<SocialProof />`, etc.) are already mounted, place this one after them but before any later-ordered sections.
