...
## ❌ Missing: `apps/landing/src/components/sections/Hero.astro`

Parent directory `apps/landing/src/components/sections/` does not exist.
...
2. **If a sibling file satisfies the same purpose** (e.g. `.ts` variant of a declared `.js` output) — update the outputs list in the source TASK.md to match what actually exists:
   - Edit `.converge/journal/landing-page/tasks/04-build-sections/tasks/001-hero`
