---
id: 008-04-integrate
title: "Integrate: Get started"
description: Mount the CtaBanner component in src/pages/index.astro at the correct position.
dependencies:
  - 008-03-build
tags:
  - integrate
...
checks:
  ...
  - id: component-imported
    ...
    cmd: "test -f apps/landing/src/pages/index.astro && grep -qE \"import\\s+CtaBanner\\s+from\" apps/landing/src/pages/index.astro"
  - id: component-rendered
    description: "<CtaBanner> is rendered in index.astro"
    cmd: "test -f apps/landing/src/pages/index.astro && grep -qE '<CtaBanner\\b' apps/landing/src/pages/index.astro"
  - id: build-clean
...
3. Add the import: `import CtaBanner from '@/components/sections/CtaBanner.astro';` (insert in alphabetical order with other section imports).
4. Add the render: `<CtaBanner />` in the page body, in the position dictated by sections.json. If earlier sections (`<Hero />`, `<SocialProof />`, etc.) are already mounted, place this one after them but before any later-ordered sections.
