---
id: 002-03-build
title: "Build: Trusted by builders"
description: "Build the SocialProof.astro component from SPEC + DESIGN, with real copy from source files."
dependencies:
  - 002-02-design
tags:
  - build
...
checks:
  - id: component-exists
    ...
    cmd: test -f apps/landing/src/components/sections/SocialProof.astro
  - id: component-uses-section-wrapper
    description: "component uses <Section> layout primitive"
    cmd: "test -f apps/landing/src/components/sections/SocialProof.astro && grep -qE '<Section\\s' apps/landing/src/components/sections/SocialProof.astro"
  - id: component-typecheck
    description: astro check passes for this component
    cmd: "test -f apps/landing/src/components/sections/SocialProof.astro && pnpm --filter @converge/landing astro check 2>&1 | (! grep -E 'error.*SocialProof\\.astro')"
  - id: no-hardcoded-hex
    description: no hardcoded hex colors (use brand tokens via Tailwind classes)
    cmd: "test -f apps/landing/src/components/sections/SocialProof.astro && ! grep -qE '#[0-9a-fA-F]{3,6}\\b' apps/landing/src/components/sections/SocialProof.astro"
  - id: no-placeholders
    description: no placeholder copy
    cmd: "test -f apps/landing/src/components/sections/SocialProof.astro && ! grep -qE 'Lorem|placeholder content|TBD|FIXME|TODO:' apps/landing/src/components/sections/SocialProof.astro"
vars:
  prefix: 002
...
# Build: Trusted by builders

Implement `apps/landing/src/components/sections/SocialProof.astro` per `apps/landing/.content/sections/social-proof/DESIGN.md`. All copy must come
...
<Section id="social-proof" padY="lg">
  <Container>
...
1. Read `apps/landing/.content/sections/social-proof/SPEC.md` and `apps/landing/.content/sections/social-proof/DESIGN.md` from previous steps.
