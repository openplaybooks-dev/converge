...
id: 001-03-build
title: "Build: Hero"
description: "Build the Hero.astro component from SPEC + DESIGN, with real copy from source files."
dependencies:
  - 001-02-design
tags:
  - build
...
checks:
  - id: component-exists
    ...
    cmd: test -f apps/landing/src/components/sections/Hero.astro
  - id: component-uses-section-wrapper
    description: "component uses <Section> layout primitive"
    cmd: "test -f apps/landing/src/components/sections/Hero.astro && grep -qE '<Section\\s' apps/landing/src/components/sections/Hero.astro"
  - id: component-typecheck
    description: astro check passes for this component
    cmd: "test -f apps/landing/src/components/sections/Hero.astro && pnpm --filter @converge/landing astro check 2>&1 | (! grep -E 'error.*Hero\\.astro')"
  - id: no-hardcoded-hex
    description: no hardcoded hex colors (use brand tokens via Tailwind classes)
    cmd: "test -f apps/landing/src/components/sections/Hero.astro && ! grep -qE '#[0-9a-fA-F]{3,6}\\b' apps/landing/src/components/sections/Hero.astro"
  - id: no-placeholders
    description: no placeholder copy
    cmd: "test -f apps/landing/src/components/sections/Hero.astro && ! grep -qE 'Lorem|placeholder content|TBD|FIXME|TODO:' apps/landing/src/components/sections/Hero.astro"
vars:
  prefix: 001
  sectionId: hero
  title: Hero
...
Implement `apps/landing/src/components/sections/Hero.astro` per `apps/landing/.content/sections/hero/DESIGN.md`. All copy must come
...
1. Read `apps/landing/.content/sections/hero/SPEC.md` and `apps/landing/.content/sections/hero/DESIGN.md` from previous steps.
