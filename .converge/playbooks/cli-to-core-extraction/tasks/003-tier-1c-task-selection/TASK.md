...
1. **Rename the exported `findNextTask`** → `findNextIncompleteTask`. The TaskTree class in `core/task/tree/` already has its own `findNextTask` method; both being public on `@converge/core` would collide. The CLI's version operates on a planning result, not a tree, so the new name is more accurate anyway.
...
4. Update `@converge/core/...` relative imports to be relative-to-the-new-location.
...
- `packages/cli/src/commands-tree.ts` — update its import from `./next-task.ts` to `@converge/core`.
- `packages/cli/src/tree-display.ts` — update its `ExecutionSpan` import to `@converge/core`.
- `packages/cli/src/autonomous-run.ts` — update its `findNextTask` import to `findNextIncompleteTask` from `@converge/core`. (autonomous-run.ts is still in CLI at this tier; it migrates in 1d.)
