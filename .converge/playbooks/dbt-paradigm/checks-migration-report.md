...
|---|---|
| `typecheck-green`: `pnpm --filter @converge/core --filter @converge/cli typecheck` | `type: test, name: typecheck, args: { pnpm_args: "--filter @converge/core --filter @converge/cli" }` |
| `tests-green`: `pnpm --filter @converge/core --filter @converge/cli test` | `type: test, name: tests-green, args: { pnpm_args: "--filter @converge/core --filter @converge/cli" }` |
...
|---|---|---|
| 01-survey-and-fence | `-r --filter '!@converge/studio' --filter '!@converge/provider-benchmark'` | `-r --filter '@converge/cli'` |
| 02-strip-callsites | `--filter @converge/core --filter @converge/navigator` | `--filter @converge/navigator` |
...
- `cli-builds`: `pnpm -F @converge/cli build 2>&1 | tail -5`
