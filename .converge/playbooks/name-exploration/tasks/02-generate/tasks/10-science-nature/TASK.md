---
id: 10-science-nature
title: Science and Nature Systems
description: "Agency-grade naming territory: Explore networks, alignment, branching, lattices, feedback, signals, strata, swarms, synapses, graphs, kernels, attractors. Favor words that imply rigorous systems converging to stable results."
...
outputs:
  - .converge/artifacts/name-exploration/science-nature-raw.json
  - .converge/artifacts/name-exploration/science-nature-candidates.json
checks:
  - id: raw-exists
    cmd: test -f .converge/artifacts/name-exploration/science-nature-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' .converge/artifacts/name-exploration/science-nature-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f .converge/artifacts/name-exploration/science-nature-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty .converge/artifacts/name-exploration/science-nature-raw.json && jq empty .converge/artifacts/name-exploration/science-nature-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' .converge/artifacts/name-exploration/science-nature-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs .converge/artifacts/name-exploration/science-nature-candidates.json science-nature
    description: Candidates pass schema, no-copy, and basic quality gates
---

# Science and Nature Systems

## Territory Brief

Explore networks, alignment, branching, lattices, feedback, signals, strata, swarms, synapses, graphs, kernels, attractors. Favor words that imply rigorous systems converging to stable results.
...
Write the raw ideas to `artifacts/name-exploration/science-nature-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/science-nature-candidates.json`.
...
```json
{
  ...
  "category": "scientific",
  "strategy_source": "science-nature",
