---
id: 08-abstract-evocative
title: Abstract Evocative Real Words
description: "Agency-grade naming territory: Find short real words with emotional charge: precision, clarity, momentum, structure, craft, trust, completion. Real words are allowed only if they can become ownable as a dev-tool brand."
...
outputs:
  - artifacts/name-exploration/abstract-evocative-raw.json
  - artifacts/name-exploration/abstract-evocative-candidates.json
checks:
  - id: raw-exists
    cmd: test -f artifacts/name-exploration/abstract-evocative-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' artifacts/name-exploration/abstract-evocative-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f artifacts/name-exploration/abstract-evocative-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty artifacts/name-exploration/abstract-evocative-raw.json && jq empty artifacts/name-exploration/abstract-evocative-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' artifacts/name-exploration/abstract-evocative-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs artifacts/name-exploration/abstract-evocative-candidates.json abstract-evocative
    description: Candidates pass schema, no-copy, and basic quality gates
---

# Abstract Evocative Real Words

## Territory Brief

Find short real words with emotional charge: precision, clarity, momentum, structure, craft, trust, completion. Real words are allowed only if they can become ownable as a dev-tool brand.
...
Write the raw ideas to `artifacts/name-exploration/abstract-evocative-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/abstract-evocative-candidates.json`.
...
```json
{
  ...
  "category": "abstract",
  "strategy_source": "abstract-evocative",
