---
id: 03-latin-greek-roots
title: Classical Roots, Modern Sound
description: "Agency-grade naming territory: Use Latin/Greek roots only as raw material. Create short modern names, not academic compounds. Roots: syn, con, telos, ergon, praxis, taxis, graph, verus, opus, iter, nexus."
...
outputs:
  - artifacts/name-exploration/latin-greek-roots-raw.json
  - artifacts/name-exploration/latin-greek-roots-candidates.json
checks:
  - id: raw-exists
    cmd: test -f artifacts/name-exploration/latin-greek-roots-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' artifacts/name-exploration/latin-greek-roots-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f artifacts/name-exploration/latin-greek-roots-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty artifacts/name-exploration/latin-greek-roots-raw.json && jq empty artifacts/name-exploration/latin-greek-roots-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' artifacts/name-exploration/latin-greek-roots-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs artifacts/name-exploration/latin-greek-roots-candidates.json latin-greek-roots
...
Use Latin/Greek roots only as raw material. Create short modern names, not academic compounds. Roots: syn, con, telos, ergon, praxis, taxis, graph, verus, opus, iter, nexus.
...
Write the raw ideas to `artifacts/name-exploration/latin-greek-roots-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/latin-greek-roots-candidates.json`.
...
```json
{
  ...
  "category": "linguistic",
  "strategy_source": "latin-greek-roots",
