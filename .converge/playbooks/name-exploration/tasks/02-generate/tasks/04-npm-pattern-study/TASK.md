---
id: 04-npm-pattern-study
title: Dev Tool Pattern Study
description: "Agency-grade naming territory: Study successful dev-tool naming patterns, then generate original names. Do not imitate existing names. Extract patterns: terse real words, productive suffixes, technical metaphors, playful monosyllables."
...
outputs:
  - artifacts/name-exploration/npm-pattern-study-raw.json
  - artifacts/name-exploration/npm-pattern-study-candidates.json
checks:
  - id: raw-exists
    cmd: test -f artifacts/name-exploration/npm-pattern-study-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' artifacts/name-exploration/npm-pattern-study-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f artifacts/name-exploration/npm-pattern-study-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty artifacts/name-exploration/npm-pattern-study-raw.json && jq empty artifacts/name-exploration/npm-pattern-study-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' artifacts/name-exploration/npm-pattern-study-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs artifacts/name-exploration/npm-pattern-study-candidates.json npm-pattern-study
    description: Candidates pass schema, no-copy, and basic quality gates
---

# Dev Tool Pattern Study

## Territory Brief

Study successful dev-tool naming patterns, then generate original names. Do not imitate existing names. Extract patterns: terse real words, productive suffixes, technical metaphors, playful monosyllables.
...
Write the raw ideas to `artifacts/name-exploration/npm-pattern-study-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/npm-pattern-study-candidates.json`.
...
```json
{
  ...
  "category": "branded",
  "strategy_source": "npm-pattern-study",
