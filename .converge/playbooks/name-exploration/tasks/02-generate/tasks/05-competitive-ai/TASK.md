---
id: 05-competitive-ai
title: AI/Automation White Space
description: "Agency-grade naming territory: Map crowded naming zones in AI orchestration, then avoid them. Find whitespace around proof, runs, plans, graphs, joins, reliability, and autonomous execution without using Agent/AI/Lang/Crew/Chain clichés."
...
outputs:
  - artifacts/name-exploration/competitive-ai-raw.json
  - artifacts/name-exploration/competitive-ai-candidates.json
checks:
  - id: raw-exists
    cmd: test -f artifacts/name-exploration/competitive-ai-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' artifacts/name-exploration/competitive-ai-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f artifacts/name-exploration/competitive-ai-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty artifacts/name-exploration/competitive-ai-raw.json && jq empty artifacts/name-exploration/competitive-ai-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' artifacts/name-exploration/competitive-ai-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs artifacts/name-exploration/competitive-ai-candidates.json competitive-ai
    description: Candidates pass schema, no-copy, and basic quality gates
---

# AI/Automation White Space

## Territory Brief

Map crowded naming zones in AI orchestration, then avoid them. Find whitespace around proof, runs, plans, graphs, joins, reliability, and autonomous execution without using Agent/AI/Lang/Crew/Chain clichés.
...
Write the raw ideas to `artifacts/name-exploration/competitive-ai-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/competitive-ai-candidates.json`.
...
```json
{
  ...
  "category": "branded",
  "strategy_source": "competitive-ai",
