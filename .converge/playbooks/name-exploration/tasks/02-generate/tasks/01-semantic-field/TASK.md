---
id: 01-semantic-field
title: Semantic Field Mining
description: "Agency-grade naming territory: Mine the language of product truth: converge/diverge, join/fork, prove/check, run/finish, plan/playbook, cache/replay, graph/layer. Build clusters, then create names from surprising verbs/nouns and light transformations."
...
outputs:
  - artifacts/name-exploration/semantic-field-raw.json
  - artifacts/name-exploration/semantic-field-candidates.json
checks:
  - id: raw-exists
    cmd: test -f artifacts/name-exploration/semantic-field-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' artifacts/name-exploration/semantic-field-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f artifacts/name-exploration/semantic-field-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty artifacts/name-exploration/semantic-field-raw.json && jq empty artifacts/name-exploration/semantic-field-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' artifacts/name-exploration/semantic-field-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs artifacts/name-exploration/semantic-field-candidates.json semantic-field
...
Mine the language of product truth: converge/diverge, join/fork, prove/check, run/finish, plan/playbook, cache/replay, graph/layer. Build clusters, then create names from surprising verbs/nouns and light transformations.
...
Write the raw ideas to `artifacts/name-exploration/semantic-field-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/semantic-field-candidates.json`.
...
```json
{
  ...
  "category": "semantic",
  "strategy_source": "semantic-field",
