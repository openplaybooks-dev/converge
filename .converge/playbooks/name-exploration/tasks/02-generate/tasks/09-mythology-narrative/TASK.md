---
id: 09-mythology-narrative
title: Narrative and Mythic Names
description: "Agency-grade naming territory: Use myths, literature, and archetypes as story engines, not dusty references. Favor guide/thread/craft/proof/navigation figures. Modernize into spellable, CLI-friendly names."
...
outputs:
  - artifacts/name-exploration/mythology-narrative-raw.json
  - artifacts/name-exploration/mythology-narrative-candidates.json
checks:
  - id: raw-exists
    cmd: test -f artifacts/name-exploration/mythology-narrative-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' artifacts/name-exploration/mythology-narrative-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f artifacts/name-exploration/mythology-narrative-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty artifacts/name-exploration/mythology-narrative-raw.json && jq empty artifacts/name-exploration/mythology-narrative-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' artifacts/name-exploration/mythology-narrative-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs artifacts/name-exploration/mythology-narrative-candidates.json mythology-narrative
    description: Candidates pass schema, no-copy, and basic quality gates
---

# Narrative and Mythic Names

## Territory Brief

Use myths, literature, and archetypes as story engines, not dusty references. Favor guide/thread/craft/proof/navigation figures. Modernize into spellable, CLI-friendly names.
...
Write the raw ideas to `artifacts/name-exploration/mythology-narrative-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/mythology-narrative-candidates.json`.
...
```json
{
  ...
  "strategy_source": "mythology-narrative",
