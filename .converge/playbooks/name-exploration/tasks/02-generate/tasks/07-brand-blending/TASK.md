---
id: 07-brand-blending
title: Controlled Brand Coinage
description: "Agency-grade naming territory: Blend product-truth morphemes into ownable names. Use convergence, proof, play, run, graph, join, fork, cache, retry, green, trace, weave, plan. Avoid obvious mashups like TaskForge or DagRun."
...
outputs:
  - artifacts/name-exploration/brand-blending-raw.json
  - artifacts/name-exploration/brand-blending-candidates.json
checks:
  - id: raw-exists
    cmd: test -f artifacts/name-exploration/brand-blending-raw.json
    description: Raw idea pool exists
  - id: raw-min-75
    cmd: jq -e 'length >= 75' artifacts/name-exploration/brand-blending-raw.json
    description: Raw idea pool has at least 75 ideas
  - id: final-exists
    cmd: test -f artifacts/name-exploration/brand-blending-candidates.json
    description: Final candidates exist
  - id: valid-json
    cmd: jq empty artifacts/name-exploration/brand-blending-raw.json && jq empty artifacts/name-exploration/brand-blending-candidates.json
    description: Raw and final outputs are valid JSON
  - id: final-count
    cmd: jq -e 'length >= 12 and length <= 20' artifacts/name-exploration/brand-blending-candidates.json
    description: Final candidate count is agency-shortlist sized
  - id: agency-schema-and-quality
    cmd: node .converge/playbooks/name-exploration/scripts/check-name-candidates.cjs artifacts/name-exploration/brand-blending-candidates.json brand-blending
    description: Candidates pass schema, no-copy, and basic quality gates
---

# Controlled Brand Coinage

## Territory Brief

Blend product-truth morphemes into ownable names. Use convergence, proof, play, run, graph, join, fork, cache, retry, green, trace, weave, plan. Avoid obvious mashups like TaskForge or DagRun.
...
Write the raw ideas to `artifacts/name-exploration/brand-blending-raw.json` as an array of strings or grouped objects.
Write the final names to `artifacts/name-exploration/brand-blending-candidates.json`.
...
```json
{
  ...
  "category": "branded",
  "strategy_source": "brand-blending",
