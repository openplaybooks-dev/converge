---
id: "{{prefix}}-04-integrate"
title: "Integrate: {{title}}"
description: "Mount the {{componentName}} component in src/pages/index.astro at the correct position."
dependencies: ["{{prefix}}-03-build"]
tags: [integrate, "section-{{sectionId}}"]
inputs:
  - "{{componentPath}}"
...
checks:
  ...
  - id: component-imported
    cmd: "test -f apps/landing/src/pages/index.astro && grep -qE \"import\\s+{{componentName}}\\s+from\" apps/landing/src/pages/index.astro"
    description: "{{componentName}} is imported in index.astro"
...
Add the import + render of `<{{componentName}}>` to
`apps/landing/src/pages/index.astro` at the right position.

The "right position" is the order defined in
`apps/landing/.content/sections.json`. Section #{{prefix}} renders in
position #{{prefix}}.
