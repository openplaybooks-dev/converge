...
checks:
  ...
  - id: probe-manifest-edges-from-parent-map
    ...
    description: Manifest exposes parent_map and child_map. Phase 04 of declarative-discovery sources these from declarations rather than from path nesting.
  - id: probe-libraries-section-in-manifest
    cmd: |
      f=$(find . -path '*/target/manifest.json' 2>/dev/null | head -1)
      test -n "$f" && jq -e '.libraries | has("seeds") and has("tests")' "$f" >/dev/null
    description: dbt-paradigm's libraries section is present (seeds + tests). Phase 04 of declarative-discovery preserves this section unchanged.
  - id: probe-child-synthesizer-callable
    ...
    description: child-synthesizer exports a callable `synthesize` function. Phase 04 of declarative-discovery uses it as the integration point for spawning-seed children.
  - id: probe-seed-spawner-honors-path
    ...
    description: seed-spawner exports the expected entry function. Phase 04 of declarative-discovery extends it to register children at custom paths.
  - id: probe-seed-md-schema-shape
    ...
    description: seed-md-definition exports a parser. Phase 02 of declarative-discovery references the seed registry from `from_seed:` declarations.
  ...
  - id: probe-no-wbs-frontmatter-field
    ...
    description: task-md schema has no wbs: frontmatter field. Phase 02 of declarative-discovery does not need to handle wbs: in the new schema.
  ...
  - id: probe-seed-and-test-selectors-work
    ...
    description: `--select 'seed:<name>'` and `test:<name>` selectors are registered. Phase 04 of declarative-discovery does not add new selectors; the existing grammar must still work.
  - id: probe-fixture-has-seeds-and-tests
    ...
    description: minimal-playbook fixture has seeds/ and tests/ directories. Phase 02 of declarative-discovery extends the same fixture with `children:` declarations.
  - id: report-present
...
directly shape this playbook's phases 02 and 04 contracts.
...
- **Renames**: `child-synthesizer.synthesize` could be
  `synthesize-child` or split into multiple exports. Update phase 04's
  references.
- **Added required arguments**: if `synthesize(parent, entry)` became
  `synthesize(parent, entry, ctx)`, phase 04's wiring needs the third
  arg.
- **Manifest schema additions**: dbt-paradigm phase 03 may have added
  fields to `libraries:` that phase 04 here didn't account for. Phase
  04 should pass them through unchanged.
...
value, and "PASS" or "PASS (with note: ...)". The note field is for
recording any contract that's *technically* the same but renamed (e.g.
"function is called `synthesize` not `synthesizeChild` as expected;
phase 04 uses the actual name").
