...
```mermaid
graph LR
    D["{id}-diverge<br/>writes &lt;id&gt;/spawn.yml<br/>framework expands + applies"] -->|"children complete"| C["{id}-converge<br/>runs TASK.md body<br/>integrates results"]
```

- **`{id}-diverge`** runs first. For `mode: spawner` it writes `<id>/spawn.yml` invocations under `$CONVERGE_SPAWN_DIR` and the framework expands them against templates; for static children it simply marks itself done. Children depend on it.
...
The body runs only during convergence. Division is handled by the `mode: spawner` body (which writes `<id>/spawn.yml` invocations) or by static children in the `tasks/` directory. The body of a converging parent is pure convergence instructions: what to do with children's outputs.
...
There's no `## Division` section in the body. The spawner body handles spawning children (via `<id>/spawn.yml` invocations); the converger body handles integrating their results. The LLM reads it like any other task prompt: the difference is timing — the converge node only runs after all children complete.
...
2. **During execution**, the diverge node runs first. For `mode: spawner`, the body writes `<id>/spawn.yml` invocations under `$CONVERGE_SPAWN_DIR` and the framework expands them against templates and applies; children are then wired to the converge node's `depends_on`.
