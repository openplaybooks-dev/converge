...
Resolves the playbook DAG and writes `target/manifest.json` with every known task node, annotated with `concrete` / `expected` / `frontier` state. No task execution by default. With `--seed`, runs WBS scripts of selected parents to materialize their children to disk — turning `frontier` nodes into `concrete` ones.
...
| `--seed` | off | Run WBS scripts of selected parents, materialize children to disk, rewrite the manifest. Cheap — scripts run, task bodies don't. |
| `--inc` | off | With `--seed`, allow re-seeding already-seeded WBS parents. |
...
#   concrete:  4
#   expected:  0
#   frontier:  1   (03-tokens/002-craft has unseeded WBS)
...
# Materialize WBS children to disk (scripts only, no task body execution).
...
- **`--seed` is cheap** — it runs WBS scripts (typically read-a-file-and-spawn) but not the expensive task work (LLM calls, rendering).
