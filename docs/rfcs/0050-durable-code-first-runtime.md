...
| **Dogfood migration: tests/* → playbook.js** | **done** | `.converge/playbooks/migrate-tests-to-flow/` is itself a flow: discovers fixture folder playbooks, fans out (workers 3) one `tasks/convert-fixture/TASK.md` per fixture (relative-path call + per-item key), each MiniMax-driven: write `playbook.js` (relative `task()` paths) → remove `playbook.yml` → verify (`node --check` + `converge compile`). **Static** fixtures clean-break (YAML removed); **dynamic** fixtures (spawner/seed/converger — `converge spawn`/`apply`) keep YAML (a static flow can't reproduce a runtime-shaped DAG; the gate now STOPs on dynamic). Ran ~17 static fixtures to flows; reverted the dynamic ones. |
| **`ctx.spawn()` — multi-level template spawning** | **done** | `spawn(template, vars, opts)` runs a template with the **strict vars contract** (reuses `task/spawn/render.ts:buildChildVars`: drops undeclared vars, fills defaults, throws on missing required) — journaled/resumable like `task()`. Multi-level fan-out is recursion (no inventory ledger). **Result-driven pattern:** a child task emits a `{subs:[…]}` manifest as its output; the flow fans out grandchildren from that result (`const m = await spawn("child"); parallel(m.subs.map(s => () => spawn("sub", …)))`). Example: `tests/test-seeding-flow/` reproduces test-seeding's 3-level tree offline (child-alpha emits N=wave subs, child-beta 2; `owner` filtered for the beta branch). 3 unit tests + the example, flow suite 15/15 green; `tests/test-seeding` (YAML) untouched. |
