...
Every task does one thing: **execute a function**. The modifiers control _how_ it runs.
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| _(none)_           | Runs `convergeFn` → `executeFn` sequentially, blocks until converged                                                                                                                 |
...
Note: no `command` field — the command lives in `.execute()`, not in the config. The config only has _behavior modifiers_.
  ...
  .execute(async (ctx) => {
    try {
      ...
      if (res.status !== 200) {
        ctx.emitGap({
          severity: "critical",
          description: `Dev returned ${res.status}`,
        });
      }
    } catch {
      ctx.emitGap({
        severity: "critical",
        description: "Dev server unreachable",
      });
    }
  })
  .build();
...
`.execute()` is optional for sidecars. The hook callbacks _are_ the execution. But you can also provide an `.execute()` for init/setup logic.
...
export const tasks = [
  ...
  taskDef()
    ...
    .execute(async (ctx) => {
      const res = await fetch("http://localhost:3000");
      if (!res.ok)
        ctx.emitGap({ severity: "critical", description: "Dev unreachable" });
    })
    .build(),
