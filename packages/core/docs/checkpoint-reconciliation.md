...
4. **Manual fixes** where user creates missing files outside Converge
...
💡 Run `pnpm converge validate --fix` to auto-fix reconcilable issues.
```
...
```bash
pnpm converge run        # Auto-reconciles before execution
pnpm converge gantt      # Auto-reconciles before display
pnpm converge tree       # Auto-reconciles before display
```
...
```bash
CONVERGE_DEBUG_DEPS=true pnpm converge gantt
```
...
```bash
cat .converge/journal/epics/{epic}/tasks/{task}/checkpoint.json
cat .converge/journal/epics/{epic}/tasks/{task}/attempts.jsonl
```
