...
Standard `converge run` executes tasks and marks them complete or failed. That's it. If a task stalls after fixing 7 of 10 gaps, all 7 fixes are thrown away. Next run starts from scratch with 10 gaps.
...
Every converge run appends a snapshot to `.converge/journal/gap-ledger.jsonl`:
...
View the trend with `converge trend`:
...
```bash
# Filter to one epic
converge run --converge --filter 02-implementation

# Verbose (shows score breakdown by kind)
converge run --converge --verbose

# Limit iterations
converge run --converge --max-iterations=20

# Force re-run a specific task (even if partial/completed)
converge run --converge --filter 02-implementation/003-build-ui --force
```
...
converge run --converge --resume

# Or: reset all non-complete tasks and start fresh
converge run --converge --restart
```
...
To continue, use one of:
   converge run --converge --resume    # recover interrupted tasks
   converge run --converge --restart   # reset all tasks to pending
```
...
The process can die at any point — kill -9, OOM, power loss, cloud spot instance reclaim. No signal handler will fire. Safety comes from the same model as the core Converge framework:
...
```
converge run --converge [--resume|--restart]
│
