...
Introduce a **plugin system** that lets users compose project configuration from reusable, stackable tech-stack plugins. Instead of writing checks, task types, hooks, and vars from scratch in `.converge/setup/index.js`, users declare plugins like `typescript`, `nextjs`, `git`, `docker` — and the framework merges their contributions into a unified configuration.
...
Today, every project must manually define checks, task types, and hooks in `.converge/setup/index.js`:
...
```typescript
export interface ConvergePlugin {
...
Add "typescript" before "nextjs" in converge.json plugins array.
```
...
```typescript
export interface PluginAPI {
  /** Options passed from converge.json plugin configuration */
...
Plugins don't replace `.converge/setup/index.js` — they complement it. Manual setup always runs **after** plugins, so it can override or extend anything:

```javascript
// .converge/setup/index.js
...
```
converge.json plugins: ["typescript", "nextjs"]
         ↓
  ...
  nextjs.setup(api)      →  api.addCheck('build', ...)    →  registerCheck('build', ...)
         ...
         ↓
  .converge/setup/index.js   →  (manual overrides/additions)
```
