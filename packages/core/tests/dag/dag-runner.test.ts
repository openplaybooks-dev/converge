  ...
  for (const node of dag.nodes.values()) {
    for (const childId of node.children) {
      if (!added.has(childId)) {
        added.add(childId);
        dag.addNode({
          id: childId, parents: [], children: [],
          depends_on: [], depended_on_by: [],
          taskDef: { id: childId, title: childId, description: '', body: '', checks: [] } as TaskDefinition,
...
describe("DAG runner", () => {
  describe("Linear A→B→C", () => {
    it("executes A before B, B before C", async () => {
      const dag = makeDag([['A', ['B']], ['B', ['C']]]);
      const { execute, calls } = mockExecute();

      await executeDag(dag, async (node) => { await execute(node.id); }, { projectDir: '/test' });
  ...
  describe("Diamond A→[B,C]→D", () => {
    it("executes A first, B and C after A, D last after both B and C", async () => {
      const dag = makeDag([['A', ['B', 'C']], ['B', ['D']], ['C', ['D']]]);
      const { execute, calls } = mockExecute();
