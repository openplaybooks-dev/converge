...
describe("Tree Display - Epic Indicators", () => {
  ...
  it("should mark epic with running task with ▶ indicator", () => {
    ...
    // Find epic lines
    const epic01Line = output.find((line) =>
      line.includes("01-prepare-requirements"),
    );
    const epic02Line = output.find((line) =>
      line.includes("02-prepare-designs"),
    );
  ...
  it("should NOT mark epic without active tasks with ▶ indicator", () => {
    const tree: TaskNode[] = [
      ...
      },
      {
        ...
        filePath:
          ".converge/epics/03-implement-app/001-implement-design-system/TASK.md",
        relPath:
          ".converge/epics/03-implement-app/001-implement-design-system/TASK.md",
  ...
  it("should mark parent task epic when child is next task", () => {
    ...
    const parentLine = output.find(
      (line) =>
        line.includes("003-generate-screens") && !line.includes("003-001"),
    );
