 ...
 *   .converge/journal/{pb}/tasks/{parent}/seed-input.json
 *   .converge/journal/{pb}/tasks/{parent}/seed-output.json
 *
 * The input snapshot captures what the Seed script sees in `ctx`; the output
...
describe("SeedExecutor — seed-input.json / seed-output.json debug artifacts", () => {
  beforeEach(() => {
    ROOT = join(
      tmpdir(),
      `converge-seed-debug-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it("writes seed-input.json capturing vars, taskMeta, and ctx surface", async () => {
    ...
    const input = readJson(join(taskDir(ROOT), "seed-input.json"));
  ...
  it("writes seed-output.json with status=success and spawned tasks", async () => {
    const parentDir = join(ROOT, ".converge/playbooks/default/tasks/epic-1");
    const exec = new SeedExecutor(
      ROOT,
      { epicId: "epic-1", taskId: "epic-1" },
    ...
    const output = readJson(join(taskDir(ROOT), "seed-output.json"));
  ...
  it("writes seed-output.json with status=error and the thrown error on failure", async () => {
    const parentDir = join(ROOT, ".converge/playbooks/default/tasks/epic-1");
    const exec = new SeedExecutor(
      ROOT,
      { epicId: "epic-1", taskId: "epic-1" },
  ...
  it("routes seed-input/output.json to the CHILD task dir when a spawned child runs its own Seed", async () => {
    // Mirror the production flow: after the root seeder spawns `epic-1/child`,
    // the child's own Seed run should write its debug artifacts at the child's
    ...
    expect(existsSync(join(childDir, "seed-input.json"))).toBe(true);
    expect(existsSync(join(childDir, "seed-output.json"))).toBe(true);
    expect(existsSync(join(childDir, "seed.json"))).toBe(true);

    const input = readJson(join(childDir, "seed-input.json"));
  ...
  it("writes seed-output.json with status=zero-spawn when the script seeds nothing", async () => {
    const parentDir = join(ROOT, ".converge/playbooks/default/tasks/epic-1");
    const exec = new SeedExecutor(
      ROOT,
      { epicId: "epic-1", taskId: "epic-1" },
    ...
    const outputPath = join(taskDir(ROOT), "seed-output.json");
    expect(existsSync(outputPath)).toBe(true);
    const output = readJson(outputPath);
    expect(output.status).toBe("zero-spawn");
    expect(output.spawnCount).toBe(0);
    expect(output.error?.name).toBe("SeedZeroSpawnError");
  });
});
