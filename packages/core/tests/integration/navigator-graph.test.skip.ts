...
import {
  ...
  injectSubgraph,
  buildNodes,
} from "../../src/repair/navigator/walker.ts";
...
describe("Core Walking", () => {
  ...
  it("emits edges for each action executed", async () => {
    ...
    const reg = registry(["a", handler()], ["b", handler()]);
...
describe("Condition Branching", () => {
  it("executes paired action when condition is true", async () => {
    ...
    const reg = registry([
      "signal-done",
      trackedHandler(calls, "signal-done", { action: "done" }),
    ]);
  ...
  it("advances past condition with no paired action at end", async () => {
    ...
    const reg = registry(["first", trackedHandler(calls, "first")]);
...
describe("Statelessness", () => {
  it("original snapshot is not mutated after walkGraph", async () => {
    ...
    const graph = makeGraph([actionNode("a")]);
    const reg = registry(["a", handler({ gaps: [makeGap()], stallCount: 3 })]);
...
describe("Edge Accumulation", () => {
  ...
  it("condition edges include condition field", async () => {
    ...
    const reg = registry(["signal-done", handler({ action: "done" })]);
...
describe("hasBlocker Nested Block", () => {
  function blockerGraph(calls: string[]): {
    graph: Graph;
    ...
    const reg = registry(
      ...
      [
        "bail-blockers",
        trackedHandler(calls, "bail-blockers", {
          action: "bail",
          reason: "blockers",
        }),
      ],
  ...
  it("hasBlocker(true) + isFirst(true) → resolve-blockers executed", async () => {
    ...
    expect(calls.indexOf("resolve-blockers")).toBeLessThan(
      calls.indexOf("bail-blockers"),
    );
  });
  ...
  it("hasBlocker(true) + isFirst(false) → bail-blockers", async () => {
    ...
    const result = await walkGraph(
      graph,
      makeSnap({ gaps: [blockerGap], iteration: 2 }),
      reg,
    );
...
describe("Injection During Walk", () => {
  it("action returning inject walks injected nodes immediately", async () => {
    const calls: string[] = [];
    const graph = makeGraph([actionNode("a"), actionNode("c")]);
    const reg = registry(
      [
        "a",
        async () => ({
  ...
  it("injected action returning done stops the walk", async () => {
    ...
    const reg = registry(
      ...
      [
        "stopper",
        async () => ({ action: "done" as const, reason: "injected done" }),
      ],
      ["c", trackedHandler(calls, "c")],
    );
...
describe("Terminal Conditions", () => {
  ...
  it("bail stops walk and emits edge to _end", async () => {
    ...
    const reg = registry([
      "a",
      async () => ({ action: "bail" as const, reason: "fatal" }),
    ]);
  ...
  it("bail with reason propagates reason in result", async () => {
    ...
    const reg = registry([
      "a",
      async () => ({ action: "bail" as const, reason: "something broke" }),
    ]);
...
describe("evalPredicate", () => {
  ...
  it("noGaps is false when gaps present", () => {
    expect(evalPredicate("noGaps", makeSnap({ gaps: [makeGap()] }))).toBe(
      false,
    );
  });
  ...
  it("hasRepairableGap detects output/checkFailed/corrupted", () => {
    ...
    expect(
      evalPredicate("hasRepairableGap", makeSnap({ gaps: [outputGap] })),
    ).toBe(true);

    const checkGap = makeGap({ metadata: { gapKind: GapKind.checkFailed } });
    expect(
      evalPredicate("hasRepairableGap", makeSnap({ gaps: [checkGap] })),
    ).toBe(true);

    const corruptedGap = makeGap({ metadata: { gapKind: GapKind.corrupted } });
    expect(
      evalPredicate("hasRepairableGap", makeSnap({ gaps: [corruptedGap] })),
    ).toBe(true);
  });

  it("last-succeeded reads lastActionSucceeded", () => {
    expect(
      evalPredicate("last-succeeded", makeSnap({ lastActionSucceeded: true })),
    ).toBe(true);
    expect(
      evalPredicate("last-succeeded", makeSnap({ lastActionSucceeded: false })),
    ).toBe(false);
  });
...
describe("TaskContext Persistence", () => {
  ...
  it("WalkerState preserves node results through serialization", async () => {
    ...
    expect(raw.nodes[0].result).toEqual({
      selectedStrategy: "task-run",
      gapCount: 2,
    });
  });
});
  ...
  it("repeated walks until done (simulating run.ts loop)", async () => {
    ...
    const buildGraphAndReg = () => {
      ...
      const reg = registry(
        ...
        [
          "signal-done",
          async () => ({ action: "done" as const, success: true }),
        ],
        ["fix", handler()],
      );
