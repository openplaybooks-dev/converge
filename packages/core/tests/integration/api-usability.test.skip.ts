...
describe("API Usability - Real World Patterns", () => {
  ...
  beforeEach(() => {
    testDir = join(tmpdir(), `converge-api-test-${Date.now()}`);
  ...
  describe("Builder API Ergonomics", () => {
    ...
    it("should create a parent task with yields", () => {
      const task = taskDef()
        ...
        .yields({
          ...
          outputDir: ".converge/screens",
          template: "000-screen-{slug}.ts.tpl",
          maxTasks: 20,
        })
        .inputs([".stitch/SITE.md"])
        .outputs([".converge/screens/**/*.ts"])
        .build();
    ...
    it("should add multiple checks easily", () => {
      const task = taskDef()
        ...
        .check({
          id: "bundle-size",
          cmd: "test $(stat -f%z dist/bundle.js) -lt 1000000",
        })
  ...
  describe("Error Handling", () => {
    it("should throw clear error when TaskDefinition missing ID", () => {
      expect(() => {
        taskDef().title("No ID Task").build();
  ...
  describe("API Consistency", () => {
    ...
    it("should have sensible defaults", () => {
      const minimalTask = taskDef().id("minimal").build();
  ...
  describe(".execute() — Unified Execution Entry Point", () => {
    ...
    it("should be equivalent to .executor()", () => {
      ...
      const viaExecute = taskDef().id("task-a").execute(fn).build();

      const viaExecutor = taskDef().id("task-b").executor(fn).build();
    ...
    it("should wire through Unit to executorFn", () => {
      ...
      const def = taskDef().id("my-task").title("My Task").execute(fn).build();
    ...
    it("should work with .converge() for convergence wrapping", () => {
      ...
      const task = taskDef()
        ...
        .execute(fn)
        .converge() // default converge wrapping
        .build();
