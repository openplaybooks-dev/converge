...
describe("Path Resolution Consistency", () => {
  ...
  it("should match for root-level task (no /tasks/ subdirectory)", () => {
    ...
    const journalFromStructure = getJournalStructure(
      projectDir,
      epicId,
      taskId,
    ).task;
    ...
    expect(journalFromStructure).toBe(
      path.join(projectDir, ".converge/journal/default/tasks", epicId, taskId),
    );
  });
  ...
  it("should match for WBS subtask (under /tasks/ subdirectory)", () => {
    ...
    const taskPath = path.join(
      ...
      ".converge/epics",
      epicId,
      "002-pages/tasks/002-001-prompt-home-dashboard",
      "TASK.md",
    );
    ...
    expect(journalFromStructure).toBe(
      path.join(
        projectDir,
        ".converge/journal/default/tasks",
        epicId,
        "002-pages/tasks/002-001-prompt-home-dashboard",
      ),
    );
