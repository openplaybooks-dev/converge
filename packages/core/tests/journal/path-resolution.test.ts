...
describe('Path Resolution Consistency', () => {
  ...
  it('should match for root-level task (no /tasks/ subdirectory)', () => {
    ...
    const taskPath = path.join(projectDir, '.converge/epics', epicId, taskId, 'TASK.md');
    ...
    expect(journalFromStructure).toBe(path.join(projectDir, '.converge/journal/default/tasks', epicId, taskId));
  });
  ...
  it('should match for WBS subtask (under /tasks/ subdirectory)', () => {
    ...
    const taskPath = path.join(projectDir, '.converge/epics', epicId, '002-pages/tasks/002-001-prompt-home-dashboard', 'TASK.md');
    ...
    expect(journalFromStructure).toBe(path.join(projectDir, '.converge/journal/default/tasks', epicId, '002-pages/tasks/002-001-prompt-home-dashboard'));
  });
  ...
  it('should match for deeply nested task', () => {
    ...
    const taskPath = path.join(projectDir, '.converge/epics', epicId, '002-pages/tasks/002-001-home', 'TASK.md');

    const journalFromStructure = getJournalStructure(projectDir, epicId, taskId).task;
    const journalFromConstruct = constructJournalPath(taskPath);

    expect(journalFromStructure).toBe(journalFromConstruct);
    expect(journalFromStructure).toBe(path.join(projectDir, '.converge/journal/default/tasks', epicId, '002-pages/tasks/002-001-home'));
  });
  ...
  it('should never add /tasks/ if not present in taskId', () => {
    ...
    expect(journalPath).toBe(path.join(projectDir, '.converge/journal/default/tasks', epicId, taskId));
  });
  ...
  it('should handle hierarchical taskId with two segments', () => {
    ...
    expect(journalPath).toBe(path.join(projectDir, '.converge/journal/default/tasks', epicId, 'parent/tasks/002-001-home'));
  });
});
