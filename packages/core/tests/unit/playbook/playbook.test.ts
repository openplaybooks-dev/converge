...
beforeEach(async () => {
  tmpDir = join(tmpdir(), `converge-pb-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
...
describe('resolvePlaybookPaths', () => {
  it('named playbook', () => {
    ...
    expect(p.tasks).toBe(join('/project', '.converge', 'playbooks', 'fix-issue', 'tasks'));
    expect(p.journalTasks).toBe(join('/project', '.converge', 'journal', 'fix-issue', 'tasks'));
    expect(p.config).toBe(join('/project', '.converge', 'playbooks', 'fix-issue', 'playbook.yml'));
  });

  it('default playbook', () => {
    const p = resolvePlaybookPaths('/project');
    expect(p.playbook).toBe('default');
    expect(p.tasks).toBe(join('/project', '.converge', 'playbooks', 'default', 'tasks'));
    expect(p.config).toBe(join('/project', '.converge', 'playbooks', 'default', 'playbook.yml'));
  });
