...
describe("format rules", () => {
  ...
  describe("id-format", () => {
    ...
    it("passes with NN-kebab format (epic-level task)", () => {
      const issues = runRule(
        "id-format",
        makeInput({ id: "01-prepare-requirements" }),
      );
      expect(issues).toHaveLength(0);
    });
  ...
  describe("title-is-string", () => {
    it("flags non-string title", () => {
      const issues = runRule(
        "title-is-string",
        makeInput({ id: "001-test" }, { title: 123 }),
      );
      expect(issues).toHaveLength(1);
    });
    ...
    it("passes with string title", () => {
      const issues = runRule(
        "title-is-string",
        makeInput({ id: "001-test" }, { title: "My Task" }),
      );
      expect(issues).toHaveLength(0);
    });
  ...
  describe("array field rules", () => {
    ...
    for (const ruleId of arrayFields) {
      ...
      it(`${ruleId}: flags non-array ${field}`, () => {
        const issues = runRule(
          ruleId,
          makeInput({ id: "001-test" }, { [field]: "not-an-array" }),
        );
        expect(issues).toHaveLength(1);
        expect(issues[0].severity).toBe("error");
      });
      ...
      it(`${ruleId}: passes with array ${field}`, () => {
        const issues = runRule(
          ruleId,
          makeInput({ id: "001-test" }, { [field]: ["item"] }),
        );
        expect(issues).toHaveLength(0);
      });
  ...
  describe("blocking-is-boolean", () => {
    it("flags non-boolean blocking", () => {
      const issues = runRule(
        "blocking-is-boolean",
        makeInput({ id: "001-test" }, { blocking: "yes" }),
      );
      expect(issues).toHaveLength(1);
    });
    ...
    it("passes with boolean blocking", () => {
      const issues = runRule(
        "blocking-is-boolean",
        makeInput({ id: "001-test" }, { blocking: true }),
      );
      expect(issues).toHaveLength(0);
    });
  });
  ...
  describe("executor-type-valid", () => {
    it("flags unknown executor type", () => {
      const issues = runRule(
        "executor-type-valid",
        makeInput({ id: "001-test" }, { executor: { type: "python" } }),
      );
      expect(issues).toHaveLength(1);
    });
    ...
    it("passes with valid executor type", () => {
      for (const type of ["ai", "script", "function"]) {
        const issues = runRule(
          "executor-type-valid",
          makeInput({ id: "001-test" }, { executor: { type } }),
        );
        expect(issues).toHaveLength(0);
      }
    });
  ...
  describe("needs-shape", () => {
    it("flags needs entry without id", () => {
      const issues = runRule(
        "needs-shape",
        makeInput({ id: "001-test" }, { needs: [{ cmd: "which node" }] }),
      );
      expect(issues).toHaveLength(1);
    });
  });
  ...
  describe("correction-budget-positive", () => {
    it("flags negative budget", () => {
      const issues = runRule(
        "correction-budget-positive",
        makeInput({ id: "001-test" }, { "correction-budget": -1 }),
      );
      expect(issues).toHaveLength(1);
    });
    ...
    it("flags zero budget", () => {
      const issues = runRule(
        "correction-budget-positive",
        makeInput({ id: "001-test" }, { "correction-budget": 0 }),
      );
      expect(issues).toHaveLength(1);
    });
    ...
    it("flags non-number budget", () => {
      const issues = runRule(
        "correction-budget-positive",
        makeInput({ id: "001-test" }, { "correction-budget": "five" }),
      );
      expect(issues).toHaveLength(1);
    });
    ...
    it("passes with positive budget", () => {
      const issues = runRule(
        "correction-budget-positive",
        makeInput({ id: "001-test" }, { "correction-budget": 3 }),
      );
      expect(issues).toHaveLength(0);
    });
  });
  ...
  describe("unknown-reserved-like-key", () => {
    ...
    it("does not flag actual reserved keys", () => {
      const issues = runRule(
        "unknown-reserved-like-key",
        makeInput({ id: "001-test" }, { dependencies: ["foo"], title: "Test" }),
      );
      expect(issues).toHaveLength(0);
    });
    ...
    it("does not flag unrelated custom keys", () => {
      const issues = runRule(
        "unknown-reserved-like-key",
        makeInput({ id: "001-test" }, { myCustomVariable: "hello" }),
      );
      expect(issues).toHaveLength(0);
    });
  });
