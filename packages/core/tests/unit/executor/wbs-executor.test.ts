...
describe('WbsExecutor', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join(tmpdir(), 'converge-wbs-test-'));
    projectDir = tempDir;

    // Create basic project structure
    mkdirSync(join(projectDir, '.converge'), { recursive: true });
    mkdirSync(join(projectDir, '.converge', 'journal'), { recursive: true });
    mkdirSync(join(projectDir, '.converge', 'epics'), { recursive: true });
    mkdirSync(join(projectDir, '.converge', 'epics', '03-implement-app'), { recursive: true });
    mkdirSync(join(projectDir, '.converge', 'epics', '03-implement-app', '002-pages'), { recursive: true });
  ...
  describe('spawn path generation', () => {
    it('should spawn subtasks to tasks/ (plural) folder', async () => {
      const taskFilePath = join(projectDir, '.converge', 'epics', '03-implement-app', '002-pages', 'TASK.md');
      ...
      const wbsFn = async (ctx: any) => {
        ...
        ctx.spawn = async (target: any, opts?: any) => {
          ...
          const relParentDir = '.converge/epics/03-implement-app/002-pages';
  ...
  describe('path structure validation', () => {
    it('should match expected pattern: parent-basename/tasks/child-id/TASK.md', async () => {
      ...
      expect(capturedPath).toBe('.converge/epics/03-implement-app/002-pages/tasks/002-001-home/TASK.md');
