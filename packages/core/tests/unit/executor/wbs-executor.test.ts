...
describe("WbsExecutor", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(projectDir, ".converge", "epics", "03-implement-app"), {
      recursive: true,
    });
    mkdirSync(
      join(projectDir, ".converge", "epics", "03-implement-app", "002-pages"),
      { recursive: true },
    );
  ...
  describe("spawn path generation", () => {
    it("should spawn subtasks to tasks/ (plural) folder", async () => {
      ...
      const wbsFn = async (ctx: any) => {
        ...
        await ctx.spawn(
          taskDef().id("002-001-home").title("Home Page").build(),
        );
      };
    ...
    it("should maintain consistent path structure for multiple spawned tasks", async () => {
      ...
      const wbsFn = async (ctx: any) => {
        ...
        await ctx.spawn(
          taskDef().id("002-002-about").title("About Page").build(),
        );
        await ctx.spawn(
          taskDef().id("002-003-contact").title("Contact Page").build(),
        );
      };
