...
describe('path-utils', () => {
  describe('extractJournalTaskId', () => {
    it('should extract leaf task ID for root tasks', () => {
      const taskPath = '.converge/epics/03-implement-app/002-generate-react-pages';
    ...
    it('should extract hierarchical ID for WBS subtasks', () => {
      const taskPath = '.converge/epics/03-implement-app/002-generate-react-pages/tasks/002-001-page-home-dashboard';
      const result = extractJournalTaskId(taskPath);
    ...
    it('should extract hierarchical ID for deeply nested tasks', () => {
      const taskPath = '.converge/epics/03-implement-app/002-pages/tasks/002-001-home/tasks/002-001-001-header';
      const result = extractJournalTaskId(taskPath);
      expect(result).toBe('002-pages/002-001-home/002-001-001-header');
    });

    it('should work with TASK.md file paths', () => {
      const taskPath = '.converge/epics/03-implement-app/002-generate-react-pages/tasks/002-001-page-home-dashboard/TASK.md';
      const result = extractJournalTaskId(taskPath);
      expect(result).toBe('002-generate-react-pages/002-001-page-home-dashboard');
    });
  ...
  describe('extractEpicId', () => {
    ...
    it('should extract epic ID from nested task path', () => {
      const taskPath = '.converge/epics/03-implement-app/002-pages/tasks/002-001-home/TASK.md';
      const result = extractEpicId(taskPath);
      expect(result).toBe('03-implement-app');
    });
  ...
  describe('extractEpicDir', () => {
    it('should extract epic directory path', () => {
      const taskPath = '.converge/epics/03-implement-app/002-pages/tasks/002-001-home';
      const result = extractEpicDir(taskPath);
      expect(result).toBe('.converge/epics/03-implement-app');
    });

    it('should work with absolute paths', () => {
      const taskPath = '/Users/minh/project/.converge/epics/03-implement-app/002-pages';
      const result = extractEpicDir(taskPath);
      expect(result).toBe('/Users/minh/project/.converge/epics/03-implement-app');
    });
  });
  ...
  describe('constructJournalPath', () => {
    it('should mirror epic structure exactly for root tasks', () => {
      ...
      expect(result).toBe('.converge/journal/default/tasks/03-implement-app/002-pages');
    });

    it('should mirror epic structure exactly for WBS subtasks', () => {
      const taskPath = '.converge/epics/03-implement-app/002-pages/tasks/002-001-home';
      const result = constructJournalPath(taskPath);
      expect(result).toBe('.converge/journal/default/tasks/03-implement-app/002-pages/tasks/002-001-home');
    });

    it('should mirror epic structure exactly for deeply nested tasks', () => {
      const taskPath = '.converge/epics/03-implement-app/002-pages/tasks/002-001-home/tasks/002-001-001-header';
      const result = constructJournalPath(taskPath);
      expect(result).toBe('.converge/journal/default/tasks/03-implement-app/002-pages/tasks/002-001-home/tasks/002-001-001-header');
    });
    ...
    it('should work with absolute paths', () => {
      ...
      const result = constructJournalPath(taskPath);
      expect(result).toBe('/Users/minh/project/.converge/journal/default/tasks/03-app/002-pages');
    });
