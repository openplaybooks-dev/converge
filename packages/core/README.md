...
**Next-generation convergence framework for deterministic, reproducible, and extremely complex AI workflows.**
...
Converge takes a different approach: **convergence**. Instead of defining what steps to take, you define what the finished state looks like. Converge continuously measures the gap between current reality and that target, generates work to close it, executes, verifies, self-corrects, and repeats — until the project is done.

---

## Use Cases

Converge is general-purpose. If the work can be expressed as a verifiable target state — files that must exist, checks that must pass, invariants that must hold — Converge can orchestrate it autonomously.
...
Run reproducible computational experiments: hypothesis formulation → data collection → analysis → visualization → reporting. Converge ensures each stage produces verified artifacts before the next begins, making experiments reproducible across runs.
...
3. **Let Converge converge** — it detects gaps, executes, verifies, self-corrects, and repeats
...
Converge is built on four core principles:
...
Unlike static pipeline systems that execute a predetermined plan, Converge adapts to actual project state, spawning new tasks just-in-time to close detected gaps.
...
Create `.converge/playbooks/default/playbook.yml`:
...
Create your first task at `.converge/playbooks/default/tasks/01-hello/TASK.md`:
...
Converge operates at three distinct levels. Each layer handles a different scope of work.
...
Playbooks are the top-level entry point for Converge workflows. A playbook defines a complete workflow — its execution mode, retry policy, and the tasks it contains. Tasks live as TASK.md files inside the playbook's `tasks/` directory.
...
Goals define measurable quality metrics with targets. When a goal's metric doesn't meet its target, the framework identifies the gap and spawns corrective tasks to close it. Goals are how Converge knows what "done" looks like.
...
Converge models work as a tree. Numeric prefixes on directories control execution order; nesting creates parent-child relationships. Parent tasks complete only when all children complete.
...
Tasks can reference Work Breakdown Structure scripts that spawn child tasks dynamically at runtime. The parent task doesn't complete until all spawned children are done. This is how Converge handles "we don't know the full scope yet" workflows.
...
Beyond the per-attempt self-correction loop, Converge has an AI-driven **repair system** that analyzes task failures at a higher level and applies targeted fixes before the next attempt. Repair strategies include:
...
Converge continuously detects gaps between desired and actual project state. Goals define the target state; the gap detector evaluates which goals are unmet and generates work to close them. Gaps are structured records with severity, suggested fixes, and resolution tracking.

Gap types include: missing outputs, failing checks, broken dependencies, unmet goals, stale inputs, and schema drift. When Converge detects a gap, it uses the goal's `plan.strategy` to determine how to generate corrective work — whether that's a single fix task, one task per failing item, or a full work breakdown.
