...
export const projectMdRules: ProjectMdRule[] = [
        ...
        return [
          issue(
            ...
            "`name` is required and must be a non-empty string",
            {
              path: filePath,
              field: "name",
              fix: "Add `name: My Project` to frontmatter",
            },
          ),
        ];
      }
          ...
          issues.push(
            issue(
              ...
              `Unknown discovery key "${key}"`,
              {
                path: filePath,
                field: `discovery.${key}`,
                expected: [...KNOWN_DISCOVERY_KEYS].join(", "),
              },
            ),
          );
        }
      ...
      if (d.watch !== undefined && typeof d.watch !== "boolean") {
        issues.push(
          issue(
            ...
            {
              path: filePath,
              field: "discovery.watch",
              actual: typeof d.watch,
              expected: "boolean",
            },
          ),
        );
      ...
      if (d.spawn !== undefined && !VALID_SPAWN_VALUES.has(d.spawn as string)) {
        issues.push(
          issue(
            ...
            {
              path: filePath,
              field: "discovery.spawn",
              actual: d.spawn,
              expected: "subtasks-only | unrestricted",
            },
          ),
        );
      }
        ...
        if (d[arrayKey] !== undefined && !Array.isArray(d[arrayKey])) {
          issues.push(
            issue(
              ...
              {
                path: filePath,
                field: `discovery.${arrayKey}`,
                actual: typeof d[arrayKey],
                expected: "array",
              },
            ),
          );
  ...
  /* ── runtime field types ───────────────────────────────────────── */
  {
    ...
    check: ({ frontmatter, filePath }) => {
          ...
          issues.push(
            issue(
              ...
              `Unknown runtime key "${key}"`,
              {
                path: filePath,
                field: `runtime.${key}`,
                expected: [...KNOWN_RUNTIME_KEYS].join(", "),
              },
            ),
          );
        }
      ...
      for (const numKey of ["maxStallCount", "maxParallelTasks"]) {
        if (
          r[numKey] !== undefined &&
          (typeof r[numKey] !== "number" || (r[numKey] as number) <= 0)
        ) {
          issues.push(
            issue(
              "project-runtime-shape",
        ...
        if (r[boolKey] !== undefined && typeof r[boolKey] !== "boolean") {
          issues.push(
            issue(
              ...
              {
                path: filePath,
                field: `runtime.${boolKey}`,
                actual: typeof r[boolKey],
                expected: "boolean",
              },
            ),
          );
      ...
      if (
        r.logLevel !== undefined &&
        !VALID_LOG_LEVELS.has(r.logLevel as string)
      ) {
        issues.push(
          issue(
            "project-runtime-shape",
      ...
      if (r.milestone !== undefined && typeof r.milestone !== "string") {
        issues.push(
          issue(
            ...
            {
              path: filePath,
              field: "runtime.milestone",
              actual: typeof r.milestone,
              expected: "string",
            },
          ),
        );
          ...
          issues.push(
            issue(
              ...
              `Unknown hook event "${event}"`,
              {
                path: filePath,
                field: `hooks.${event}`,
                expected: "Valid event (e.g., task:fail, project:complete)",
              },
            ),
          );
        }
  ...
  /* ── plugins must be array ─────────────────────────────────────── */
  {
    ...
    check: ({ frontmatter, filePath }) => {
      if (
        frontmatter.plugins !== undefined &&
        !Array.isArray(frontmatter.plugins)
      ) {
        return [
          issue(
            ...
            "`plugins` must be an array",
            {
              path: filePath,
              field: "plugins",
              actual: typeof frontmatter.plugins,
              expected: "array",
            },
          ),
        ];
  ...
  /* ── variables must be an object ────────────────────────────────── */
  {
    ...
    check: ({ frontmatter, filePath }) => {
      if (frontmatter.variables !== undefined) {
        if (
          typeof frontmatter.variables !== "object" ||
          frontmatter.variables === null ||
          Array.isArray(frontmatter.variables)
        ) {
          return [
            issue(
              ...
              "`variables` must be an object",
              {
                path: filePath,
                field: "variables",
                actual: typeof frontmatter.variables,
                expected: "object",
              },
            ),
          ];
  ...
  /* ── skill file paths must exist ─────────────────────────────── */
  {
    ...
    check: ({ frontmatter, filePath }) => {
      ...
      const issues: ValidationIssue[] = [];
      for (const [name, skillPath] of Object.entries(
          ...
          issues.push(
            issue(
              ...
              `Skill "${name}" points to "${skillPath}" which does not exist`,
              {
                ...
                field: `skills.${name}`,
                actual: skillPath,
                fix: `Create ${resolved} or fix the path`,
              },
            ),
          );
        }
  ...
  /* ── hook script paths must exist ────────────────────────────── */
  {
    ...
    check: ({ frontmatter, filePath }) => {
      ...
      const issues: ValidationIssue[] = [];
      for (const [event, scriptPath] of Object.entries(
        hooks as Record<string, unknown>,
      )) {
