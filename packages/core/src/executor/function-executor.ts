...
export class FunctionExecutor {
  ...
  private async executeAsSkill(
    ...
    try {
      ...
      // Simulation mode is gated behind an explicit env flag.
      //
      // Without the gate, this branch used to write placeholder files like
      // `# {title}\nThis is a placeholder output generated by skill task
      // simulation.\n` and then validateOutputs would see the files exist
      // and return success — meaning the framework could (and did) claim
      // a skill task succeeded while having produced nothing real. That
      // is a correctness hazard: downstream tasks would consume the
      // placeholders as if they were genuine inputs.
      //
      // Production deployments must use the real claudefn path (TODO
      // above). Dry-run / test contexts that genuinely want the simulator
      // can opt in by setting CONVERGE_ALLOW_SKILL_SIMULATION=1.
      const allowSimulation =
        process.env.CONVERGE_ALLOW_SKILL_SIMULATION === "1" ||
        process.env.CONVERGE_ALLOW_SKILL_SIMULATION === "true";
      if (!allowSimulation) {
        throw new Error(
          `Skill-task '${taskId}' would run via the placeholder simulator, ` +
            `which writes fake outputs and claims success. Wire the ` +
            `claudefn integration (function-executor.ts TODO above), or ` +
            `set CONVERGE_ALLOW_SKILL_SIMULATION=1 to opt into the ` +
            `simulator for dry-run/test contexts only.`,
        );
      }
      ctx.log.warn(
        `⚠️  CONVERGE_ALLOW_SKILL_SIMULATION=1 — skill task ${taskId} will ` +
          `produce placeholder outputs, NOT real artifacts. Disable in ` +
          `production.`,
      );
