...
const PREDICATES: Record<string, (s: Snapshot) => boolean> = {
  ...
  'hasBlocker':   s => s.gaps.some(g => g.metadata?.gapKind === GapKind.blocker),
  'isFirst':      s => s.iteration === 1,
  'hasSystemic':  s => s.gaps.some(g => g.metadata?.gapKind === GapKind.systemic),
  'hasWbsScript': s => s.gaps.some(g => g.metadata?.gapKind === GapKind.wbsScript),
  'hasUserQ':     s => s.gaps.some(g => g.metadata?.gapKind === GapKind.userQuestion),
  'hasOutput':    s => s.gaps.some(g => g.metadata?.gapKind === GapKind.output),
  'hasCheckFail': s => s.gaps.some(g => g.metadata?.gapKind === GapKind.checkFailed),
  'hasCorrupted': s => s.gaps.some(g => g.metadata?.gapKind === GapKind.corrupted),
  'hasRepairableGap': s => s.gaps.some(g => {
    const kind = g.metadata?.gapKind as string | undefined;
    return kind === GapKind.output || kind === GapKind.checkFailed || kind === GapKind.corrupted;
  }),

  // Compound predicates for navigator applicability
  'hasBlockerAndIsFirst':    s => s.gaps.some(g => g.metadata?.gapKind === GapKind.blocker) && s.iteration === 1,
  'hasBlockerAndNotFirst':   s => s.gaps.some(g => g.metadata?.gapKind === GapKind.blocker) && s.iteration > 1,
  ...
  'hasSkill':      s => {
    ...
    return Array.isArray(skill) ? skill.length > 0 : typeof skill === 'string' && skill.length > 0;
  },
  ...
  'gap-is-blocker':      s => s.gaps.some(g => g.metadata?.gapKind === 'blocker'),
  'gap-is-check-failed': s => s.gaps.some(g => g.metadata?.gapKind === 'check-failed'),
  'gap-is-output':       s => s.gaps.some(g => g.metadata?.gapKind === 'output'),
  'backoff-scheduled':   s => typeof s.lastActionRetryMode === 'object'
