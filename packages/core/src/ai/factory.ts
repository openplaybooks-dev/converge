...
import { agentfn, type AgentFnOptions, type AgentFnResult } from '@converge/agentfn';
import type { AIConfig, AIProviderConfig, AIMultiProviderConfig } from '../storage/types.ts';
 ...
 */
function isMultiProviderConfig(config: AIConfig): config is AIMultiProviderConfig {
...
export function resolveAIConfig(
  ...
  if (isMultiProviderConfig(aiConfig)) {
    ...
    if (!providerName) {
      console.warn('   ⚠️  No default provider configured in multi-provider AI config');
      return null;
    }

    const providerConfig = multiConfig.providers[providerName];
    if (!providerConfig) {
      console.warn(`   ⚠️  Provider '${providerName}' not found in AI configuration`);
      console.warn(`      Available providers: ${Object.keys(multiConfig.providers).join(', ')}`);
      return null;
    }

    // The provider name IS the type (e.g., 'acp', 'kimi', 'claude')
    // Use explicit provider field if set, otherwise use the key name
    const resolvedProvider = (providerConfig.provider || providerName) as AIProvider;
