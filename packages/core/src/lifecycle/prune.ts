...
import { getTaskCorrectionsDir, getJournalStructure } from '../journal/structure.ts';
...
export async function pruneTaskJournal(
  ...
  summary.correctionsCompressed = await compressOldCorrections(correctionsDir, policy.maxCorrectionHistory);

  // 3. Write PRUNE_MARKER to events.jsonl so readers know where this run starts
  await logTaskEvent(projectDir, epicId, taskId, 'PRUNE_MARKER', `New execution started: ${executionId}`, {
    executionId,
...
async function compressOldCorrections(correctionsDir: string, maxHistory: number): Promise<number> {
  ...
  for (const file of toCompress) {
    try {
      ...
      const date = typeof record.timestamp === 'string'
        ? record.timestamp.slice(0, 10)
        : '????-??-??';
      const fixDesc = typeof record.fixApplied === 'string'
        ? record.fixApplied.slice(0, 80)
        : '(unknown fix)';
      const outcome = record.resolved === true ? 'SUCCESS' : 'FAILED';
      const cause = typeof (record.diagnosis as Record<string, unknown>)?.likelyCause === 'string'
  ...
  if (rows.length > 0) {
    ...
    let existing = '';
    try { existing = await readFile(historyPath, 'utf8'); } catch { /* new file */ }
    ...
    await writeFile(historyPath, header + existing.replace(/^#.*\n\n\|.*\|.*\n\|.*\|\n/, '') + rows.join('\n') + '\n');
  }
