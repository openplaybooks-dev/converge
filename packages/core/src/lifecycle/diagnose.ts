...
export async function diagnoseFailure(
  ...
  const ruleBased = tryRuleBasedDiagnosis(combinedOutput, failedChecks, diagnosisHints);
  ...
  await logTaskEvent(projectDir, epicId, taskId, 'CORRECTION_DIAGNOSIS',
...
function heuristicDiagnosis(
  ...
  if (/timeout|timed out|ETIMEDOUT/i.test(combinedOutput)) {
    return {
      ...
      suggestedFix: 'Retry — the task may need a longer timeout or should be decomposed into smaller steps',
      automatable: true,
      confidenceScore: 0.85,
    };
  }
  // Missing bash / shell
  if (/spawn.*bash.*ENOENT|Command not found|No such file or directory.*bash/i.test(combinedOutput)) {
    return {
      errorClass: 'tool-error',
      likelyCause: 'Bash binary not found on PATH — check commands cannot run',
      suggestedFix: 'Add bash to PATH (e.g. Git for Windows) or rewrite checks as PowerShell',
      automatable: false,
      confidenceScore: 0.9,
    };
  }
  // All checks fail with empty output → likely nothing was produced
  const nothingCreated = failedChecks.every(c => /test -f|test -s/.test(c.cmd) && c.stdout === '');
  ...
  return {
    ...
    suggestedFix: 'Re-run the task. If it fails again, review agent logs for clues.',
    automatable: true,
    confidenceScore: 0.4,
  };
}

function buildCheckFailures(failedChecks: CheckRunResult[]): CheckFailureDetail[] {
