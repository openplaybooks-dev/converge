...
function tryQuoteSpecialPaths(cmd: string): string | null {
  ...
  const pathWithExtPattern = /(?<!["'])((?:src|\.stitch|\.converge|node_modules)\/\S*?\.(?:tsx?|jsx?|css|json|html?|md|ya?ml))(?!["'])/g;
  const dirPathPattern = /(?<!["'])((?:src|\.stitch|\.converge|node_modules)\/[^\s"'|><;)]+?(?:\/_components|\/\*)?)(?=\s|$)/g;
