#!/usr/bin/env node
/**
 * jq shim — portable subset of jq for converge check commands.
 *
...
function accessPath(data: JqValue, path: string): JqValue {
  ...
  if (bracketIdx !== -1 && (dotIdx === -1 || bracketIdx < dotIdx)) {
    ...
    if (Array.isArray(current)) {
      ...
      current = isNaN(idx)
        ? null
        : (current[idx < 0 ? current.length + idx : idx] ?? null);
...
function applyFilter(data: JqValue, f: string): JqValue {
  ...
  if (containsMatch) {
    ...
    if (typeof data === "string" && typeof val === "string")
      return data.includes(val);
    if (Array.isArray(data))
      return Array.isArray(val)
        ? val.every((v) => data.includes(v))
        : data.includes(val);
    return false;
  }
  ...
  if (mapMatch) {
    ...
    return data
      .map((item) => applyFilter(item, mapMatch[1]))
...
async function main() {
  ...
  const outputs: JqValue[] =
    Array.isArray(result) && filter!.includes(".[]")
      ? (result as JqValue[])
      : [result];
