...
function buildExecutablePrompt(args: ImplementExecutableArgs): string {
  ...
  lines.push(
    ...
    "contract (goal, scope, outputs, checks, body) verbatim:",
    "",
    planMd.trim(),
    "",
    "## Your task",
    ...
    "verbatim from PLAN.md's per-child `body` field. Reference shape:",
    "`examples/cinematic-video-production/.converge/playbooks/default/",
    "tasks/02-cast/001-extract/TASK.md`. If PLAN.md omitted the schema",
    ...
    "",
    "**`description` vs `goal`**: `goal` is one sentence (\"what to",
