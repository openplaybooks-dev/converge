...
export class AutonomousOrchestrator {
  ...
  private async autoDiscoverAll(): Promise<void> {
    ...
    // Scan tasks from .converge/tasks/ (global tasks)
    ...
    try {
      ...
      if (attempt.result.success) {
        ...
        if (verificationResult && verificationResult.issues.length > 0) {
          verificationResult.issues.forEach((issue: any) => {
  ...
  /**
   * Validate task output with AutoConverge
   */
  private async validateTaskOutput(
    ...
    const verification = await synthesizer.synthesize({
      taskContext: {
        id: taskMeta.task.id,
