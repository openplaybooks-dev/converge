 ...
 *
 * Usage:
 *   pnpm converge inspect-session [sessionId]
 *   pnpm converge inspect-session --latest
 *   pnpm converge inspect-session --list
 *   pnpm converge inspect-session <id> --gaps
 *   pnpm converge inspect-session <id> --strategies
 *   pnpm converge inspect-session <id> --tasks
 *   pnpm converge inspect-session <id> --timeline
 */
...
async function listSessions(projectDir: string): Promise<void> {
  ...
  if (!existsSync(sessionsDir)) {
    console.log(
      "No sessions found. Run `pnpm converge run` to create a session.",
    );
    return;
  }
  ...
  for (const session of sessions.sort((a, b) => b.name.localeCompare(a.name))) {
    ...
    if (existsSync(metadataPath)) {
      ...
      if (metadata.outcomes) {
        console.log(
          `   Tasks: ${metadata.outcomes.tasksCompleted}/${metadata.outcomes.tasksCompleted + metadata.outcomes.tasksFailed} | Gaps: ${metadata.outcomes.gapsResolved}`,
        );
      }
      console.log("");
    }
...
async function showSessionSummary(
  ...
  console.log(
    `  Max Attempts Per Task: ${metadata.config.maxAttemptsPerTask || 2}`,
  );
  ...
  if (metadata.outcomes) {
    ...
    console.log(
      `  Convergence: ${metadata.outcomes.convergenceAchieved ? "Yes ✅" : "No ❌"}`,
    );
  }
...
async function showGaps(projectDir: string, sessionId: string): Promise<void> {
  ...
  console.log(
    `║  Gaps Detected (${String(gaps.length).padEnd(3)})                                       ║`,
  );
  console.log(
    `╚════════════════════════════════════════════════════════════╝\n`,
  );
...
async function showStrategies(
  ...
  console.log(
    `║  Strategies Attempted (${String(strategies.length).padEnd(3)})                            ║`,
  );
  console.log(
    `╚════════════════════════════════════════════════════════════╝\n`,
  );
  ...
  for (const [name, count] of Array.from(strategyCount.entries()).sort(
    (a, b) => b[1] - a[1],
  )) {
...
async function showTasks(projectDir: string, sessionId: string): Promise<void> {
  ...
  const taskEvents = lines
    .map((line) => JSON.parse(line) as SessionEvent)
    .filter((e) =>
      ["TASK_SELECTED", "TASK_ATTEMPT_COMPLETE"].includes(e.eventType),
    );
  ...
  for (const event of taskEvents) {
    if (event.eventType === "TASK_SELECTED") {
      ...
      const durationStr = duration
        ...
        : "unknown";
      console.log(
        `   ${icon} ${success ? "Completed" : "Failed"} in ${durationStr}`,
      );
      currentTask = null;
      taskStartTime = null;
    }
