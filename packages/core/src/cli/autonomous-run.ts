...
export async function guardDirtySession(projectDir: string, resume?: boolean, restart?: boolean): Promise<void> {
  ...
  console.error(`   converge run --resume    # recover and continue from where it stopped`);
  console.error(`   converge run --restart   # reset non-complete tasks and start fresh\n`);
  process.exit(1);
}
  ...
  try {
    ...
    if (execResult.success) {
      ...
      if (attempts >= maxTaskAttempts) {
        ...
        console.log(`   ↳  Retry manually with: converge reset ${node.taskId}`);
        consecutiveFailures++;
        tasksFailed++;
        await sessionLogger.logConvergence(node.journalTaskId, false);
