...
export function renderSingleSessionTimeline(
  ...
  console.log(`   converge inspect --session=${session.sessionId} --dirs          Show directory structure`);
  console.log(`   converge inspect --session=${session.sessionId} --toolDetails   Show detailed tool calls`);
  console.log(`   converge inspect --session=${session.sessionId} --ai            Show AI activity`);
  console.log('');
}
...
export function renderMultiSessionTimeline(
  ...
  console.log('');
  console.log(color('📁 Session directories:', COLORS.GRAY) + ' .converge/journal/sessions/');
  console.log('');
  console.log(color('Usage:', COLORS.BOLD));
  console.log('  converge inspect --session=<id>     View session details');
  console.log('  converge inspect --dirs              Show directory structure');
...
export async function renderAggregatedInspection(
  ...
  console.log(`   converge inspect --session=<id>    Deep dive into specific session`);
  console.log(`   converge tree                       View task dependency tree`);
  console.log(`   converge gantt                      Visualize execution order`);
  console.log('');
}
...
function renderTaskWorkflow(
  ...
  if (issueData?.history) {
    ...
    if (totalAttempts > 0 || Object.keys(strategySummary).length > 0 || stratAttempts.length > 0) {
      ...
      console.log(`         ${color('(ConvergeController loop retried without invoking GapResolutionPipeline)', COLORS.GRAY)}`);
    }
  }
