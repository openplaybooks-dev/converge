...
context:
  ...
  - type: ai
    ...
    timeoutMs: 45000
  - type: cmd
    cmd: "ls .converge/epics/{epicId}/tasks/ 2>/dev/null | tail -5"
...
2. Create the directory: `.converge/epics/{epicId}/tasks/{taskId}/`
...
node -e "require('yaml').parse(require('fs').readFileSync('.converge/epics/{epicId}/tasks/{taskId}/SKILL.md','utf8').match(/^---\n([\s\S]*?)\n---/)[1])"
```
