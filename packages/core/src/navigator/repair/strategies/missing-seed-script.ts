/**
 * MissingSeedScriptStrategy
 *
 * Handles gaps where a Seed script (seed.js) is missing.
 * This typically happens when a task was seeded from a template but the seed.js
 * wasn't copied (bug in the seeder).
 *
 * Gap kind: 'seed-script-error' with missing script
 */
...
export class MissingSeedScriptStrategy implements FixStrategy {
  readonly name = "missing-seed-script";

  canHandle(gap: Gap): boolean {
    // Handle seed-script-error gaps where the script is actually missing
    if (gap.metadata?.gapKind !== "seed-script-error") {
      return false;
    }
    const errorMessage = (gap.metadata?.errorMessage as string) ?? "";
    return (
      errorMessage.includes("Seed script not found") ||
      errorMessage.includes("seed.js") && errorMessage.includes("not found")
    );
  }
  ...
  async tryFix(gap: Gap, ctx: StrategyContext): Promise<StrategyOutcome> {
    ...
    // Check for TASK.md to understand what Seed is needed
    ...
    console.log(`   [missing-seed-script] Analyzing TASK.md: ${taskMdPath}`);
    ...
    // Check if TASK.md has a seed section
    if (!taskMdContent.includes("seed:")) {
      return {
        success: false,
        reason: "TASK.md has no seed section - no script needed",
      };
    }

    // Try to find a template that matches this task's pattern
    // Look for seed.js in parent template directories
    const templateWbsPath = await this.findTemplateWbsJs(taskDir, projectDir);

    if (templateWbsPath) {
      console.log(`   [missing-seed-script] Found template: ${templateWbsPath}`);
      
      try {
        const templateContent = await readFile(templateWbsPath, "utf-8");
        const destPath = join(taskDir, "seed.js");
        ...
        console.log(`   [missing-seed-script] Created: ${destPath}`);
        return {
          success: true,
          reason: "Copied seed.js from template",
          retryMode: "full",
        };
      } catch (err: any) {
        return {
    ...
    // No template found - generate a basic seed.js based on TASK.md content
    console.log(`   [missing-seed-script] No template found, generating basic seed.js`);
    
    const generatedSeed = this.generateBasicSeedJs(taskMdContent);
    const destPath = join(taskDir, "seed.js");
    
    try {
      await writeFile(destPath, generatedSeed, "utf-8");
      console.log(`   [missing-seed-script] Created basic seed.js: ${destPath}`);
      return {
        success: true,
        reason: "Generated basic seed.js from TASK.md",
        retryMode: "full",
      };
    } catch (err: any) {
      return {
        success: false,
        reason: `Failed to write seed.js: ${err.message}`,
      };
    }
  }

  /**
   * Try to find a matching template seed.js by looking at:
  ...
  private async findTemplateWbsJs(
    ...
    if (tasksMatch) {
      // Look for seed/templates in the grandparent directory
      const grandparentDir = taskDir.split(/tasks[\/][^\/]+[\/]tasks/)[0];
      const templatePatterns = [
        join(grandparentDir, "seed", "templates", "screen", "tasks", "{{prefix}}-05-split", "seed.js"),
        join(grandparentDir, "seed", "templates", "screen", "tasks", "{{prefix}}-06-lift", "seed.js"),
        join(grandparentDir, "seed", "templates", "screen", "tasks", "{{prefix}}-01-spec", "seed.js"),
        join(grandparentDir, "seed", "templates", "screen", "tasks", "{{prefix}}-02-design", "seed.js"),
        join(grandparentDir, "seed", "templates", "screen", "tasks", "{{prefix}}-03-convert", "seed.js"),
        join(grandparentDir, "seed", "templates", "screen", "tasks", "{{prefix}}-04-analyze", "seed.js"),
      ];
    ...
    // Look for sibling tasks that have seed.js (pattern matching)
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const siblingWbsPath = join(siblingDir, "seed.js");
  ...
  /**
   * Check if two TASK.md files have similar structure (same seed type, similar vars)
   */
  private async hasSimilarStructure(
    ...
    try {
      ...
      // Compare seed type
      const wbsMatch1 = content1.match(/seed:\s*\n\s*type:\s*(\w+)/);
      const wbsMatch2 = content2.match(/seed:\s*\n\s*type:\s*(\w+)/);
  ...
  /**
   * Generate a basic seed.js based on TASK.md content
   */
  private generateBasicSeedJs(taskMdContent: string): string {
    ...
    if ((isSplit || isLift) && hasWidgetsJson) {
      return `// Auto-generated Seed script for ${isSplit ? "split" : "lift"} task
...
export async function run(ctx) {
  ...
  for (let i = 0; i < widgets.length; i++) {
    ...
    await ctx.spawn({
      ...
      template: '.converge/playbooks/default/tasks/03-build-screens/seed/templates/screen/tasks/{{prefix}}-${isSplit ? '05-split' : '06-lift'}/tasks/subtask/TASK.md',
      vars: {
        ...ctx.vars,
...
`;
    ...
    // Default basic Seed script
    return `// Auto-generated Seed script
export async function run(ctx) {
  ctx.log.info('Seed script running for task: ' + ctx.vars?.title);
  
  // This is a placeholder - the task should have specific Seed logic
