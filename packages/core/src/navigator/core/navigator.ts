  ...
  while (actionCount < maxActions) {
    ...
    if (!next) {
      ...
      if (!next) {
        ...
        // Only require execution if we have 0 gaps but also have no way to verify outputs exist
        // (i.e., task has outputs defined but we never ran anything to create them)
        const hasOutputsDefined = (unit.outputs?.length ?? 0) > 0;
        const neverExecuted = executionCount === 0 && cycle === 1;

        // If outputs are defined and we never executed, we need to verify they actually exist
        // by checking if detect-gaps would have found output gaps
        const needsVerification = hasOutputsDefined && neverExecuted;
