...
```
000-setup-skills   → install converge-planning skill to .converge/skills
...
005-emit           → write playbook files to .converge/playbooks/<name>/
```
...
converge plan --prompt "Add dark mode" --name dark-mode
...
converge run --playbook=dashboard          # start building
converge plan --prompt "Add export feature" --name dashboard --update
converge run --playbook=dashboard          # resumes with new tasks
```
