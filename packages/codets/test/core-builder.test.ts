...
describe("CoreBuilder", () => {
  describe("line emission", () => {
    ...
    it("emits an empty line when text is empty string", () => {
      expect(out(new CoreBuilder().line("a").line("").line("b"))).toBe(
        "a\n\nb",
      );
    });
  ...
  describe("section markers", () => {
    ...
    it("adjusts padding for long names", () => {
      const b = new CoreBuilder().section(
        "A very long section name that exceeds padding",
      );
      expect(out(b)).toContain(
        "// ── A very long section name that exceeds padding",
      );
    });
  });
  ...
  describe("control flow", () => {
    ...
    it("each() iterates items", () => {
      const b = new CoreBuilder().each(["a", "b", "c"], (b, item) =>
        b.line(item),
      );
      expect(out(b)).toBe("a\nb\nc");
    });
    ...
    it("each() provides index", () => {
      const b = new CoreBuilder().each(["x", "y"], (b, item, i) =>
        b.line(`${i}:${item}`),
      );
