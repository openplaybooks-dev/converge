...
import {
  ...
  ArrayBuilder,
  FnBody,
} from "../src/semantic/sub-builders.ts";
...
describe("ExprHelper", () => {
  ...
  describe("raw", () => {
    it("creates raw expression", () => {
      expect(e.raw("process.env.X")).toEqual({
        kind: "raw",
        code: "process.env.X",
      });
    });
...
describe("Formatter: Expressions", () => {
  ...
  function render(expr: ExprNode): string {
    return formatFile(
      [{ kind: "const", name: "x", value: expr, exported: false }],
      opts,
    )
      .replace("const x = ", "")
      .replace(/;\n$/, "");
        ...
        [
          {
            kind: "const",
            name: "x",
            value: { kind: "string", value: 'say "hi"' },
            exported: false,
          },
        ],
        dqOpts,
  ...
  describe("raw expression", () => {
    it("renders raw code as-is", () => {
      expect(render({ kind: "raw", code: "process.env.NODE_ENV" })).toBe(
    ...
    s.const("nextConfig")
      ...
      .value((v) =>
        v.object((o) => {
          ...
          o.when(needsImageConfig, (o) =>
            o.propObj("images", (img) => {
              img.when(patch.staticExport, (img) =>
                img.prop("unoptimized", img.bool(true)),
              );
...
describe("File formatting", () => {
  ...
  it("banner before directives", () => {
    ...
    const output = s.toString();
    expect(output.indexOf("// Auto-generated")).toBeLessThan(
      output.indexOf("'use client'"),
    );
  });
});
