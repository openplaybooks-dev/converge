...
describe("ProjectBuilder", () => {
  ...
  describe("flush()", () => {
    ...
    beforeEach(() => {
      tmpDir = join(
        tmpdir(),
        `codegen-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      );
    ...
    it("writes files to disk", () => {
      ...
      expect(readFileSync(join(tmpDir, "src", "hello.ts"), "utf-8")).toBe(
        'console.log("hello");\n',
      );
      expect(readFileSync(join(tmpDir, "config.json"), "utf-8")).toBe(
        '{"key": "value"}',
      );
    });
    ...
    it("supports diffOnly mode", () => {
      ...
      expect(readFileSync(join(tmpDir, "diff.txt"), "utf-8")).toBe(
        "new-content",
      );
    });
