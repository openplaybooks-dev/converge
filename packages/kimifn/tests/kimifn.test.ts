...
import { kimifn } from "../src/index.js";
import { z } from "zod";
...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  let _nextStdout = "Hello from Kimi";
  let _nextStderr = "";
  let _nextExitCode = 0;
  let _nextDelay = 0;
  let _spawnArgs: any[] = [];
  ...
  return {
    ...
    __setNext: (opts: {
      ...
      _nextStdout = opts.stdout ?? "Hello from Kimi";
...
describe("kimifn — core API", () => {
  it("returns a callable function", () => {
    const fn = kimifn({ prompt: "Say hello" });
    expect(typeof fn).toBe("function");
  });

  it("invokes kimi CLI with -y --print and -p flag", async () => {
    const fn = kimifn({ prompt: "Say hello" });
    await fn();
  ...
  it("interpolates {{input}} in prompt template", async () => {
    const fn = kimifn({ prompt: "Translate {{input}} to French" });
    await fn("hello");
  ...
  it("works without {{input}} placeholder", async () => {
    const fn = kimifn({ prompt: "What is 2+2?" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    const pIdx = args.indexOf("-p");
  ...
  it("passes cwd to spawn options", async () => {
    const fn = kimifn({ prompt: "test", cwd: "/tmp/myproject" });
    await fn();
...
describe("kimifn — dynamic prompt", () => {
  it("accepts a function that returns a prompt string", async () => {
    const fn = kimifn({ prompt: () => "dynamic hello" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    const pIdx = args.indexOf("-p");
  ...
  it("prompt function receives undefined when no input given", async () => {
    ...
    const fn = kimifn({ prompt: promptSpy });
    await fn();
...
describe("kimifn — schema validation", () => {
...
describe("kimifn — error handling", () => {
  ...
  it("times out and kills the process", async () => {
    ...
    const fn = kimifn({ prompt: "test", timeoutMs: 50 });
  ...
  it("retries on failure up to maxRetries", async () => {
    ...
    const fn = kimifn({ prompt: "test", maxRetries: 3 });
    const result = await fn();
  ...
  it("throws after exhausting all retries", async () => {
    ...
    const fn = kimifn({ prompt: "test", maxRetries: 2 });
    await expect(fn()).rejects.toThrow();
  });
});
...
describe("kimifn — promptless usage", () => {
