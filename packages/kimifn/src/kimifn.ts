...
/**
 * Create a callable function backed by the Kimi CLI.
 *
 * Spawns `kimi -y --print -p "..."` as a subprocess.
 *
...
export async function executeViaCli<T>(
  ...
  schema: KimiFnOptions<T>["schema"],
  hooks: KimiFnOptions<T>["hooks"],
  timeoutMs: number,
  cliFlags: string[],
  ...
  if (promptTemplate) {
    ...
    throw new Error(
      'kimifn requires either a "prompt" option or an input argument',
    );
  }
