 ...
 * (or self) of `cwd` containing .converge/. Honors KIMI_SKILLS_PATH override.
 */
function getSkillsDir(cwd?: string): string | null {
  if (process.env.KIMI_SKILLS_PATH) return process.env.KIMI_SKILLS_PATH;
...
/**
 * Resolve <projectRoot>/agents/. Honors KIMI_AGENTS_PATH override.
 */
function getAgentsDir(cwd?: string): string | null {
  if (process.env.KIMI_AGENTS_PATH) return process.env.KIMI_AGENTS_PATH;
