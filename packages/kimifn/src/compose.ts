...
/** Extract a JS/TS code block from Kimi's response */
...
/** Parse `<tool_call>` blocks from Kimi's response */
...
/** Build the code-mode preamble that tells Kimi to write executable JS */
...
/** Spawn the Kimi CLI and collect output */
function spawnCli(
  prompt: string,
  cliFlags: string[],
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    proc.on("close", (code: number | null) => {
      ...
      if (code !== 0) {
        reject(
          new Error(stderr.trim() || `kimi exited with code ${code}`),
        );
      } else {
        resolve(stdout);
      }
...
export async function executeCode(
  code: string,
  tools: Record<string, KimiFn<any>>,
): Promise<unknown> {
...
/**
 * Create a composed function that can use other kimifn functions as tools.
 *
 * Two compose modes:
 * - `"code"` (default): Kimi writes a JS function body; the system executes
 *   it with tools injected as named async functions.
 * - `"tool_call"`: Kimi uses `<tool_call>` XML blocks; the system parses
 *   them, executes tools, and feeds results back in a loop.
 */
export function compose<T = string>(
  options: ComposeOptions<T>,
): (input?: string) => Promise<KimiFnResult<T>> {
  const {
  ...
  return async (input?: string): Promise<KimiFnResult<T>> => {
    let attempt = 0;
...
async function executeComposeCode<T>(
  ...
  const wrappedTools: Record<string, KimiFn<any>> = {};
