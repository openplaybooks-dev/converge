...
const MIGRATION_ROWS = [
  ...
  { oldCommand: ['verify', '--fix'],   hint: /use:?\s+converge\s+debug\s+--fix/i },
  // spec §10 — v1 status → v2 list / show graph
  { oldCommand: ['status'],            hint: /use:?\s+converge\s+(list|show graph)/i },
  { oldCommand: ['status', '--only-incomplete'], hint: /use:?\s+converge\s+list/i },
  // spec §10 — v1 reset → v2 clean
  { oldCommand: ['reset', 'pb', 'task'], hint: /use:?\s+converge\s+clean/i },
  // spec §10 — v1 playbook → v2 list/inspect/show
  { oldCommand: ['playbook', 'list'],  hint: /use:?\s+converge\s+list\s+--playbooks/i },
  { oldCommand: ['playbook', 'info', 'X'], hint: /use:?\s+converge\s+inspect\s+playbook/i },
  { oldCommand: ['playbook', 'history', 'X'], hint: /use:?\s+converge\s+show\s+trend/i },
  ...
  { oldCommand: ['plan', 'goal text'], hint: /use:?\s+converge\s+init\s+--from-prompt/i },
  // spec §10 — v1 checkpoint → v2 inspect checkpoint
  { oldCommand: ['checkpoint'],        hint: /use:?\s+converge\s+inspect\s+checkpoint/i },
  // spec §10 — v1 plugins → v2 deps list
  { oldCommand: ['plugins'],           hint: /use:?\s+converge\s+deps\s+list/i },
];

describe.each(MIGRATION_ROWS)('redirect for $oldCommand', ({ oldCommand, hint }) => {
  beforeAll(() => {
    if (!existsSync(CLI)) {
      throw new Error(
