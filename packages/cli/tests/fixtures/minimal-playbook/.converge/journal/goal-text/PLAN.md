...
This playbook's name, description, and the user intent (`-p "goal text"`) are all the literal string `"goal text"`. The scope packet carries only run configuration (autonomous mode, resume enabled, max 3 task attempts) with zero artifact outputs, zero file contracts, and no behavioral specification. Its location under `tests/fixtures/minimal-playbook` suggests it validates that the planner and runner handle an intentionally empty autonomous playbook gracefully.
