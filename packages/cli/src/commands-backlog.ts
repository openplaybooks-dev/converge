...
import type { BacklogItem } from "@converge/core/backlog/types.ts";
...
export async function backlogCommand(
  ...
  for (const file of files.sort()) {
    ...
    const items: BacklogItem[] = content
      ...
      .map((line) => {
        try {
          return JSON.parse(line) as BacklogItem;
        } catch {
          return null;
        }
      })
    ...
    const minSeverity =
      options.severity && severityOrder.includes(options.severity)
        ? severityOrder.indexOf(options.severity)
        : 0;
    const filtered = items.filter(
      (item) => severityOrder.indexOf(item.severity) >= minSeverity,
    );
    ...
    for (const [categoryId, items] of byCategory) {
      for (const item of items) {
        ...
        const location = item.file
          ...
          : "";
        const preview =
          item.raw.length > 60 ? item.raw.slice(0, 57) + "..." : item.raw;
        console.log(
          `    ${icon}  ${COLORS.DIM}${categoryId.padEnd(18)}${COLORS.RESET} ${location}  ${COLORS.GRAY}${location ? preview.replace(item.file + (item.line ? `:${item.line}:` : ""), "").trim() : preview}${COLORS.RESET}`,
        );
      }
    }
  }
  ...
  // Summary
  const highCount = groups.reduce(
    (s, g) => s + g.items.filter((i) => i.severity === "high").length,
    0,
  );
  const mediumCount = groups.reduce(
    (s, g) => s + g.items.filter((i) => i.severity === "medium").length,
    0,
  );
  const lowCount = groups.reduce(
    (s, g) => s + g.items.filter((i) => i.severity === "low").length,
    0,
  );

  console.log(`\n${"─".repeat(50)}`);
  console.log(
    `  ${totalItems} items: ${highCount} high, ${mediumCount} medium, ${lowCount} low\n`,
  );
}
