...
import { codexfn } from "../src/index.js";
import { z } from "zod";
...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  let _nextStdout = "Hello from Codex";
  let _nextStderr = "";
  let _nextExitCode = 0;
  let _nextDelay = 0;
  let _spawnArgs: any[] = [];
  ...
  return {
    ...
    __setNext: (opts: {
      ...
      _nextStdout = opts.stdout ?? "Hello from Codex";
...
describe("codexfn — core API", () => {
  it("returns a callable function", () => {
    const fn = codexfn({ prompt: "Say hello" });
    expect(typeof fn).toBe("function");
  });

  it("invokes codex CLI with exec subcommand and sandbox flags", async () => {
    const fn = codexfn({ prompt: "Say hello" });
    await fn();
  ...
  it("passes prompt as the last positional argument", async () => {
    const fn = codexfn({ prompt: "Say hello" });
    await fn();
  ...
  it("interpolates {{input}} in prompt template", async () => {
    const fn = codexfn({ prompt: "Translate {{input}} to French" });
    await fn("hello");
    const [, args] = mockCp.__getSpawnArgs();
    expect(args[args.length - 1]).toBe("Translate hello to French");
  });

  it("works without {{input}} placeholder", async () => {
    const fn = codexfn({ prompt: "What is 2+2?" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
  ...
  it("passes cwd to spawn options", async () => {
    const fn = codexfn({ prompt: "test", cwd: "/tmp/myproject" });
    await fn();
  ...
  it("passes --model flag when model option is set", async () => {
    const fn = codexfn({ prompt: "test", model: "gpt-5.4" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
  ...
  it("allows overriding sandbox level", async () => {
    const fn = codexfn({ prompt: "test", sandbox: "workspace-write" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
  ...
  it("omits --skip-git-repo-check when false", async () => {
    const fn = codexfn({ prompt: "test", skipGitRepoCheck: false });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    expect(args).not.toContain("--skip-git-repo-check");
  });

  it("omits --ephemeral when false", async () => {
    const fn = codexfn({ prompt: "test", ephemeral: false });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
...
describe("codexfn — dynamic prompt", () => {
  it("accepts a function that returns a prompt string", async () => {
    const fn = codexfn({ prompt: () => "dynamic hello" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    expect(args[args.length - 1]).toBe("dynamic hello");
  });
  ...
  it("dynamic prompt works with before hook", async () => {
    ...
    await fn("data");
    const [, args] = mockCp.__getSpawnArgs();
    expect(args[args.length - 1]).toBe("HOOKED(base: data)");
  });
  ...
  it("dynamic prompt can use closures for context", async () => {
    ...
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    expect(args[args.length - 1]).toBe("request #2");
...
describe("codexfn — schema validation", () => {
...
describe("codexfn — hooks", () => {
  it("calls before hook and allows prompt modification", async () => {
    ...
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    expect(args[args.length - 1]).toBe("MODIFIED: original prompt");
  });
  ...
  it("before hook returning void keeps original prompt", async () => {
    ...
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    expect(args[args.length - 1]).toBe("keep me");
  });
});

// -- Error Handling --

describe("codexfn — error handling", () => {
  ...
  it("times out and kills the process", async () => {
    ...
    const fn = codexfn({ prompt: "test", timeoutMs: 50 });
  ...
  it("retries on failure up to maxRetries", async () => {
    ...
    const fn = codexfn({ prompt: "test", maxRetries: 3 });
    const result = await fn();
  ...
  it("throws after exhausting all retries", async () => {
    ...
    const fn = codexfn({ prompt: "test", maxRetries: 2 });
    await expect(fn()).rejects.toThrow();
  });
});
...
describe("codexfn — promptless usage", () => {
