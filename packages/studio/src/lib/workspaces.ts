...
export function listWorkspaces(): Workspace[] {
  ...
  return [...ws].sort((a, b) => (b.lastOpenedAt || '').localeCompare(a.lastOpenedAt || ''));
}
...
export function addWorkspace(input: Omit<Workspace, 'id' | 'lastOpenedAt'> & { id?: string }): Workspace {
  const cfg = loadConfig();
...
export function removeWorkspace(id: string): void {
  ...
  const currentWorkspaceId = cfg.currentWorkspaceId === id ? null : cfg.currentWorkspaceId;
...
export function updateWorkspaceMeta(id: string, patch: Partial<Workspace>): void {
  const cfg = loadConfig();
  const workspaces = (cfg.workspaces ?? []).map(w => (w.id === id ? { ...w, ...patch } : w));
  saveConfig({ ...cfg, workspaces });
}
