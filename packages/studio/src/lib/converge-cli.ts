...
let _resolved: { command: string; prefixArgs: string[]; shell: boolean } | null = null;
 ...
 */
export function getConvergeCli(): { command: string; prefixArgs: string[]; shell: boolean } {
  if (_resolved) return _resolved;

  const override = process.env.CONVERGE_CLI?.trim();
  if (override) {
    if (override.endsWith('.js') || override.endsWith('.mjs') || override.endsWith('.cjs')) {
      _resolved = { command: process.execPath, prefixArgs: [override], shell: false };
    } else {
  ...
  if (local) {
    _resolved = { command: process.execPath, prefixArgs: [local], shell: false };
    return _resolved;
  }
