...
interface NavItem {
  ...
  href?: string // Direct link route (for converge shell)
}
...
const navGroups: NavGroup[] = [
  {
    id: 'core',
    items: [
      { id: 'playbooks', label: 'Playbooks', icon: <PlaybooksIcon />, priority: true, href: '/playbooks' },
      { id: 'runs', label: 'Runs', icon: <RunsIcon />, priority: true, href: '/runs' },
      { id: 'settings', label: 'Settings', icon: <SettingsIcon />, priority: false, href: '/settings' },
    ],
  },
]
  ...
  if (item.href) {
    if (expanded) {
      return (
        <a
          href={item.href}
          className={`w-full flex items-center gap-2 px-2 h-auto rounded-lg text-left justify-start relative no-underline ${
    ...
    return (
      <a
        href={item.href}
        title={item.label}
        className={`rounded-lg group relative no-underline ${
          active ? 'bg-primary/15 text-primary hover:bg-primary/20' : ''
        }`}
      >
