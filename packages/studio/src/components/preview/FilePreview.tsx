...
function isMarkdown(contentType: string, name: string): boolean {
  return contentType === 'text/markdown' || ['md', 'markdown'].includes(ext(name));
}
...
function JsonTree({ data, depth = 0 }: { data: any; depth?: number }): React.ReactNode {
  if (data === null) return <span style={{ color: 'var(--cv-text-muted)' }}>null</span>;
  if (typeof data === 'boolean') return <span style={{ color: '#C678DD' }}>{String(data)}</span>;
  if (typeof data === 'number') return <span style={{ color: '#98C379' }}>{String(data)}</span>;
  if (typeof data === 'string') return <span style={{ color: '#98C379' }}>"{data}"</span>;
  if (Array.isArray(data)) {
    if (data.length === 0) return <span style={{ color: 'var(--cv-text-muted)' }}>[]</span>;
    return (
      <div style={{ paddingLeft: depth ? 16 : 0 }}>
        ...
        {data.map((v, i) => (
          <div key={i} style={{ marginLeft: 16 }}>
            ...
            {i < data.length - 1 && <span style={{ color: 'var(--cv-text-dim)' }}>,</span>}
          </div>
        ))}
        <span style={{ color: 'var(--cv-text-dim)' }}>]</span>
      </div>
  ...
  if (typeof data === 'object') {
    ...
    if (entries.length === 0) return <span style={{ color: 'var(--cv-text-muted)' }}>{'{}'}</span>;
    return (
      <div style={{ paddingLeft: depth ? 16 : 0 }}>
        <span style={{ color: 'var(--cv-text-dim)' }}>{'{'}</span>
        {entries.map(([k, v], i) => (
          <div key={k} style={{ marginLeft: 16 }}>
            ...
            <span style={{ color: 'var(--cv-text-dim)' }}>: </span>
            <JsonTree data={v} depth={depth + 1} />
            {i < entries.length - 1 && <span style={{ color: 'var(--cv-text-dim)' }}>,</span>}
          </div>
        ))}
        <span style={{ color: 'var(--cv-text-dim)' }}>{'}'}</span>
      </div>
...
export function FilePreview({ content, contentType, base64, fileName, filePath, playbookName }: FilePreviewProps) {
    ...
    return (
      <div style={{ padding: 16, display: 'flex', justifyContent: 'center' }}>
        <img
          ...
          alt={fileName}
          style={{ maxWidth: '100%', maxHeight: '70vh', borderRadius: 4, border: '1px solid var(--cv-border)' }}
        />
      </div>
    );
  }
  ...
  if (isMarkdown(contentType, fileName)) {
    return (
      <div
        className="cv-md"
        style={{ padding: '20px 32px', fontFamily: 'var(--cv-sans)', fontSize: 13.5, lineHeight: 1.6, color: 'var(--cv-text)' }}
  ...
  if (isJson(contentType, fileName)) {
    ...
    return (
      <div style={{ padding: '20px 24px', fontFamily: 'var(--cv-mono)', fontSize: 12 }}>
  ...
  if (isHtml(contentType, fileName)) {
    ...
    const adjusted = content.replace(
      ...
      (_, prefix, url) => {
        ...
        const abs = dir ? (dir.endsWith('/') ? dir + cleanUrl : dir + '/' + cleanUrl) : cleanUrl;
