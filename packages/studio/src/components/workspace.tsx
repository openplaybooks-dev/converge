              ...
              {t.id === "playbook" && awaitingCount > 0 && (
                <span
                  style={{
                    ...
                    fontSize: 10,
                    fontWeight: 600,
                    background: "var(--cv-status-delta)",
                    color: "#FFF",
                    padding: "1px 6px",
                    borderRadius: 2,
                  }}
...
function ReviewSpotlight({
  ...
  return (
    <section style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      >
        <Caption color="#92400E">
          ◆ Needs review
          {queueSize > 1 ? ` · ${queueIndex + 1} of ${queueSize}` : ""}
        </Caption>
        {queueSize > 1 && (
          <button
          ...
          <header style={{ padding: "14px 20px 12px" }}>
            ...
            >
              ...
              <button
                ...
                className={`cv-btn cv-btn--changes${openForm ? " cv-btn--changes--active" : ""}`}
              >
                ✕ Reject…
              </button>
            </div>
...
function TaskDetailPanel({
  ...
  const [producedModalPath, setProducedModalPath] = useState<string | null>(
    null,
  );
    ...
    <>
      ...
      <aside
        style={{
          ...
          boxShadow:
            "-12px 0 32px rgba(17,24,39,0.14), -4px 0 12px rgba(17,24,39,0.08)",
          display: "flex",
          flexDirection: "column",
          overflowY: "auto",
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              {[task.mode, task.group, task.duration]
                .filter(Boolean)
                .join(" · ")}
            </div>
          </div>
        ...
        >
            ...
            <DetailSection label="Body">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: marked.parse(task.body) as string,
                }}
              />
            </DetailSection>
          )}
          ...
          <DetailSection label="Inputs">
            <Chips items={task.inputs} empty="—" />
          </DetailSection>
          <DetailSection label="Outputs">
            <Chips items={task.outputs} empty="—" />
          </DetailSection>
          ...
          {task.spawnedChildren && task.spawnedChildren.length > 0 && (
            <DetailSection
              label={`Spawned children (${task.spawnedChildren.length})`}
            >
          ...
          {(() => {
            const entries = Object.entries(task.vars ?? {}).filter(
              ([k]) => k !== "group",
            );
              ...
              <DetailSection label="Vars">
                ...
                >
                  {entries.map(([k, v]) => (
                    ...
                    >
                      <span
                        style={{ color: "var(--cv-text-dim)", flexShrink: 0 }}
                      >
                        {k}
                      </span>
                      <span
    ...
    >
      <header style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <p
          ...
          dangerouslySetInnerHTML={{ __html: italicize(playbook.goal) }}
        />
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          ) : playbook.runStatus === "complete" ||
            playbook.runStatus === "error" ? (
            <>
              {onReset && (
                <button
                  className="cv-btn cv-btn--ghost"
      ...
      {warning && (
        ...
        >
          ...
          <div
            style={{ flex: 1, fontSize: 12, lineHeight: 1.5, color: "#7C3D08" }}
          >
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        {selectedTask && (
          <TaskDetailPanel
            ...
            onAddComment={(kind, body) =>
              onAddComment(selectedTask.id, kind, body)
            }
...
export function JournalTab({ playbook }: { playbook: PlaybookData }) {
  const entries: [string, string, string][] = [
    ...
    [
      "14:41:34",
      "human",
      "◆ 04c-confirm-italic-motif · awaiting human verdict",
    ],
    ...
    <div style={{ padding: "20px 32px", maxWidth: 1100, marginInline: "auto" }}>
      <div className="journal-log">
        {entries.map(([ts, kind, msg], i) => (
          <div
            key={i}
            className={`journal-log__line journal-log__line--${kind}`}
          >
            ...
            <span
              className={`journal-log__msg${kind === "human" ? " journal-log__msg--human" : ""}`}
            >
              {msg}
            </span>
          </div>
    ...
    >
      ...
      {files.map(([path, size, state]) => (
        ...
        >
          ...
          <code style={{ fontFamily: "var(--cv-mono)", fontSize: 13 }}>
            {path}
          </code>
...
function classifyEvent(ev: JournalEvent): TimelineEntry["kind"] {
  ...
  if (
    t.includes("EXECUTION_START") ||
    t.includes("EXECUTION_END") ||
    t.includes("SESSION")
  )
    return "execution";
  if (
    t.includes("TASK_ATTEMPT_START") ||
    t === "task_start" ||
    t.includes("TASK_START")
  )
    return "task-start";
  if (
    t.includes("TASK_EXECUTION_COMPLETE") ||
    t === "task_complete" ||
    t.includes("TASK_COMPLETE")
  )
    return "task-complete";
  if (t.includes("TASK_FAILED") || t.includes("TASK_CRASH"))
    return "task-failed";
...
function extractTaskId(ev: JournalEvent): string | null {
  ...
  if (typeof ev.scope === "string" && ev.scope && !ev.scope.includes("."))
    return ev.scope;
  return null;
}
...
function groupEvents(events: JournalEvent[]): {
...
function statusGlyph(s: TimelineGroup["status"]): {
  glyph: string;
  color: string;
} {
  switch (s) {
    case "ok":
      return { glyph: "✓", color: "var(--cv-status-ok)" };
    case "fail":
      return { glyph: "✕", color: "var(--cv-status-fail)" };
    case "live":
      return { glyph: "●", color: "var(--cv-status-live)" };
    case "human":
      return { glyph: "◆", color: "var(--cv-status-delta)" };
    default:
      return { glyph: "◯", color: "var(--cv-text-dim)" };
  }
}
    ...
    <div className="timeline-row">
      <div className="timeline-rail">
        <span
          ...
          style={{ color: sg.color, borderColor: sg.color }}
        >
          {sg.glyph}
        </span>
      </div>
      <div className="timeline-card">
        ...
        >
          ...
          <span
            className={`timeline-card__pill timeline-card__pill--${group.status}`}
          >
            {sg.glyph}{" "}
            {statusLabel(
              group.status === "ok"
...
export function TimelineTab({ events }: { events: JournalEvent[] }) {
  ...
  const items = useMemo(() => {
    const mixed: Array<
      | { kind: "group"; data: TimelineGroup }
      | { kind: "standalone"; data: TimelineEntry }
    > = [];
    ...
    <div className="timeline-shell">
      <header className="timeline-shell__head">
        ...
        <span className="timeline-shell__meta">
          {totalEvents} {totalEvents === 1 ? "event" : "events"} · {totalTasks}{" "}
...
export function LogsTab({
  ...
  useEffect(() => {
    ...
    const headers: HeadersInit = ((): Record<string, string> => {
      try {
        const cfg = JSON.parse(
          localStorage.getItem("converge-studio-config") || "{}",
        );
        const ws = (cfg.workspaces || []).find(
          (w: any) => w.id === cfg.currentWorkspaceId,
        );
    ...
    fetch(`/api/playbooks/${encodeURIComponent(playbookName)}/logs`, {
      headers,
    })
      .then((r) => r.text())
      .then((text) => {
        if (!cancelled) setSavedLogs(text);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  return (
    <div className="logs-shell">
      <header className="logs-shell__head">
        ...
        <span className="logs-shell__meta">
          {combined.split("\n").length} lines
        </span>
        <span style={{ flex: 1 }} />
        ...
        <button
          type="button"
          className="cv-btn cv-btn--ghost logs-shell__btn"
          onClick={copyAll}
        >
          Copy
        </button>
