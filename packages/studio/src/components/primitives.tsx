...
const REVIEW_STYLES: Record<string, { bg: string; fg: string; border: string }> = {
...
export function ReviewPill({ state, children }: { state: string; children: ReactNode }) {
  const s = REVIEW_STYLES[state] || { bg: 'var(--cv-bg-elev)', fg: 'var(--cv-text-muted)', border: 'var(--cv-border-strong)' };
  return (
    <span style={{
...
export function MetaCell({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <span style={{ fontFamily: 'var(--cv-mono)', fontSize: 13, color: 'var(--cv-text)' }}>{value}</span>
    </div>
  );
}
