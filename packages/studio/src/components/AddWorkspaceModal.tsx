...
export function AddWorkspaceModal({
  ...
  async function handleOpenSelect(path: string, hasConverge: boolean) {
    if (!hasConverge) {
      setError(`No .converge/ found at ${path}. Use "Create new folder" instead.`);
      return;
    }
    setLoading(true);
    setError('');
    try {
      const res = await fetch(`/api/workspaces/inspect?path=${encodeURIComponent(path)}`);
  ...
  const title =
    step.kind === 'open-browse' ? 'Pick a workspace folder' :
    step.kind === 'create-browse' ? 'Pick a folder to create the workspace in' :
    step.kind === 'create-name' ? 'Name your workspace' :
  ...
  return (
    <div className="folder-picker__backdrop" onClick={close}>
      <div className="add-workspace-modal" onClick={(e) => e.stopPropagation()}>
        <header className="add-workspace-modal__head">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {step.kind !== 'menu' && (
              <button
                type="button"
                onClick={() => { setStep({ kind: 'menu' }); setError(''); }}
        ...
        {error && (
          <div className="folder-picker__error" style={{ margin: '8px 16px 0' }}>{error}</div>
        )}
        ...
        {step.kind === 'menu' && (
          <div className="add-workspace-modal__menu">
            ...
            {recents.length > 0 && (
              <>
                <div className="add-workspace-modal__section-label">Recent workspaces</div>
                  ...
                  {recents.map(ws => (
                    ...
                    >
                      ...
                      <span className="add-workspace-modal__recent-main">
                        <span className="add-workspace-modal__recent-name">{ws.name}</span>
                        <span className="add-workspace-modal__recent-path">{ws.path}</span>
                      </span>
                ...
                <div className="add-workspace-modal__divider"><span>or</span></div>
              </>
            )}
            ...
            <div className="add-workspace-modal__actions">
              <button
                ...
                onClick={() => { setStep({ kind: 'open-browse' }); setError(''); }}
              >
                <FolderOpen size={18} />
                <span>
              ...
              <button
                type="button"
                className="add-workspace-modal__action"
                onClick={() => { setStep({ kind: 'create-browse' }); setError(''); }}
              >
                <Plus size={18} />
                <span>
                  <span className="add-workspace-modal__action-title">
        ...
        {step.kind === 'create-name' && (
          <div className="add-workspace-modal__create-form">
            ...
            <label className="folder-picker__label" style={{ marginTop: 12 }}>Name</label>
            <input
              type="text"
            ...
            <label className="folder-picker__label" style={{ marginTop: 12 }}>Description (optional)</label>
            <input
              type="text"
              className="folder-picker__input"
            ...
            <div className="folder-picker__actions" style={{ marginTop: 16 }}>
              <button type="button" className="folder-picker__cancel" onClick={close}>
                Cancel
              </button>
              <button
                type="button"
