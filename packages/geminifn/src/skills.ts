/**
 * Adaptive skill reference loader for Gemini.
 *
 ...
 *
 * Note: Gemini does not support subagents, so only skills are supported
 ...
 *
 * Gemini does not support subagents, so @agent references are treated
...
export function enhancePrompt(prompt: string, cwd?: string): string {
  ...
  // Collect @agent references (also treated as skills since Gemini has no subagents)
