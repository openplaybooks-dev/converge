...
function spawnCli(
  ...
  return new Promise<string>((resolve, reject) => {
    const allowedToolsFlags = allowedTools?.length
...
export async function executeCode(
  code: string,
  tools: Record<string, ClaudeFn<any>>,
): Promise<unknown> {
...
/**
 * Create a composed function that can use other claudefn functions as tools.
 *
 * Two compose modes:
...
): (input?: string) => Promise<ClaudeFnResult<T>> {
  const {
...
async function executeComposeCode<T>(
  ...
  const wrappedTools: Record<string, ClaudeFn<any>> = {};
