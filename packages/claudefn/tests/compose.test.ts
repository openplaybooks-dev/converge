...
describe("compose — code mode (default)", () => {
  ...
  it("executes code returned by Claude and returns the result", async () => {
    mockCp.__setQueue([
      {
        stdout:
...
describe("compose — code mode orchestration", () => {
  it("Claude can chain multiple tools in code", async () => {
    const translateFn = vi.fn(async (input?: string) => ({
      data: `fr:${input}`,
      raw: `fr:${input}`,
      durationMs: 1,
  ...
  it("Claude can use loops and conditionals with tools", async () => {
