...
import { qwenfn } from "../src/index.js";
import { z } from "zod";
...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  let _nextStdout = "Hello from Qwen";
  let _nextStderr = "";
  let _nextExitCode = 0;
  let _nextDelay = 0;
  let _spawnArgs: any[] = [];
  ...
  return {
    ...
    __setNext: (opts: {
      ...
      _nextStdout = opts.stdout ?? "Hello from Qwen";
...
describe("qwenfn — core API", () => {
  it("returns a callable function", () => {
    const fn = qwenfn({ prompt: "Say hello" });
    expect(typeof fn).toBe("function");
  });

  it("invokes qwen CLI with -y and -p flag", async () => {
    const fn = qwenfn({ prompt: "Say hello" });
    await fn();
  ...
  it("works without {{input}} placeholder", async () => {
    const fn = qwenfn({ prompt: "What is 2+2?" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    const pIdx = args.indexOf("-p");
  ...
  it("passes cwd to spawn options", async () => {
    const fn = qwenfn({ prompt: "test", cwd: "/tmp/myproject" });
    await fn();
...
describe("qwenfn — dynamic prompt", () => {
  it("accepts a function that returns a prompt string", async () => {
    const fn = qwenfn({ prompt: () => "dynamic hello" });
    await fn();
    const [, args] = mockCp.__getSpawnArgs();
    const pIdx = args.indexOf("-p");
  ...
  it("prompt function receives undefined when no input given", async () => {
    ...
    const fn = qwenfn({ prompt: promptSpy });
    await fn();
...
describe("qwenfn — schema validation", () => {
...
describe("qwenfn — error handling", () => {
  ...
  it("times out and kills the process", async () => {
    ...
    const fn = qwenfn({ prompt: "test", timeoutMs: 50 });
  ...
  it("retries on failure up to maxRetries", async () => {
    ...
    const fn = qwenfn({ prompt: "test", maxRetries: 3 });
    const result = await fn();
  ...
  it("throws after exhausting all retries", async () => {
    ...
    const fn = qwenfn({ prompt: "test", maxRetries: 2 });
    await expect(fn()).rejects.toThrow();
  });
});
...
describe("qwenfn — promptless usage", () => {
