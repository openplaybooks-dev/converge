...
/** Hooks that fire at various points in the qwenfn lifecycle */
export interface QwenFnHooks {
...
/** Options passed to qwenfn() to create a callable function */
export interface QwenFnOptions<T = string> {
  /**
  ...
  /** Extra CLI flags passed to `qwen` */
  cliFlags?: string[];
}

/** The result returned by a qwenfn invocation */
export interface QwenFnResult<T = string> {
...
export interface ToolDef {
  /** The qwenfn to invoke when this tool is called */
  fn: QwenFn<any>;
  /** Description shown to Qwen so it knows when to use this tool */
  description: string;
}

/** Extended hooks for composed functions */
export interface ComposeHooks extends QwenFnHooks {
  /** Called after each tool invocation with the tool name, input, and result */
  onToolCall?: (ctx: {
    name: string;
    input: string;
    result: QwenFnResult<any>;
  }) => void | Promise<void>;
}
...
export interface ComposeOptions<T = string> {
  ...
  /**
   * Composition mode:
   * - `"code"` (default): Qwen writes a Node.js async function body that calls the
   ...
   * - `"tool_call"`: Qwen uses `<tool_call>` XML blocks to invoke tools.
