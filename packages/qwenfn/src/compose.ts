...
/** Extract a JS/TS code block from Qwen's response */
...
/** Parse `<tool_call>` blocks from Qwen's response */
...
/** Build the code-mode preamble that tells Qwen to write executable JS */
...
/** Spawn the Qwen CLI and collect output */
function spawnCli(
  prompt: string,
  cliFlags: string[],
...
export async function executeCode(
  code: string,
  tools: Record<string, QwenFn<any>>,
): Promise<unknown> {
...
/**
 * Create a composed function that can use other qwenfn functions as tools.
 *
 * Two compose modes:
 * - `"code"` (default): Qwen writes a JS function body; the system executes
 *   it with tools injected as named async functions.
 * - `"tool_call"`: Qwen uses `<tool_call>` XML blocks; the system parses
 *   them, executes tools, and feeds results back in a loop.
 */
export function compose<T = string>(
  options: ComposeOptions<T>,
): (input?: string) => Promise<QwenFnResult<T>> {
  const {
...
async function executeComposeCode<T>(
  ...
  const wrappedTools: Record<string, QwenFn<any>> = {};
