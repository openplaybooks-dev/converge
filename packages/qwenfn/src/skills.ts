 ...
 * (or self) of `cwd` containing .converge/. Honors QWEN_SKILLS_PATH override.
 */
function getSkillsDir(cwd?: string): string | null {
  if (process.env.QWEN_SKILLS_PATH) return process.env.QWEN_SKILLS_PATH;
...
/**
 * Resolve <projectRoot>/agents/. Honors QWEN_AGENTS_PATH override.
 */
function getAgentsDir(cwd?: string): string | null {
  if (process.env.QWEN_AGENTS_PATH) return process.env.QWEN_AGENTS_PATH;
