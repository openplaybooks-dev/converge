/**
 * Adaptive skill reference loader for Qwen.
 *
 ...
 *
 * Note: Qwen does not support subagents, so only skills are supported
 ...
 * Checks QWEN_SKILLS_PATH env var, then looks in project root/skills
 */
 ...
 * Checks QWEN_AGENTS_PATH env var, then looks in project root/agents
 */
 ...
 *
 * Qwen does not support subagents, so @agent references are treated
...
export function enhancePrompt(prompt: string, cwd?: string): string {
  ...
  // Collect @agent references (also treated as skills since Qwen has no subagents)
