...
export interface GlobalQueueOptions {
  /** Maximum concurrent SDK queries across all Node processes (default: 5) */
  maxConcurrent?: number;
  /** Maximum requests per minute across all Node processes (default: 60) */
  maxPerMinute?: number;
  /** Directory for cross-process state files (default: ~/.acpfn) */
  stateDir?: string;
...
export class GlobalQueue {
  ...
  constructor(options: GlobalQueueOptions = {}) {
    ...
    this.stateDir = options.stateDir ?? join(homedir(), ".acpfn");
   ...
   * Must be called when the SDK query completes (or errors).
   */
 ...
 * All acpfn/compose calls share this queue unless a custom queue is provided.
 */
