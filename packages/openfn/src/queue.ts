...
export interface GlobalQueueOptions {
  /** Maximum concurrent Opencode processes across all Node processes (default: 5) */
  maxConcurrent?: number;
  /** Maximum requests per minute across all Node processes (default: 60) */
  maxPerMinute?: number;
  /** Directory for cross-process state files (default: ~/.openfn) */
  stateDir?: string;
...
export class GlobalQueue {
  ...
  constructor(options: GlobalQueueOptions = {}) {
    ...
    this.stateDir = options.stateDir ?? join(homedir(), ".openfn");
   ...
   * Must be called when the Opencode process completes (or errors).
   */
 ...
 * All openfn/compose calls share this queue unless a custom queue is provided.
 */
