...
/** Extract a JS/TS code block from the response */
...
/** Build the code-mode preamble that tells the model to write executable JS */
...
export async function executeCode(
  code: string,
  tools: Record<string, OpenFn<any>>,
): Promise<unknown> {
...
/**
 * Create a composed function that can use other openfn functions as tools.
 *
 * Two compose modes:
 * - `"code"` (default): The model writes a JS function body; the system executes
 *   it with tools injected as named async functions.
 * - `"tool_call"`: The model uses `<tool_call>` XML blocks; the system parses
 *   them, executes tools, and feeds results back in a loop.
 */
export function compose<T = string>(
  options: ComposeOptions<T>,
): (input?: string) => Promise<OpenFnResult<T>> {
  const {
...
async function executeComposeCode<T>(
  ...
  const wrappedTools: Record<string, OpenFn<any>> = {};
  ...
  for (let iteration = 0; iteration < maxIterations; iteration++) {
    ...
    const callFn = () =>
      openfn({
        ...
        model,
        logDir,
        hooks: hooks?.onStream ? { onStream: hooks.onStream } : undefined,
      })();
...
async function executeComposeToolCall<T>(
  ...
  for (let iteration = 0; iteration < maxIterations; iteration++) {
    ...
    const toolCalls = parseToolCalls(response.data);
    ...
    history.push({ response: response.data, toolResults });

    if (iteration === maxIterations - 1) {
      finalResponse = stripToolCalls(response.data);
    }
  }
