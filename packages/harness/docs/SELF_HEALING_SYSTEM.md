...
3. Generates two files:
   ...
   - Both saved to: `.harness/journal/epics/{epic}/tasks/{task}/attempts/{n}/`
...
```typescript
import { TaskDefinitionRepairStrategy } from 'harness/repair';
...
```typescript
import { createAIContext } from 'harness/ai';
...
```bash
cd example
pnpm harness run --epic 02-prepare-designs
...
```typescript
// In harness.ts
import { registerHealthCheckHooks } from 'harness/repair';
...
**Solutions**:
- Check permissions on `.harness/journal` directory
- Check available disk space
- Review journal events for errors:
  ```bash
  cat .harness/journal/epics/{epic}/tasks/{task}/attempts/01/logs/events.jsonl
  ```
...
find .harness/journal -name "LEARN.md"

# View latest LEARN.md
cat .harness/journal/epics/{epic}/tasks/{task}/attempts/01/LEARN.md

# View journal events
cat .harness/journal/epics/{epic}/tasks/{task}/attempts/01/logs/events.jsonl | jq

# View attempt records
cat .harness/journal/epics/{epic}/tasks/{task}/attempts.jsonl | jq
```
...
```typescript
import { logTaskEvent } from 'harness/journal';
...
```typescript
const DRY_RUN = process.env.HARNESS_DRY_RUN === 'true';
...
```typescript
import { FixStrategy, StrategyContext, StrategyOutcome } from 'harness/repair';
import { createAIContext } from 'harness/ai';
...
```typescript
import { StrategyRegistry } from 'harness/repair';
...
4. File issue in harness framework repository
