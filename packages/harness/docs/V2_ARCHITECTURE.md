...
harness run-v2 .harness/epics/01-data-analysis.ts                    # Parent task
harness run-v2 .harness/epics/01-data-analysis/001-analyze.ts       # Child task
harness run-v2 .harness/epics/02-ux-design/003-generate-screens/001-screen-home.ts  # Grandchild
```
...
```typescript
// .harness/epics/02-ux-design/003-generate-screens.ts
...
export default taskDef()
  ...
  .yields({
    ...
    outputDir: '.harness/epics/02-ux-design/003-generate-screens',
    template: '.harness/epics/02-ux-design/003-generate-screens/000-screen-{slug}.ts.tpl',
    maxTasks: 20,
  })
  .inputs(['.stitch/SITE.md', '.stitch/DESIGN.md'])
  .outputs(['.harness/epics/02-ux-design/003-generate-screens/**/*.ts'])
  .build();
```
...
```typescript
// .harness/epics/02-ux-design/003-generate-screens/001-screen-home.ts (AI-generated)
export default taskDef()
  .id('screen-home')
  .title('Home Screen')
  .agent('ui-developer')
...
2. Add CLI flag: `harness run --v2 <task-file>`
...
harness run --v2 .harness/epics/01-data-analysis.ts

# Works with any level
harness run --v2 .harness/epics/01-data-analysis/001-analyze.ts
harness run --v2 .harness/epics/02-ux/003-screens/001-screen-home.ts
```
