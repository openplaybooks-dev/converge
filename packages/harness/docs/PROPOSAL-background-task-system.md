...
**Scope:** `harness/packages/harness` — new runtime primitives

---

## 1. Problem Statement

The current harness executes tasks **synchronously and sequentially** within the convergence loop. This creates problems for:
...
## 3. Execution Model: `unit.run` → `harnessFn` → `executeFn`

The user writes `.execute(fn)`. But that `fn` never runs bare — it's wrapped by the harness convergence machinery. Understanding this layering is critical:

```
unit.run(task)                          ← convergence loop (gap detect → fix → retry)
  └─ harnessFn(ctx)                     ← convergence policy (run → check outputs → check gaps)
...
|-------|---------------|
| `unit.run()` | Outer convergence loop. Detects gaps, plans fixes, retries. Calls `harnessFn` per iteration. |
| `harnessFn(ctx)` | Per-iteration policy. Calls `ctx.run()` (the executor), then checks outputs and validations. Returns `{ converged: boolean }`. |
...
|----------|----------------------------------|
| *(none)* | Runs `harnessFn` → `executeFn` sequentially, blocks until converged |
...
| `.schedule('15s')` | `unit.run` calls `harnessFn` → `executeFn` on an interval. Each invocation is a fresh convergence iteration. |
...
**The harness wrapping is the same for all modifiers.** The default `harnessFn` always does:
...
- Write state to `.harness/runtime/bg/<task-id>/`
...
- [ ] Filesystem state output (`.harness/runtime/bg/`)
- [ ] `ctx.bg(id)` accessor
