...
`scan/backlog-runner.ts` runs shell commands and parses output into `BacklogItem[]`. But these items are stored in `backlogs.jsonl` and displayed by `harness backlog` — they **never enter the gap system**. The convergence loop doesn't know about them.
...
```
.harness/journal/gap-ledger.jsonl
```
...
| `cli/commands-trend.ts` | **New file**: `harness trend` command | Small |
...
> **Harness doesn't just run tasks. It compounds.**
>
