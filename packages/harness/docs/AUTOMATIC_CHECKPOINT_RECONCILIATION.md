...
The harness framework now automatically detects and corrects inconsistencies between checkpoint state and filesystem reality, ensuring that the execution state always matches the actual project state.
...
```bash
$ pnpm harness tree 002-generate-screen-prompts
...
▶  Next: .harness/epics/02-prepare-designs/002-generate-screen-prompts/002-001-prompt-home-lesson-tree/SKILL.md
```
...
5. **Manual reconcile command** (`harness reconcile --fix-all`)
