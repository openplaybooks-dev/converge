...
The `harness search` command helps you find information in crew projects using tools you already know: **Grep, Glob, and Read**.
...
grep -r '"status": "pending"' .harness/epics/*/tasks/*/task.json
```
...
grep -r '"title".*authentication' .harness/epics/*/tasks/*/task.json
```
...
cat .harness/epics/02-*/tasks/03-*/task.json
```
...
- `m1.1` → `.harness/epics/01-{slug}/tasks/01-{slug}/task.json`
- `m2.3` → `.harness/epics/02-{slug}/tasks/03-{slug}/task.json`
- `m4.5` → `.harness/epics/04-{slug}/tasks/05-{slug}/task.json`

## All Available Patterns

Run `harness search` to see all 12 available search patterns, or `harness search quick` for a quick reference.
...
2. Agent runs: `harness search status`
3. Agent sees example: `grep -r '"status": "pending"' .harness/epics/*/tasks/*/task.json`
...
The `.harness/` directory structure IS the search index. File navigation IS the search engine.
