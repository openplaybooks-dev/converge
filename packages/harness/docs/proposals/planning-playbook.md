...
Add a built-in `plan` playbook that auto-scans the current project state and takes a user prompt to generate (or update) a full harness playbook — turning `harness plan --prompt "..."` into continuous, decomposable, top-down planning.

---

## Problem

Today, the `harness-planning` skill provides excellent planning *guidance* — analyze, discover, architect, validate — but it's a passive reference. A human (or AI agent) must manually:

1. Read SKILL.md, pick the right phase
2. Load the right playbook (analyze.md, discovery.md, etc.)
3. Run each step manually
4. Create `.harness/` artifacts by hand
...
| No playbook output | Plans live in `.harness/epics/` but aren't runnable playbooks |
...
│         └──────────┘          │
│                                 │
│  Output: .harness/playbooks/X/  │  ← Runnable playbook
└─────────────────────────────────┘
```
...
harness plan --prompt "Add dark mode support" --name dark-mode
...
harness plan --prompt "Implement the features described in REQUIREMENTS.md"

# Equivalent long form (plan is a built-in playbook)
harness run --playbook=plan --prompt="Build a todo app with auth"
```

### What Happens

```
harness plan --prompt "Build a dashboard with real-time data"

  000-setup-skills — SETUP
    ├── Install harness-planning skill to .harness/skills/
  ...
  001-scan — SCAN (auto)
    ...
    └── Output: .harness/plan-state/analysis.json
  ...
  002-research — RESEARCH (from prompt)
    ...
    └── Output: .harness/plan-state/requirements.json
  ...
  003-outline — OUTLINE (high-level)
    ...
    └── Output: .harness/plan-state/outline.json
...
```yaml
# Built-in: skills/harness-planning/playbooks/plan/playbook.yml
name: plan
description: Generate a harness playbook from a prompt and project scan
...
Each phase:
- Has explicit **inputs** and **outputs** (JSON files in `.harness/plan-state/`)
- Has **checks** (output file exists, valid JSON, required fields present)
- References the `harness-planning` skill for domain knowledge
...
**Skip condition**: If `.harness/plan-state/analysis.json` already exists and cwd hasn't changed, reuse it.
...
**Input**: `.harness/plan-state/analysis.json` + user prompt
**Output**: `.harness/plan-state/requirements.json`
```json
{
...
**Input**: `.harness/plan-state/requirements.json` + `.harness/plan-state/analysis.json`
**Output**: `.harness/plan-state/plan.json`
```json
{
  "name": "iot-dashboard",
...
**Input**: `.harness/plan-state/plan.json` + `.harness/plan-state/requirements.json`
**Output**: `.harness/plan-state/validation.json`
...
```bash
# Initial plan
harness plan --prompt "Build a todo app" --name todo-app

# User reviews, wants changes
harness plan --prompt "Add collaborative editing and offline sync" --name todo-app --update
...
hand-written `wbs.js`, the harness drives AI to generate the script dynamically:
...
TASK.md (type: ai, prompt)
    ↓
harness reads prompt + task context (vars, inputs, project state)
    ↓
AI generates wbs.js (using WBS API reference + examples)
    ↓
harness validates wbs.js (syntax, ESM, spawn calls)
    ↓
harness executes wbs.js via standard nodejs WBS executor
    ↓
...
# Start executing
harness run --playbook=dashboard

# Midway through, realize we need more planning
harness plan --prompt "Add export-to-PDF feature" --name dashboard --update
...
The built-in `plan` playbook ships as a **system playbook** inside the `harness-planning` skill:
...
When the user runs `harness plan` for the first time:

1. Harness detects the `plan` playbook is a **system playbook** (not in `.harness/playbooks/`)
2. Copies it to `.harness/playbooks/plan/`
...
- `harness plan --prompt "..."` works out of the box with no setup
...
| File | Purpose |
|------|---------|
| `skills/harness-planning/playbooks/plan/playbook.yml` | System playbook config |
| `skills/harness-planning/playbooks/plan/tasks/TASK.md` | Root WBS task |
| `skills/harness-planning/playbooks/plan/tasks/wbs.js` | Main pipeline generator |
| `skills/harness-planning/playbooks/plan/tasks/wbs/decompose-epics-wbs.js` | Per-epic delegation WBS (nodejs fallback) |
| `skills/harness-planning/playbooks/plan/tasks/wbs/deepen-tasks-wbs.js` | Conditional sub-decomposition WBS (nodejs fallback) |
| `skills/harness-planning/preferences/wbs-ai-guide.md` | AI-driven WBS reference and patterns |
| `examples/planning/README.md` | Example documentation |
| `examples/planning/.harness/playbooks/plan/` | Example playbook (mirrors skill) |
...
| File | Change |
|------|--------|
| `skills/harness-planning/SKILL.md` | Add `plan` playbook to phase detection matrix and quick reference |

---

## Open Questions

1. **Prompt storage**: Should the original prompt be saved in `.harness/plan-state/prompt.md` for re-planning context?
2. **Plan diffing**: When `--update` is used, should we show a diff of plan changes before applying?
3. **Interactive mode**: Should `harness plan` support `--interactive` for the discovery phase (ask clarifying questions)?
...
- **Replacing the skill**: The `harness-planning` skill remains the authoritative reference for manual planning. The playbook automates the common case.
- **AI model selection**: The playbook doesn't pick which AI model to use — that's a runtime concern.
- **Execution**: The `plan` playbook only generates plans. Execution is handled by `harness run --playbook=<generated-name>`.
