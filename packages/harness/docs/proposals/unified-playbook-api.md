...
Today, the harness has two overlapping systems:

### Workflow API (the "what")

```
.harness/workflows/react-app/
...
- `harness workflow run react-app --idea="todo app"` → generates an epic
- Then you **separately** run `harness run` to execute it
...
- `harness run --converge` — wave-based convergence
- `harness run --step` — single task execution
...
```
.harness/playbooks/react-app/
├── playbook.yml                ← unified definition
├── EPIC.md                     ← epic template (optional)
└── tasks/
...
### 1. The default `harness run` is an implicit playbook

Today's `harness run` discovers tasks from the filesystem and executes them. Under the playbook model, this is simply the **default playbook** — an unnamed playbook that discovers its task list from `.harness/epics/`.

```bash
# These are equivalent:
harness run                          # implicit default playbook
harness playbook run default         # explicit default playbook
```
...
```bash
harness playbook run react-app --idea="expense tracker"
```

This single command:
1. Loads `playbook.yml` (definition + run config)
2. Resolves inputs → generates epicId
3. Instantiates task templates into `.harness/epics/{epicId}/`
...
harness workflow info react-app
harness workflow run react-app --idea="todo app"

# Then separately:
harness run                          # execute everything
harness run --converge               # with convergence
harness run --step                   # one task at a time
harness run --filter fix-issue-42    # filter to a specific epic
```

### After (one system)

`--playbook` is a flag on `harness run`. Playbook is a wrapper layer — all existing
run flags work with or without it.

```bash
# Run with a named playbook (generate epic + execute)
harness run --playbook=react-app --idea="todo app"
harness run --playbook=react-app --idea="todo" --converge --max-iterations=50

# Default run (no playbook — existing behavior, unchanged)
harness run                                      # autonomous loop
harness run --converge                           # convergence mode
harness run --step                               # one task at a time

# Playbook inspection (separate subcommand)
harness playbook list                            # list available playbooks
harness playbook info react-app                  # show details + run config
harness playbook history react-app               # list past executions
```
...
```bash
# CLI overrides:
harness run --playbook=react-app --converge --max-iterations=50
```
...
- Add `harness playbook run` command
- Internally, `playbook run` calls `generateEpicFromWorkflow` + `autonomousRun`/`convergeRun`
- Existing `harness workflow run` and `harness run` continue to work

### Phase 2: Wire `harness run` to the default playbook

- `harness run` becomes shorthand for `harness playbook run default`
...
- `harness workflow run` prints deprecation notice, delegates to `harness playbook run`
- `harness workflow list` → `harness playbook list`
- Rename `.harness/workflows/` to `.harness/playbooks/` (support both during transition)
...
4. **No execution grouping** — a single `harness run` may touch multiple sessions, but there's no "execution" wrapper
...
// → .harness/journal/epics/{epicId}/{taskId}/...

// After:
getJournalStructure(projectDir, epicId, taskId, { playbook, executionId })
// → .harness/journal/playbooks/{playbook}/executions/{executionId}/epics/{epicId}/{taskId}/...
```

The context can be provided explicitly or read from environment variables (`HARNESS_PLAYBOOK`, `HARNESS_EXECUTION_ID`) — same pattern as `HARNESS_TASK_ATTEMPT`.
...
`_default` is the implicit playbook for `harness run` with no named playbook. It groups what today are anonymous sessions + flat epic journals into a structured execution history.
...
- `process.env.HARNESS_PLAYBOOK` → playbook name
- `process.env.HARNESS_EXECUTION_ID` → execution ID
...
### Simple one-off playbook (replaces `harness run`)
...
```yaml
# .harness/playbooks/fix-issue/playbook.yml
name: fix-issue
...
```bash
harness playbook run fix-issue --issue=42
harness playbook run fix-issue --issue=43
harness playbook run fix-issue --issue=44
...
```yaml
# .harness/playbooks/self-dev/playbook.yml
name: self-dev
...
```bash
harness playbook run self-dev --idea="expense tracker"
...
| **Epic definitions** | No | `.harness/epics/` still holds instantiated task definitions. |
...
| `harness workflow run` + `harness run` (two steps) | `harness playbook run` (one step) |
| No execution identity | `journal/playbooks/{name}/` tracks all runs |
| No composition | Playbooks can reference other playbooks |
| `harness run` is anonymous | `harness run` = default playbook (with history) |
| Three overlapping config types | One `PlaybookDef` + CLI overrides |

The playbook is the single abstraction for "something you run in the harness." It can be a one-off task board, a repeatable pipeline, or a long-running convergence loop. Define it once, run it many times, track it forever.
