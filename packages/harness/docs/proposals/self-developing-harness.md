...
A named, reusable pipeline. Definition lives in `.harness/workflows/{slug}.md`.

```yaml
# .harness/workflows/react-app.md
---
name: react-app
...
checks:
  ...
  - id: stores-connected
    cmd: "node .harness/goals/002-stores-connected/check.js"
---
```
...
Same `ArtifactAPI` the framework already uses. Each execution gets its own artifact store at `executions/{id}/artifacts/`. Same timestamped folder layout as `.harness/artifacts/`.
...
{"executionId":"exec-001","id":"check:stores-connected","type":"check","cmd":"node .harness/goals/002-stores-connected/check.js","ok":false,"output":"3 stores unused","exitCode":1,"collectedAt":"2026-04-12T02:45:01Z"}
...
harness run --workflow react-app --input inputs/todo-app.md

# Run with multiple inputs (one execution each)
harness run --workflow react-app --inputs inputs/
...
harness trend --workflow react-app

# Show facts for a specific check across all executions
harness facts --workflow react-app --filter "id:check:tsc-clean"

# Show recurring gaps
harness gaps --workflow react-app --recurring

# List all executions
harness executions --workflow react-app
```

### Workflow without a definition file

Any `harness run` can optionally tag itself as a workflow execution:

```bash
harness run --workflow my-pipeline
```

This creates `journal/workflows/my-pipeline/` and records the execution there, even without a `.harness/workflows/my-pipeline.md` file. The definition file adds checks and pipeline structure but isn't required.
...
The existing `ArtifactStore` writes to `.harness/artifacts/{key}/{timestamp}/`. Per-execution artifacts use the same `ArtifactAPI` but scoped to `journal/workflows/{slug}/executions/{id}/artifacts/`.
...
**1. Workflow definition loader** — Reads `.harness/workflows/{slug}.md` (YAML frontmatter + markdown body). Same pattern as `PROJECT.md` and `TASK.md`.
...
**4. CLI integration** — `harness run --workflow {slug}` routes through the execution manager. `harness trend`, `harness facts`, `harness gaps` query workflow-level files.
