...
A playbook is the root unit of organization in the harness. Everything — tasks, goals, journal, sessions — lives under a playbook.
...
```bash
harness run                          # first run — executes 01, 02 spawns subtasks, 03
harness run                          # second run — picks up where it left off
harness run --converge               # converge until goals pass
```
...
export async function run(ctx) {
  ...
  await ctx.spawn({
    ...
    body: `Read issue #${issue}, reproduce the bug, identify root cause.
           Write findings to .harness/fix-${issue}/analysis.json.`,
    outputs: [`.harness/fix-${issue}/analysis.json`],
  });
  ...
  await ctx.spawn({
    ...
    inputs: [`.harness/fix-${issue}/analysis.json`],
...
```bash
harness run --playbook=fix-issue --issue=42    # spawns 3 tasks for issue 42
harness run --playbook=fix-issue --issue=43    # spawns 3 tasks for issue 43
harness run --playbook=fix-issue --issue=44    # spawns 3 tasks for issue 44
harness playbook history fix-issue             # shows all 3 runs
```
...
| Example | `harness run` | `harness run --playbook=X --key=Y` |
...
harness run --playbook=fix-issue --issue=42 --converge
harness run --playbook=fix-issue --issue=42 --step --dry
harness run --playbook=fix-issue --issue=42 --max-iterations=50
...
`--playbook` is a flag on `harness run`, not a separate command. All existing flags compose with it. The playbook's `run:` config provides defaults, CLI flags override them.

### Inspecting playbooks

```bash
# List available playbooks
harness playbook list

# Show details (inputs, task DAG, run config, checks)
harness playbook info fix-issue
...
```bash
harness tree --playbook=fix-issue
harness inspect --playbook=fix-issue
harness metrics --playbook=fix-issue
harness journal --playbook=fix-issue
```

## How It Works

**Continuous playbook** (`harness run`):
...
**Keyed playbook** (`harness run --playbook=fix-issue --issue=42`):
...
```typescript
import { resolvePlaybookPaths } from 'harness';

const paths = resolvePlaybookPaths(projectDir, 'fix-issue');
paths.tasks        // .harness/playbooks/fix-issue/tasks/
paths.goals        // .harness/playbooks/fix-issue/goals/
paths.journal      // .harness/journal/fix-issue/
paths.journalTasks // .harness/journal/fix-issue/tasks/
paths.sessions     // .harness/journal/fix-issue/sessions/
paths.config       // .harness/playbooks/fix-issue/playbook.yml

const defaults = resolvePlaybookPaths(projectDir);
paths.tasks        // .harness/playbooks/default/tasks/
paths.journal      // .harness/journal/default/
```
...
```bash
mkdir -p .harness/playbooks/fix-issue/tasks/001-investigate
mkdir -p .harness/playbooks/fix-issue/tasks/002-fix
```
...
title: Investigate issue ${issue}
---
Read issue ${issue} and identify the root cause.' > .harness/playbooks/fix-issue/tasks/001-investigate/TASK.md

echo '---
title: Fix issue ${issue}
---
Implement the fix.' > .harness/playbooks/fix-issue/tasks/002-fix/TASK.md
```
