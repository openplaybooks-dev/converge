# Harness Converge: Idea to Production
...
|-------|-----------|------------|---------|
| Ideation & Requirements | **File existence + structure** | Low — "does the artifact exist and have sections?" | `test -f .harness/requirements.md && grep -q "## User Stories" .harness/requirements.md` |
...
**Goal**: Project structure exists with harness configuration.

```yaml
goal: project-bootstrapped
description: "Harness project initialized with valid configuration"
checks:
...
checks:
  - id: requirements-exists
    cmd: "test -f .harness/analysis/requirements.md"
  - id: requirements-has-stories
    cmd: "grep -c '## User Stor' .harness/analysis/requirements.md | xargs test 0 -lt"
  - id: requirements-has-acceptance
    cmd: "grep -c '## Acceptance Criteria' .harness/analysis/requirements.md | xargs test 0 -lt"
  - id: data-model-exists
    cmd: "test -f .harness/analysis/data-model.md"
  - id: sitemap-exists
    cmd: "test -f .harness/analysis/sitemap.md"
  - id: sitemap-has-routes
    cmd: "grep -cE '^- /' .harness/analysis/sitemap.md | xargs test 2 -le"
outputs:
  - .harness/analysis/requirements.md
  - .harness/analysis/data-model.md
  - .harness/analysis/sitemap.md
  - .harness/analysis/tech-stack.md
```

**Skill**: `harness-planning` (ANALYZE → DISCOVER → ARCHITECT → VALIDATE)
...
checks:
  ...
  - id: all-screens-have-prompts
    cmd: |
      ROUTES=$(grep -cE '^- /' .harness/analysis/sitemap.md)
...
harness converge --goals "implementation-complete"

# Or resume from where you left off
harness converge --resume
```
...
Phase 1 outputs `.harness/analysis/sitemap.md` → Phase 2 reads it to know which screens to design. Phase 2 outputs `.stitch/designs/` → Phase 4 reads them to know which components to implement. The chain is self-documenting.
...
If Phase 4 stalls after 15 iterations, the harness:
...
**Mitigation**: Human review gates between phases. The harness pauses and asks "Phase 1 complete — review requirements before proceeding?"
...
| **3** | `harness converge` CLI command | Medium | The entry point |
