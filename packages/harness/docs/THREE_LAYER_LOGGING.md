...
The harness framework uses a **three-layer logging architecture** where each layer serves a specific purpose:
...
**Location**: `.harness/journal/sessions/{sessionId}/`
...
**Location**: `.harness/journal/epics/{epic}/tasks/{task}/attempts/{n}/`
...
const taskEvents = new TaskEventWriter(
  `.harness/journal/epics/03-implement-app/tasks/task-1/attempts/01/events.jsonl`
);
...
const sessionFormatter = new ConsoleFormatter(
  `.harness/journal/sessions/2026-04-04T09-44-30-rot6k8/events.jsonl`
);
