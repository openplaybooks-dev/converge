...
The `--step` flag enables **single-step execution mode** for debugging and controlled iteration through the autonomous harness loop.

## Purpose

When developing or debugging harness workflows, you often want to:
...
harness run --step --verbose

# Run one iteration without auto-fix (manual mode)
harness run --step --auto-fix=false
```
...
### 3. Debug Messages
- Shows "Running Single Step (Debug Mode)" instead of "Starting Autonomous Harness Loop"
- Shows "Single Step Complete" instead of "Autonomous Loop Complete"
- Displays tip: "Run 'harness run --step' again to execute the next iteration"
...
cat .harness/journal/project/*.events.jsonl
cat .harness/journal/project/*.gaps.yml
...
# Manually fix issues
vim .harness/epics/my-epic/tasks/my-task.ts
...
export async function autonomousRun(userConfig: AutonomousRunConfig = {}): Promise<void> {
  ...
  if (config.step) {
    log(config, `\n💡 Tip: Run 'harness run --step' again to execute the next iteration`);
  }
}
```
...
cat .harness/journal/project/project.gaps.yml
...
| Loop message | "Starting Autonomous Harness Loop" | "Running Single Step (Debug Mode)" |
