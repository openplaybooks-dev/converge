...
Standard WBS (`type: nodejs`) requires a hand-written `wbs.js`. AI-driven WBS (`type: ai`) tells the harness to:
...
TASK.md (type: ai, prompt)
    │
    ▼
  harness reads prompt + context
    │
    ▼
  AI generates wbs.js
    │
    ▼
  harness validates wbs.js (syntax, structure)
    │
    ▼
  harness executes wbs.js via nodejs WBS executor
    │
    ▼
...
The harness constructs a generation prompt from:
...
When `type: ai` is used, the harness:

### Step 1: Generate

Constructs a prompt:
```
You are generating a WBS script for the harness task system.
...
If validation fails, the harness retries generation (up to `maxAttempts`).
...
wbs:
  type: ai
  prompt: |
    Read .harness/plan-state/outline.json.
...
wbs:
  type: ai
  prompt: |
    Read .harness/plan-state/plan.json.
