# Playbook: Setup Harness for Any Project

## Mission

**Initialize harness framework in existing projects to track work systematically.**
...
- `.harness/` - Task definitions (our workspace)
```
...
```bash
mkdir -p .harness/epics/00-foundation/tasks/001-setup-project
```

```yaml
# .harness/epics/00-foundation/tasks/001-setup-project/TASK.md
---
...
```yaml
# .harness/epics/00-foundation/tasks/002-implement-auth/TASK.md
---
...
```bash
# Initialize harness
harness init --name="[Project Name]"

# Run checks to mark completed tasks
harness run --step  # Each placeholder will pass checks and complete
```
...
```yaml
# .harness/epics/01-api-completion/tasks/001-implement-users-endpoint/TASK.md
---
...
```yaml
# .harness/epics/02-frontend-ui/tasks/001-generate-dashboard/TASK.md
---
...
```yaml
# .harness/epics/00-foundation/tasks/000-nextjs-setup/TASK.md
---
...
```yaml
# .harness/epics/00-foundation/tasks/000-react-setup/TASK.md
---
...
```yaml
# .harness/epics/00-foundation/tasks/000-express-setup/TASK.md
---
...
```yaml
# .harness/epics/00-foundation/tasks/000-django-setup/TASK.md
---
...
harness run 00-foundation  # All should pass immediately
...
harness run --step  # Now runs first real task
```
...
```bash
# User Request: "Setup harness for my Next.js project"
...
mkdir -p .harness/epics/00-foundation/tasks/001-existing-pages
cat > .harness/epics/00-foundation/tasks/001-existing-pages/TASK.md <<'EOF'
---
...
for page in blog services pricing faq team; do
  mkdir -p .harness/epics/01-new-pages/tasks/001-generate-${page}
  cat > .harness/epics/01-new-pages/tasks/001-generate-${page}/TASK.md <<EOF
---
...
# Phase 5: Execute
harness run 00-foundation  # Mark existing work complete
harness run --step         # Start generating new pages
```
...
✅ **Harness initialized** - `.harness/` directory created
...
✅ **Executable** - Can run `harness status` and see clear path forward
...
# Start execution
harness run --step  # Or harness run for autonomous mode
