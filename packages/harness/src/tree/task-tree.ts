...
export class TaskTree {
   ...
   */
  static async load(projectDir: string, harnessConfig: HarnessConfig, preScannedDiscovery?: import('../discovery/types.ts').DiscoveryResult): Promise<TaskTree> {
    ...
    if (preScannedDiscovery) {
      ...
      const scanner = createDiscoveryScanner(harnessConfig.discovery || { epics: [], tasks: [] }, projectDir);
    ...
    for (const result of epicResults) {
      ...
      const node = new TreeNode(result.unit, checkpoint);
    ...
    // 3. Build parent-child edges (use journalTaskId structure, not Unit.parent)
    // Units loaded via fromPath() don't have parent set, so we use journalTaskId instead
    // journalTaskId format: "parentTaskId/childTaskId" for WBS children, "taskId" for top-level
    ...
    for (const [parentId, children] of childrenMap.entries()) {
      ...
      children.sort((a, b) => Unit.compareBySortIndex(a.unit, b.unit));
    ...
    root.children.sort((a, b) => Unit.compareBySortIndex(a.unit, b.unit));
    ...
    return new TaskTree(projectDir, root, nodes, checkpoint, harnessConfig);
  }
  ...
  async reload(): Promise<void> {
    const newTree = await TaskTree.load(this.projectDir, this.harnessConfig);
   ...
   *
   * Traverses .harness/journal/tasks/** for per-task checkpoint.json files,
  ...
  async reconcileFromJournal(): Promise<{ added: string[]; removed: string[]; autoCompleted: string[]; completed: Set<string>; failed: Set<string>; seeded: Set<string> }> {
    ...
    if (existing) {
      if (existing.version === 1) {
        // V1 checkpoint has arrays
        flatCompleted = new Set(existing.completedTasks ?? []);
        flatFailed = new Set(existing.failedTasks ?? []);
        flatSeeded = new Set(existing.seededTasks ?? []);
      } else if (existing.version === 2) {
        // V2 checkpoint - get from filesystem via CheckpointManager
        ...
        // Note: seeded tasks are tracked in per-task checkpoints, not separately in V2
      }
    }
