...
The `JournalTree` loads execution history from the journal (`.harness/journal/`) and provides a parallel tree structure to `TaskTree`:

- **TaskTree**: What to execute (from `.harness/epics/`)
- **JournalTree**: What was executed (from `.harness/journal/`)
...
```typescript
import { JournalTree } from '@harness/core/tree';
