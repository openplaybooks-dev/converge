...
Successfully migrated `harness tree` and `harness gantt` commands to use the new unified TaskTree API instead of direct file scanning.
...
Both commands now use the same tree structure as `harness run`, ensuring consistency across all commands.
...
### Phase 1 (Complete ✅)
- ✅ Migrate `harness tree` to use TaskTree API
- ✅ Migrate `harness gantt` to use TaskTree API
...
⟳  Currently executing: .harness/epics/01-data-analysis/002-generate-design-system/task.ts
```
...
Example output:
```bash
$ pnpm harness tree 03-implement-app
...
⟳  Parent task executing: .harness/epics/03-implement-app/003-generate-svg-assets
▶  Next subtask: .harness/epics/03-implement-app/003-generate-svg-assets/task/003-012-asset-loading-spinner
```
...
Successfully migrated both `harness tree` and `harness gantt` commands to use the unified TaskTree API. The migration:
...
The commands now use the same tree structure as `harness run`, ensuring consistency across the entire harness CLI.
