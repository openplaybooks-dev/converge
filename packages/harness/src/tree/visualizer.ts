...
export class TreeVisualizer {
  private options: Omit<Required<VisualizationOptions>, 'cursor'> & { cursor?: Cursor };
  ...
  private async buildTree(unit: Unit, depth: number): Promise<TreeNode> {
    ...
    if (unit.children) {
      for (const child of unit.children) {
        ...
        if (this.options.showOnlyIncomplete && childNode.status === 'completed') {
          continue;
        }
  ...
  private renderNode(
    ...
    const titleColor = node.status === 'completed' ? 'green' : node.status === 'failed' ? 'red' : 'white';
    ...
    if (this.options.showDescriptions && (node as any).description) {
      ...
      lines.push(`${prefix}${descPrefix}${this.color((node as any).description, 'dim')}`);
    }
  ...
  private abbreviatePath(fullPath: string): string {
    // Show only relative path from .harness/
    const harnessIndex = fullPath.indexOf('.harness/');
    if (harnessIndex !== -1) {
      return fullPath.slice(harnessIndex);
    }
    return fullPath;
  }
