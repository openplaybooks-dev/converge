...
```
.harness/epics/03-implement-app/001-implement-design-system/SKILL.md
```

The corresponding journal path is:
```
.harness/journal/tasks/03-implement-app/001-implement-design-system/checkpoint.json
```
...
for (const taskNode of taskTree.getAllNodes()) {
  const taskPath = taskNode.unit.path;  // e.g., ".harness/epics/03-implement-app/001-task"
...
```typescript
const unit = await Unit.fromPath('.harness/epics/03-implement-app/001-task');
...
- **Old**: `.harness/journal/tasks/03-implement-app/tasks/001-task/`
- **New**: `.harness/journal/tasks/03-implement-app/001-task/`
...
cd .harness/journal/tasks/03-implement-app
mv tasks/* .
rmdir tasks
```
...
private getCheckpointPath(): string {
  const journalRoot = path.join(this.projectDir, '.harness', 'journal');
