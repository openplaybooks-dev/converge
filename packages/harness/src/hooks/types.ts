 ...
 *
 * Provides typed hooks for every stage of the harness lifecycle:
 ...
 * Use these as keys in `harness.ts` hooks config or `registry.register()`.
 */
...
export interface HookPayloads {
  ...
  'project:complete': { ctx: ProjectContext; result: ProjectOrchestrationResult };
  ...
  // Convergence
  'convergence:achieved': { epicId: string; iterations: number; gapsResolved: number };
  'convergence:stalled':  { epicId: string; reason: string; stallCount: number; gaps: Gap[] };

  // Checkpoint
  'checkpoint:created':  { checkpointId: string; iteration: number; epicId: string };
  'checkpoint:restored': { checkpointId: string; iteration: number };

  // Discovery
  'discovery:found':   { files: string[]; type: 'task' | 'epic' | 'check' | 'plan' | 'skill' | 'agent' };
  'discovery:changed': { added: string[]; removed: string[]; modified: string[] };
  ...
  'background:crashed':   { taskId: string; code: number | null; signal: string | null };
 ...
 */
export type AnyHookFn = (payload: HookPayloads[HookEvent]) => void | Promise<void>;
...
/* ------------------------------------------------------------------ */
/*  Hooks Config Shape (for harness.ts)                               */
/* ------------------------------------------------------------------ */

/**
 * Optional hook map used in `defineHarness({ hooks: ... })`.
 ...
 */
export type LegacyHookFn = (ctx: unknown, ...args: unknown[]) => void | Promise<void>;
