...
export class HookRegistry {
  ...
  /**
   * Register all hooks from a `HarnessHooks` map at once.
   * Used internally by the CLI when loading `harness.ts`.
   */
  registerAll(hooks: HarnessHooks, source: 'user' | 'plugin' = 'user'): void {
 ...
 * that don't use `defineHarness()`). For full projects with
 * `harness.ts`, the CLI creates and injects a dedicated instance.
 */
