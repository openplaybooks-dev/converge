 ...
 * with a glob-pattern-based approach that reads actual `harness.ts` config.
 *
...
const DEFAULT_CHECK_PATTERNS = [
  '.harness/checks/**/*.ts',
  '.harness/checks/**/*.check.ts',
];

const DEFAULT_PLAN_PATTERNS = [
  '.harness/plans/**/*.ts',
  '.harness/plans/**/*.plan.ts',
];
...
export class DiscoveryScanner {
  ...
  > {
    try {
      // Handle markdown files (skills, agents, TASK.md, EPIC.md, and any .md file).
  ...
  private async scanMarkdownTasks(
    ...
    const mdPatterns = [
      '.harness/epics/**/*/TASK.md',            // Direct task folders
      '.harness/epics/**/*/EPIC.md',            // EPIC.md alias for TASK.md
      '.harness/epics/**/*/tasks/**/TASK.md',   // Subtasks in tasks/ folder
      '.harness/epics/**/*/tasks/**/EPIC.md',   // Nested EPIC.md alias
      // Playbook API: tasks live under playbooks/{name}/tasks/
      '.harness/playbooks/*/tasks/**/TASK.md',
      '.harness/playbooks/*/tasks/**/EPIC.md',
    ];
    ...
    for (const mdPath of mdFiles) {
      ...
      try {
        ...
        // EPIC.md at the epic root is an epic definition, not a task
...
function inferTypeFromPath(filePath: string): DiscoveredFileType | null {
  ...
  if (filePath.endsWith('/EPIC.md')) return 'epic';
  return null;
}
