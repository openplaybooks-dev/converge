...
import { markAncestorsRunning, rollUpCompletion } from './ancestor-propagation.ts';
 ...
 */
async function loadParentFacts(ctx?: TaskContext): Promise<Record<string, any>> {
  if (!ctx?.parent) return {}; // No parent

  const parentCtx = ctx.parent;
  const factsFile = path.join(parentCtx.journalPath, 'attempts/wip/logs/facts.jsonl');
...
export async function executeTask(
  ...
  // through the normal execution path so they can execute their own logic.
  {
    ...
    if (!guardUnit.executorFn && !guardUnit.loopFn) {
      ...
      const hasChildren = existsSync(tasksSubDir) && await (async () => {
        const { readdir } = await import('node:fs/promises');
        const entries = await readdir(tasksSubDir, { withFileTypes: true });
        return entries.some(e => e.isDirectory() && /^\d{2,3}-/.test(e.name));
      })();
      ...
      if (hasChildren && !ctx.stepMode) {
        ...
        console.log(`   ⏩ Container task — skipping (children run independently)`);
        return {
          success: true,
          attemptNumber: 0,
          isWbsTask: false,
      ...
      if (guardUnit.wbsFn) {
        ...
        const journalTaskDir = ctx.journalPath
          ...
          try {
            ...
            if (raw.spawnCount > 0) {
              ...
              const anyChildExists = subtasks.length > 0 && subtasks.some(
                (t) => existsSync(path.join(parentDir, 'tasks', t.id, 'TASK.md'))
              );

              if (anyChildExists) {
                console.log(`   ⏩ WBS container — already seeded (${raw.spawnCount} tasks)`);
                return {
                  success: true,
                  attemptNumber: 0,
                  isWbsTask: true,
              ...
              console.log(`   ⚠️  WBS container — stale seed (${raw.spawnCount} tasks claimed but none on disk). Re-seeding...`);
        ...
        if (!ctx.stepMode) {
          ...
          console.log(`   ✅ WBS seeded ${result.spawnCount} children — next step will run them`);
          return {
            success: true,
            attemptNumber: 0,
            isWbsTask: true,
  ...
  if (isResumingInterrupted) {
    ...
    const currentAttempts = await checkpointMgr.getTaskAttemptCount(ctx.journalTaskId, ctx.epicId);
    attemptNumber = Math.max(currentAttempts, 1);
    console.log(`   ⚡ Resuming interrupted attempt #${attemptNumber} (wip/ preserved)`);
  } else {
    attemptNumber = await checkpointMgr.incrementTaskAttempt(ctx.journalTaskId, ctx.epicId);
  }
  ...
  if (!isResumingInterrupted) {
    const { symlink: fsSymlink, rm: fsRm, mkdir: fsMkdir, copyFile, lstat } = await import('node:fs/promises');
    ...
    if (existsSync(wipDir)) {
      ...
      try { wipStat = await lstat(wipDir); } catch { /* ignore */ }
      ...
      if (wipStat?.isSymbolicLink()) {
        ...
        if (attemptNumber > 1) {
          ...
          if (!existsSync(prevDir)) {
            ...
            for (let i = 0; i < 5; i++) {
              try { await fsRename(wipDir, prevDir); break; }
              catch (err: any) {
                if (err.code === 'EPERM' && i < 4) await new Promise(r => setTimeout(r, 300 * (i + 1)));
                else throw err;
              }
            }
            console.log(`   📦 Archived attempt #${attemptNumber - 1} → attempts/${prevPadded}/ (migrated)`);
          } else {
  ...
  const displayPath = normalizedWipDir.split('.harness/journal/')[1] ?? normalizedWipDir;
  console.log(`   Attempt #${attemptNumber} → journal/${displayPath}`);

  // Set environment variables for journal routing
  process.env.HARNESS_TASK_ATTEMPT = attemptPadded;
  process.env.HARNESS_TASK_ATTEMPT_DIR = attemptDir;
  ...
  let parsedDef: { description?: string; inputs?: string[]; outputs?: string[]; checks?: any[] } | undefined;
  ...
  try {
    ...
    if (existsSync(ctx.filePath)) {
      ...
      if (parsed) {
        ...
        console.log(`   ⚠️  parseTaskMd returned null/undefined for ${ctx.filePath}`);
      }
    } else {
    ...
    if (snapshotPaths.blocked) {
      ...
      const journalCtxForRepair = { epicId: ctx.epicId, taskId: ctx.journalTaskId };
      ...
      try {
        ...
        if (unblockResult.success && unblockResult.retryMode &&
          ...
          if (ctx.analyzeOnly) {
            ...
            console.log(`   Strategy : ${unblockResult.metadata?.solvedBy ?? 'unblock-coordinator'}`);
            console.log(`   Reason   : ${unblockResult.reason}`);
            if (producers.length > 0) {
              console.log(`   Producers to re-run:`);
              producers.forEach(p => console.log(`     → ${p.epicId}/${p.journalTaskId} (${p.filePath})`));
            }
            console.log(`\n   To execute: pnpm harness run --step\n`);
            delete process.env.HARNESS_TASK_ATTEMPT;
            delete process.env.HARNESS_TASK_ATTEMPT_DIR;
            return { success: true, attemptNumber, isWbsTask: false, durationMs: 0, isBlocking: false };
          }
          ...
          if (producers.length > 0) {
            ...
            if (!snapshotPaths.blocked) {
              ...
              console.log(`   ❌ Inputs still missing after producer re-run: ${snapshotPaths.blockedReason}`);
            }
          }
        } else if (unblockResult.success) {
          // Pattern repair or other non-backoff strategy succeeded — re-check inputs
          console.log(`   ✅ ${unblockResult.metadata?.solvedBy ?? 'UnblockStrategy'} succeeded — re-checking inputs`);
          snapshotPaths = await writeContextSnapshot(snapshotArgs);
          if (!snapshotPaths.blocked) {
            proceedToExecution = true;
          }
          ...
          delete process.env.HARNESS_TASK_ATTEMPT;
          delete process.env.HARNESS_TASK_ATTEMPT_DIR;
          return { success: false, attemptNumber, isWbsTask: false, durationMs: 0, isBlocking: false };
        }
      } catch (err: any) {
      ...
      if (!proceedToExecution) {
        ...
        await writeResultSnapshot(attemptDir, ctx.projectDir, 'blocked', 0, attemptNumber);

        // Write checkpoint so tree/gantt/status commands show correct failed state
        const blockedUnitCkpt = new UnitCheckpointManager(ctx.projectDir, 'task', ctx.epicId, ctx.journalTaskId);
        await blockedUnitCkpt.completeAttempt(attemptNumber, 'failed', blockedStartedAt);
        ...
        if (preloadedUnit?.context) {
          ...
          await rollUpCompletion(ctx.projectDir, ctx.epicId, ctx.journalTaskId, checkpointMgr);
        }
  ...
  try {
    ...
    console.warn(`   ⚠️  Events will still be written to file, but won't be formatted to console`);
  }
  ...
  const unitCkpt = new UnitCheckpointManager(ctx.projectDir, 'task', ctx.epicId, ctx.journalTaskId);
  ...
  const prevCurrentTask = (global as any).__HARNESS_CURRENT_TASK__;
  (global as any).__HARNESS_CURRENT_TASK__ = { journalTaskId: ctx.journalTaskId, unitCkpt };

  // Also update legacy TaskCheckpointManager for backward compatibility
  const taskCkpt = new TaskCheckpointManager(ctx.projectDir, ctx.epicId, ctx.journalTaskId);
  ...
  try {
    ...
    (global as any).__HARNESS_EVENT_WRITER__ = eventWriter;

    // Set session logger in global context for gap resolution to access
    if (sessionLogger) {
      (global as any).__HARNESS_SESSION_LOGGER__ = sessionLogger;
    }
    ...
    delete (global as any).__HARNESS_EVENT_WRITER__;
    delete (global as any).__HARNESS_SESSION_LOGGER__;

    // Restore previous task tracking (supports nested task execution)
    if (prevCurrentTask) {
      (global as any).__HARNESS_CURRENT_TASK__ = prevCurrentTask;
    } else {
      delete (global as any).__HARNESS_CURRENT_TASK__;
    }
  }
  ...
  if (success) {
    ...
    await writeResultSnapshot(wipDir, ctx.projectDir, 'success', durationMs, attemptNumber);

    // Enhance session logs with detailed tool calls from agentfn index.jsonl
    if (sessionLogger) {
      await enhanceSessionLogsFromAttempt(wipDir, ctx.journalTaskId, sessionLogger);
    }
    ...
    await writeResultSnapshot(wipDir, ctx.projectDir, 'failed', durationMs, attemptNumber);
    ...
    try {
      ...
      const isFailed = await checkpointMgr.isTaskFailed(ctx.journalTaskId, ctx.epicId);
      if (!isFailed) {
        console.warn(`⚠️  Warning: Task ${ctx.journalTaskId} not found as failed after markTaskFailed`);
      } else {
        console.log(`   ✓ Checkpoint updated: task marked as failed`);
      }
    } catch (err: any) {
      console.error(`❌ Failed to update checkpoint for failed task: ${err.message}`);
  ...
  if (existsSync(wipDir)) {
    ...
    const wipLstat = await import('node:fs/promises').then(m => m.lstat(wipDir).catch(() => null));
    ...
    if (existsSync(readmePath)) {
      ...
      const updated = content.replace('attempts/wip/', `attempts/${attemptPadded}/`);
