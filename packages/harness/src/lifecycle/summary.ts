...
export async function writeSummaryMd(
  ...
  await logTaskEvent(projectDir, epicId, taskId, 'SUMMARY_WRITTEN',
    `Summary updated (${content.length} chars) — status: ${input.status}`, {
      phase: input.phase, status: input.status, attempt: input.attempt,
    });
}
...
function renderSummary(input: SummaryInput): string {
  ...
  const statusEmoji = status === 'complete' ? '✓' : status === 'failed' ? '✗' : '⟳';
  const lines: string[] = [
    `# Task Summary: ${taskId}`,
  ...
  if (input.after?.diff) {
    ...
    lines.push('|----------|-------|');
    lines.push(`| Created  | ${diff.created.length > 0 ? diff.created.join(', ') : '_(none)_'} |`);
    lines.push(`| Modified | ${diff.modified.length > 0 ? diff.modified.join(', ') : '_(none)_'} |`);
    lines.push(`| Deleted  | ${diff.deleted.length > 0 ? diff.deleted.join(', ') : '_(none)_'} |`);
  } else {
  ...
  if (input.after?.checkResults && input.after.checkResults.length > 0) {
    ...
    for (const cr of input.after.checkResults) {
      ...
      const output = `${cr.stdout}\n${cr.stderr}`.trim().slice(0, 100).replace(/\n/g, ' ');
      lines.push(`| ${cr.id} | ${cr.description} | ${result} | \`${output}\` |`);
    }
  } else {
  ...
  if (input.diagnosis) {
    ...
    lines.push(`**Error**: ${d.errorClass} | **Automatable**: ${d.automatable ? 'YES' : 'NO'} | **Confidence**: ${(d.confidenceScore * 100).toFixed(0)}%`);
  ...
  if (input.correction && input.correction.history.length > 0) {
    ...
    for (const ca of input.correction.history) {
      ...
      lines.push(`| ${ca.attemptNumber} | ${ca.fixDescription.slice(0, 60)} | ${passed}/${total} | ${ca.resolved ? '✓' : '✗'} |`);
    }
  } else {
  ...
  if (input.before?.contextDoc && input.before.contextDoc.trim() && !input.before.contextDoc.includes('_(No prior')) {
  ...
  if (status === 'complete') {
    ...
    if (!input.diagnosis.automatable) {
      lines.push(`**Manual intervention required.** ${input.diagnosis.suggestedFix}`);
    } else if (input.correction && !input.correction.resolved) {
      lines.push(`Inner correction loop exhausted (${input.correction.attemptsUsed}/${input.correction.maxAttempts} attempts). Falling through to external repair pipeline.`);
      lines.push('');
  ...
  lines.push('');
  lines.push(`---`);
  lines.push(`_Generated by Harness lifecycle — ${now}_`);
