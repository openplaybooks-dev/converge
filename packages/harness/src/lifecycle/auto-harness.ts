...
import type { AutoHarnessPolicy, EnrichField } from '../config/skill-definition.ts';
 ...
 * @param policyInput  The value of the `auto-harness` frontmatter field
 * @param projectDir   Working directory for script execution / AI tool calls
 */
export async function runAutoHarnessPhase(
  taskMdPath: string,
  def: TaskMdDef,
  body: string,
  policyInput: boolean | AutoHarnessPolicy,
  projectDir: string,
): Promise<AutoHarnessResult> {
...
function normalizePolicy(input: boolean | AutoHarnessPolicy): AutoHarnessPolicy {
  if (input === true) {
