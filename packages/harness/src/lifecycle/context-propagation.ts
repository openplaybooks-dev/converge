...
import { getJournalStructure, getTaskAfterDir, getTaskCorrectionsDir } from '../journal/structure.ts';
...
export async function loadAncestorContext(
  ...
  for (let d = 1; d <= Math.min(depth, segments.length - 1); d++) {
    ...
    const summary = await loadAncestorSummary(projectDir, epicId, ancestorId, d);
  ...
  if (segments.length > 1) {
    ...
    const siblings = await findCompletedSiblings(projectDir, epicId, parentId, segments[segments.length - 1]);
    siblingOutcomes.push(...siblings.slice(0, 10));
  }

  // Load correction history from current task (prior compressed attempts)
  const correctionHistory = await loadCorrectionHistory(projectDir, epicId, taskId);
...
async function loadAncestorSummary(
  ...
  try {
    const outcome = JSON.parse(await readFile(outcomePath, 'utf8')) as TaskOutcomeJson;
    ...
    try {
      const st = JSON.parse(await readFile(statusPath, 'utf8')) as { status?: string };
...
async function findCompletedSiblings(
  ...
  let entries: string[] = [];
  try { entries = await readdir(tasksDir); } catch { return []; }
  ...
  for (const sibling of candidates) {
    ...
    } catch { /* sibling hasn't completed yet */ }
  }
