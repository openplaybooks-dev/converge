...
export async function writeContextSnapshot(
  ...
  const {
    projectDir, taskId, attemptDir,
    description, inputs, outputs, checks, skillBody, attemptNumber,
  } = params;
  ...
  for (const pattern of (inputs ?? [])) {
    try {
      ...
      resolvedInputs.push({ pattern, count: matches.length, samples: matches.slice(0, 10).sort() });
    } catch {
  ...
  await writeFile(needsJson, JSON.stringify({
  ...
  await writeFile(checkJson, JSON.stringify({
    taskId,
    checks: (checks ?? []).map(c => ({
      id: c.id,
  ...
  const reqResultLines: string[] = [`# NEEDS.result.md — Attempt ${attemptNumber}`, ''];

  if (resolvedInputs.length > 0) {
    reqResultLines.push('## Input State', '');
    for (const inp of resolvedInputs) {
      if (inp.count === 0) {
        reqResultLines.push(`### \`${inp.pattern}\``, '⛔ **NO FILES FOUND**', '');
      } else {
        reqResultLines.push(
          `### \`${inp.pattern}\``,
          `✓ **${inp.count} file(s)**`,
  ...
  for (const c of (checks ?? [])) {
    checkLines.push('', `## ${c.id}`, `**Description**: ${c.description ?? c.id}`);
  ...
  return {
    needsMd, needsResultMd, taskMd, checkMd, needsJson, checkJson,
    ...
    relDir, hasLearn, hasErrorContext, hasInterrupted, blocked, blockedReason,
...
async function writeTaskReadme(
  ...
  const checkSnippet = checks.length > 0
    ? checks.map(c => `  ${c.cmd ?? `# ${c.id}`}`).join('\n')
    : '  # (no checks defined)';

  await writeFile(join(taskJournalDir, 'README.md'), [
    `# Task Journal: ${taskId}`,
    '',
    ...
    '',
    '```bash',
    'pnpm harness run --step   # run next pending task',
    'pnpm harness run          # run all remaining tasks',
    '```',
    '',
