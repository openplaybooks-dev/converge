 ...
 *
 * Path routing:
 *   With playbook context (HARNESS_PLAYBOOK=X):
...
function resolveFirstSegmentPrefix(
  ...
  const sourceEpicDir = join(projectDir, '.harness', 'epics', epicId);
  if (existsSync(join(sourceEpicDir, 'tasks', segment))) return ['tasks'];
  if (existsSync(join(sourceEpicDir, segment))) return [];
  // Also check playbook source: .harness/playbooks/{epicId}/tasks/{segment}
  const sourcePlaybookDir = join(projectDir, '.harness', 'playbooks', epicId, 'tasks');
...
export interface JournalStructure {
  ...
  /**
   * Attempt-level directory (if task + attempt number provided or HARNESS_TASK_ATTEMPT env var set).
   */
  attempt?: string;
}
...
function getPlaybookContextFromEnv(): PlaybookContext | undefined {
  const playbook = process.env.HARNESS_PLAYBOOK;
  if (playbook) {
    return { playbook };
  }
  return undefined;
...
function getActiveAttemptNumber(): string | undefined {
  return process.env.HARNESS_TASK_ATTEMPT;
}
...
export function getTaskAttemptDir(
  ...
  const padded = typeof attemptNumber === 'number'
...
export function getJournalStructure(
  ...
  const root = join(projectDir, '.harness', 'journal');
  ...
  if (epicId) {
    ...
    if (taskId) {
      ...
      if (skipEpicSegment) {
        const prefix = resolveFirstSegmentPrefix(structure.epic!, projectDir, epicId, segments[0]);
      ...
      const activeAttemptDir = process.env.HARNESS_TASK_ATTEMPT_DIR;
      if (activeAttemptDir) {
 ...
 */
export function getTaskBeforeDir(projectDir: string, epicId: string, taskId: string): string {
  const s = getJournalStructure(projectDir, epicId, taskId);
  return join(s.attempt ?? s.task!, 'before');
}

export function getTaskAfterDir(projectDir: string, epicId: string, taskId: string): string {
  const s = getJournalStructure(projectDir, epicId, taskId);
  return join(s.attempt ?? s.task!, 'after');
}

/** @deprecated Corrections no longer exist — each retry is a new attempt. */
export function getTaskCorrectionsDir(projectDir: string, epicId: string, taskId: string): string {
  const s = getJournalStructure(projectDir, epicId, taskId);
  ...
  const breadcrumbs: Breadcrumb[] = [
    {
      level: 'project',
      name: projectName,
      path: join(projectDir, '.harness', 'journal', 'project'),
    },
  ];
