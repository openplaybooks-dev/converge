 ...
 * Harness Client
 *
 * SDK for external WBS scripts to interact with the harness framework.
 ...
 *
 * const screens = JSON.parse(harness.readFile('.stitch/screens.json'));
...
import type { HarnessClientContext, SpawnedTask } from './types.ts';

export class HarnessClient {
  private context: HarnessClientContext;
  ...
  /**
   * Get the harness context (project dir, vars, etc.)
   */
  getContext(): Readonly<HarnessClientContext> {
    return this.context;
  }
   ...
   */
  private readContext(): HarnessClientContext {
    // Try HARNESS_CONTEXT_JSON first (comprehensive context)
    const contextJson = process.env.HARNESS_CONTEXT_JSON;
    if (contextJson) {
      try {
    ...
    // Fall back to individual HARNESS_VAR_* env vars
    const vars: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(process.env)) {
      if (key.startsWith('HARNESS_VAR_') && value !== undefined) {
        const varName = key.slice('HARNESS_VAR_'.length).toLowerCase();
        vars[varName] = value;
      }
    }

    return {
      projectDir: process.env.HARNESS_PROJECT_DIR ?? process.cwd(),
      epicId: process.env.HARNESS_EPIC_ID,
      taskId: process.env.HARNESS_TASK_ID,
      vars,
    };
  }
}
