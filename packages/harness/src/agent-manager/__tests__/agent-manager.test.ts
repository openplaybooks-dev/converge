...
describe('AgentManager', () => {
  ...
  const stateFilePath = join(homedir(), '.harness', 'agent-registry.json');
  ...
  it('tracks harness metadata', async () => {
    const proc = spawn('sleep', ['1']);
    ...
    const info = manager.getProcess(proc.pid!);
    expect(info?.harnessMetadata.epicId).toBe('epic-1');
    expect(info?.harnessMetadata.taskId).toBe('task-1');
    expect(info?.harnessMetadata.phase).toBe('execution');
