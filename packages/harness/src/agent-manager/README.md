...
Internal process lifecycle management for Claude CLI agents spawned by the harness framework.
...
### Harness Integration
- Track harness metadata: `taskId`, `epicId`, `phase`, `strategyType`
...
The harness automatically registers processes via the `onProcessSpawned` hook:
...
Cleanup handlers are automatically registered at harness startup:
...
Process state is persisted to `~/.harness/agent-registry.json`:
  ...
  "processes": [
    {
      ...
      "status": "running",
      "logPath": "/project/.harness/logs/claudefn/2026-04-02T12-00-00-000Z_abc.log",
      "parentPid": 54321,
      "harnessMetadata": {
        "projectDir": "/project",
        "epicId": "epic-1",
...
This hook is called immediately after spawning, allowing the harness to register the process before it starts executing.

## Design Decisions

### Why Harness Package (Not claudefn)?

1. **Separation of concerns**: claudefn is a low-level spawn utility; harness is the orchestration layer
2. **Metadata richness**: Harness has taskId, epicId, phase context that claudefn doesn't know about
...
5. **Cleanup scope**: Harness can kill all agents for a task/epic/session; claudefn has no concept of tasks
