 ...
 * WBS Executor
 *
 * Executes a wbs() function, providing WbsContext with ctx.spawn().
 *
 * Spawn behavior:
...
export interface WbsExecutorResult {
  spawnCount: number;
  durationMs: number;
  error?: string;
}
...
export class WbsExecutor {
  ...
  async run(wbsFn: WbsFn, attemptNumber: number = 1): Promise<WbsExecutorResult> {
    ...
    try {
      ...
      // Write wbs.json to the parent task's journal directory so the harness can
...
/**
 * Resolve a WbsSpawnTarget to a TaskMdShape for writing.
 *
 * Detection order:
...
export async function resolveWbsTarget(
  ...
  opts: WbsSpawnOptions | undefined,
  ctx: WbsContext,
): Promise<TaskMdShape> {
  // 1. RawMarkdown
