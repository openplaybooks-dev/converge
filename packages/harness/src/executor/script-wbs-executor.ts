...
async function runAsChildProcess(
  ...
  const env: Record<string, string> = {
    ...
    HARNESS_PROJECT_DIR: ctx.projectDir,
    HARNESS_CONTEXT_JSON: JSON.stringify({
  ...
  for (const [key, value] of Object.entries(ctx.vars)) {
    if (value !== undefined && value !== null) {
      env[`HARNESS_VAR_${key.toUpperCase()}`] = String(value);
    }
  }
...
function buildGenerationPrompt(userPrompt: string, ctx: WbsContext): string {
  ...
  return `You are generating a WBS (Work Breakdown Structure) script for the harness task system.
