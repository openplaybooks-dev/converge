...
import type { AgentFnResult } from '@crew/agentfn';
...
export class SpawnRunner {
  ...
  async executeSpawnPath(
    ...
    //   absSkillPath = /project/.harness/skills/stitch-loop/SKILL.md
    //   skillDir     = /project/.harness/skills/stitch-loop
    ...
    try {
      const harnessDir = join(this.projectDir, '.harness');
  ...
  /*  Resolves each skill name → .harness/skills/{name}/SKILL.md     */
  ...
  async executeSpawnBuilder(
    ...
    for (let i = 0; i < skills.length; i++) {
      ...
      const skillPath = `.harness/skills/${skills[i]}/SKILL.md`;
 ...
 * Only serializes JSON-safe fields; skips executorFn, loopFn, harnessConfig.
 */
export async function writeInlineDefinitionToFile(
  ...
  const lines: string[] = ["import { taskDef } from 'harness';", 'export default taskDef()'];
