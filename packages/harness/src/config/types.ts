 ...
 *
 * The `HarnessConfig` type is the shape of `.harness/PROJECT.md` frontmatter.
...
import type { HarnessHooks } from '../hooks/types.ts';
...
/**
 * Controls where Harness looks for task/epic/check/plan files.
 * All values are glob patterns resolved relative to the project dir.
 *
 * @example
 * discovery: {
 *   tasks: ['.harness/tasks/**\/*.ts'],
 *   epics: ['.harness/epics/**\/*.ts'],
 *   watch: true,
 * }
 */
export interface DiscoveryConfig {
   ...
   * Defaults: ['.harness/tasks/**\/*.ts', '.harness/tasks/**\/*.task.ts']
   */
  tasks?: string[];

  /**
   * Glob patterns for epic files.
   * Defaults: ['.harness/epics/**\/*.ts', '.harness/epics/**\/*.epic.ts']
   */
  epics?: string[];

  /**
   * Glob patterns for check function files.
   * Defaults: ['.harness/checks/**\/*.ts', '.harness/checks/**\/*.check.ts']
   */
  checks?: string[];

  /**
   * Glob patterns for plan function files.
   * Defaults: ['.harness/plans/**\/*.ts']
   */
  plans?: string[];
   ...
   *   in `.harness/tasks/` are blocked with a warning.
...
export interface RuntimeConfig {
   ...
   * When enabled, harness can detect and fix its own bugs autonomously.
   *
   * @example
   * selfRepair: {
   *   enabled: true,
   *   maxAttempts: 3,
   *   allowedFiles: ['packages/harness/src/**\/*.ts']
   * }
   */
  selfRepair?: {
    ...
    /** Only repair these files (glob patterns). Default: ['packages/harness/src/**\/*.ts'] */
    allowedFiles?: string[];
    /** Never modify these files (glob patterns). Default: ['packages/harness/src/self-repair/**\/*.ts'] */
...
/**
 * Root configuration object for `.harness/PROJECT.md`.
 *
 * @example
 * ---
 * name: My App
...
export interface HarnessConfig {
   ...
   * Defaults to the directory containing `.harness/PROJECT.md`.
   */
  dir?: string;

  /**
   * Auto-discovery configuration — where to find tasks, epics, checks, etc.
   * When set, Harness scans these paths and registers found functions
   ...
   *
   * @example
   * skills: { planning: '.harness/skills/planning.md' }
   */
