 ...
 *
 * The harness V2 architecture uses a single Unit class for all levels
...
export interface TaskDefinition {
  ...
  /**
   * Work Breakdown Structure function. When present, the harness calls this
  ...
  /**
   * Plan mode configuration. When present, the harness runs a planning phase
  ...
  /**
   * AI-generated task configuration. When present, the harness uses AI to
  ...
  /**
   * Loop execution function. When present, the harness runs this function
  ...
  /**
   * Executor function. When present, the harness runs this function once
  ...
  /**
   * Harness configuration. Controls how the executor is wrapped.
   * Set via .harness(opts?) on the builder.
   */
   ...
   * - .harness/journal/tasks/{epic}/tasks/{task}/logs/facts.jsonl (append-only log)
   * - .harness/journal/tasks/{epic}/tasks/{task}/attempts/{N}/data/facts.json (snapshot)
   *
   * @example
   * ```ts
   * .factsApi(async (ctx) => {
 ...
 *   .inputs(['data/**\/*.tsv'])
 *   .outputs([
 *     '.harness/epics/01-data-analysis/001-analyze.ts',
 *     '.harness/epics/01-data-analysis/002-model.ts',
 *   ])
 ...
 *   .outputs(['.harness/qa-reports/responsive-check.json'])
...
/**
 * Check if a TaskDefinition uses the executor/harness execution primitive.
 */
 ...
 * When set, the harness runs a planning phase before execution:
 ...
 * The harness will invoke Claude to produce a SKILL.md or task.ts
...
export interface InlineTaskTarget {
   ...
   * Example: '.harness/epics/02-ux/003-screens/gen-homepage.ts'
   */
  writeToPath?: string;
}
 ...
 * The harness checks `.claude/settings.json` before running and fails
...
export function isBuilderTarget(t: LoopSpawnTarget): t is TaskDefinitionBuilder {
  ...
  // (task.ts loads harness via package resolution; internals load directly from src).
 ...
 *     await ctx.spawn('.harness/skills/stitch-loop/SKILL.md', { label: 'stitch-loop' });
 *   }
 * })
 * ```
 */
export interface ExecutorContext {
  ...
  task: {
    id: string;
    title?: string;
    /** Harness iteration (1-based). Always 1 in manual mode. */
    iteration: number;
  };
  ai: {
    /**
 ...
 * Returns void — the harness decides when to stop.
 */
...
/** Result returned by a single executor run inside the harness */
...
/**
 * Context passed to a custom or AI-generated harness function.
 * The harness fn controls when and how often the executor runs.
 */
...
/** Return value of a harness function */
...
/**
 * A harness function controls convergence.
 ...
 * })
 */
export type HarnessFn = (ctx: HarnessContext) => Promise<HarnessDecision>;

/**
 * Configuration for .harness() on the builder.
 *
 * Three unified modes — all reduce to a single HarnessFn at runtime:
 * - 'default' : built-in policy — runs executor once, checks outputs + .check() validators
 * - 'custom'  : user-provided HarnessFn (set via .harness(fn) shorthand)
 * - 'ai'      : AI generates __harness__.ts in the task folder; loaded and run like custom
 */
export interface HarnessConfig {
  mode?: 'default' | 'custom' | 'ai';
  /** Custom harness function — set automatically when passing fn to .harness(fn) */
  fn?: HarnessFn;
  /** Outer loop cap (default 20) — harness fn is called up to this many times */
  maxIterations?: number;
  /** Per-run timeout in ms */
  timeoutMs?: number;
  /** AI mode: injected into the generation prompt to guide what the harness checks */
...
export class TaskDefinitionBuilder {
   ...
   * Before executing, the harness generates `plan.md` in the task journal
   ...
   *   await ctx.loop.spawn('.harness/skills/stitch-loop/SKILL.md');
   ...
   * Pair with .harness(opts?) to configure how it is wrapped.
   *
   * @example
   * ```ts
   * .executor(async (ctx) => {
   ...
   *   });
   *   while (!(await checkDone()).done) {
   *     await ctx.spawn('.harness/skills/stitch-loop/SKILL.md');
   *   }
   * })
   * ```
   */
  ...
  /**
   * Configure the harness wrapper for this executor. Three forms:
   *
   * - .harness()       → default mode: checks outputs + .check() validators
   * - .harness(fn)     → custom mode: your HarnessFn controls convergence
   * - .harness({...})  → explicit config: set mode, maxIterations, etc.
   *   - { mode: 'default' } — same as .harness()
   *   - { mode: 'custom', fn } — same as .harness(fn)
   *   - { mode: 'ai' }      — AI generates __harness__.ts, runs it like custom
   */
  harness(arg?: HarnessFn | HarnessConfig): this {
    if (!arg) {
      this.def.harnessConfig = { mode: 'default' };
    } else if (typeof arg === 'function') {
      this.def.harnessConfig = { mode: 'custom', fn: arg };
    } else {
      this.def.harnessConfig = { mode: 'default', ...arg };
    }
    return this;
  }
   ...
   *
   * @example
   * await ctx.loop.spawn(taskDef().fromPath('.harness/epics/02-ux/003/task.ts'))
   */
