 ...
 *
 * Discovers and loads `.harness/PROJECT.md` or `.harness/project.yaml` from the filesystem,
...
import type { HarnessHooks, HookEvent } from '../hooks/types.ts';

/** Legacy config file name inside .harness/ (with YAML frontmatter) */
const LEGACY_CONFIG_NAME = '.harness/PROJECT.md';
/** V2 storage config file name inside .harness/ (pure YAML) */
const V2_CONFIG_NAME = '.harness/project.yaml';
...
/**
 * Find `.harness/PROJECT.md` or `.harness/project.yaml` by walking up from `startDir`.
 ...
 *
 * @example
 * const result = await findHarnessConfig(process.cwd());
 * // → { path: '/my-project/.harness/PROJECT.md', type: 'PROJECT.md' } or null
 */
export async function findHarnessConfig(startDir: string): Promise<{ path: string; type: ConfigFileType } | null> {
 ...
 *
 * Reads YAML frontmatter and maps it to HarnessConfig.
 ...
 */
async function loadProjectMdConfig(configPath: string): Promise<HarnessConfig> {
  let raw: string;
  try {
  ...
  const projectDir = dirname(dirname(configPath)); // .harness/PROJECT.md → project root
 ...
 *
 * Reads pure YAML and maps it to HarnessConfig.
 * Hook strings are converted to functions via convertScriptHooks().
 *
 * @throws If the file cannot be read or the YAML is invalid.
 */
async function loadProjectYamlConfig(configPath: string): Promise<HarnessConfig> {
  let raw: string;
  try {
    raw = await readFile(configPath, 'utf-8');
  } catch (err: any) {
  ...
  const projectDir = dirname(dirname(configPath)); // .harness/project.yaml → project root

  return buildConfigFromData(data, projectDir);
}

/**
 * Build HarnessConfig from parsed data.
 */
function buildConfigFromData(
  ...
  const config: HarnessConfig = {
    ...
    discovery: data.discovery as HarnessConfig['discovery'],
    plugins: data.plugins as HarnessConfig['plugins'],
    variables: data.variables as HarnessConfig['variables'],
    runtime: data.runtime as HarnessConfig['runtime'],
    agents: data.agents as HarnessConfig['agents'],
    skills: data.skills as HarnessConfig['skills'],
    ai: data.ai as HarnessConfig['ai'],
  };
 ...
 * @throws If the file cannot be read or the YAML is invalid.
 */
export async function loadHarnessConfig(configPath: string, type?: ConfigFileType): Promise<HarnessConfig> {
 ...
 * typed HarnessHooks (event → async function).
 *
...
/**
 * Find and load the harness config from `startDir` (or its ancestors).
 *
 * Returns `null` if no config file is found — the caller should fall
 * back to default behavior.
 *
 * @example
 * const config = await resolveHarnessConfig(process.cwd());
 * if (config) {
...
export async function resolveHarnessConfig(
  startDir: string
): Promise<{ config: HarnessConfig; configPath: string; type: ConfigFileType } | null> {
  const result = await findHarnessConfig(startDir);
  if (!result) return null;

  const config = await loadHarnessConfig(result.path, result.type);
