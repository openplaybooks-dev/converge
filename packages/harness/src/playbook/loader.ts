 ...
 * Playbook Loader
 *
 * Discovers playbook template directories from .harness/playbooks/.
...
function getProjectPlaybooksDir(projectDir: string): string {
  return join(projectDir, '.harness', 'playbooks');
}
 ...
 * Searches skill playbooks (builtins) and .harness/playbooks/ (project).
