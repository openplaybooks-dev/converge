 ...
 * Playbook Executor
 *
 * Copies tasks/ from the playbook template into .harness/epics/{epicId}/
 ...
 * Copies tasks/ with variable substitution into .harness/epics/{epicId}/.
 */
export async function generateEpicFromPlaybook(
  playbook: ResolvedPlaybook,
  projectDir: string,
): Promise<string> {
  const epicDir = join(projectDir, '.harness', 'epics', playbook.epicId);
...
/**
 * Install a playbook template into .harness/playbooks/{name}/.
 ...
 * the epicId so the harness scanner can derive a proper task ID.
 *
...
export async function installPlaybook(
  ...
  const playbookDir = join(projectDir, '.harness', 'playbooks', playbook.def.name);
  ...
  if (existsSync(templateTasksDir)) {
    ...
    if (hasRootTask && !hasSubdirTasks) {
      ...
      // so the harness scanner can derive a proper task ID.
