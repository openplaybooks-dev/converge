...
class TaskDefBuilderImpl implements ITaskDefBuilder {
  ...
  autoHarness(config: import('./types.ts').AutoHarnessConfig): this {
    this._vars.autoHarness = config;
    return this;
  }
  ...
  build(): TaskConfig {
    ...
    // AUTO-ENABLE AutoHarness if not explicitly configured
    if (!this._vars.autoHarness) {
      this._vars.autoHarness = {
        enabled: true,
        from: 'task-prompt',
        refinable: true,
        maxRefinements: 3,
