...
export interface YieldsDeclarative {
   ...
   * e.g. ".harness/epics/02-epic/003-generate-all-screens"
   ...
   * e.g. ".harness/epics/02-epic/003-generate-all-screens/000-screen-{slug}.ts.tpl"
...
/* ------------------------------------------------------------------ */
/*  AutoHarness API (Phase 2)                                         */
/* ------------------------------------------------------------------ */

/**
 * AutoHarness configuration for task validation
 */
export interface AutoHarnessConfig {
  ...
  /** Cache harness across similar tasks */
  cache?: boolean;
...
export interface HarnessResult {
  ...
  /** Harness execution time (ms) */
  duration: number;

  /** Whether harness was refined */
  refined?: boolean;
...
export interface TaskDefBuilder {
  ...
  /** Configure AutoHarness validation (Phase 2) */
  autoHarness(config: AutoHarnessConfig): this;
   ...
   *
   * When `runtime.milestone` is set in harness.ts, only tasks whose milestone
