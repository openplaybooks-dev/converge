 ...
 */
export async function discoverEpicChildren(unit: Unit, gaps: Gap[], forceAll: boolean): Promise<Unit[]> {
  ...
  for (const taskDir of scanDirs) {
    try {
      ...
      for (const dir of numberedDirs) {
        ...
        for (const candidate of ['TASK.md', 'EPIC.md']) {
 ...
 */
export async function discoverChildren(unit: Unit, gaps: Gap[], forceAll: boolean = false): Promise<Unit[]> {
