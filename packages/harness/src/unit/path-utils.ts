...
export function extractJournalTaskId(taskPath: string): string {
  ...
  // Try playbook path first: .harness/playbooks/{name}/tasks/{taskId}/TASK.md
...
export function extractEpicId(taskPath: string): string {
  ...
  // Try playbook path: .harness/playbooks/{name}/tasks/...
...
export function extractEpicDir(taskPath: string): string {
  ...
  // Try playbook path: .harness/playbooks/{name}/tasks/...
  // The "epic dir" equivalent is .harness/playbooks/{name}/tasks/
  const playbooksIndex = parts.indexOf('playbooks');
  if (playbooksIndex !== -1 && playbooksIndex + 2 < parts.length && parts[playbooksIndex + 2] === 'tasks') {
 ...
 *   .harness/epics/03-app/002-pages
 *   → .harness/journal/tasks/03-app/002-pages
 *
 *   .harness/epics/03-app/002-pages/tasks/002-001-home
 *   → .harness/journal/tasks/03-app/002-pages/tasks/002-001-home
 *
 *   .harness/epics/03-app/002-pages/tasks/002-001-home/TASK.md
 *   → .harness/journal/tasks/03-app/002-pages/tasks/002-001-home
 *
 * @param taskPath - Absolute or relative path to task file or directory
...
export function constructJournalPath(taskPath: string): string {
  ...
  const playbook = process.env.HARNESS_PLAYBOOK ?? 'default';
