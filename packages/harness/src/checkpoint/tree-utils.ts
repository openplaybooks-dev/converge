 ...
 *
 * Walks the entire .harness/epics directory and discovers task trees.
 */
export async function discoverAllTaskHierarchies(harnessDir: string): Promise<Map<string, TaskTree>> {
  const epicsDir = path.join(harnessDir, 'epics');
 ...
 *
 * Examples:
 *   .harness/epics/01-epic/003-task/TASK.md → "003-task"
 *   .harness/epics/01-epic/003-task/tasks/002-subtask/TASK.md → "002-subtask"
 */
