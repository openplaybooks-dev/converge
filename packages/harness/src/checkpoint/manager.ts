 ...
 * Checkpoint Manager
 *
 * Saves and loads harness execution state to enable resumable autonomous runs.
...
export class CheckpointManager {
  ...
  constructor(projectDir: string) {
    ...
    this.checkpointFile = path.join(projectDir, '.harness/journal/.checkpoint.json');
  ...
  async reconcileTask(taskId: string, reason: string, explicitEpicId?: string): Promise<void> {
    ...
    // inconsistent HARNESS_PLAYBOOK context during original execution.
