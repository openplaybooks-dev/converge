...
export const syntaxRules: ValidationRule[] = [
    ...
    check: ({ shape, filePath }) => {
      if (shape.executor && shape.wbs) {
        return [{
          ...
          layer: 'syntax',
          severity: 'error',
          message: 'Both `executor` and `wbs` are declared — they are mutually exclusive',
          path: filePath,
...

  {
    ...
    layer: 'syntax',
    severity: 'warning',
    description: 'script/function executor with skills array — skills will not be used',
    check: ({ shape, filePath }) => {
      if (
        shape.executor &&
        (shape.executor.type === 'script' || shape.executor.type === 'function') &&
        shape.skills?.length
      ) {
        return [{
...

  {
    ...
    check: ({ shape, filePath }) => {
      if (shape.blocking === true && (!shape.outputs || shape.outputs.length === 0)) {
        return [{
          ruleId: 'blocking-without-outputs',
          layer: 'syntax',
          severity: 'warning',
          message: '`blocking: true` but no outputs declared — nothing to block on',
          path: filePath,
          field: 'blocking',
...

  {
    ...
    check: ({ shape, rawFrontmatter, filePath }) => {
      if (rawFrontmatter.skills !== undefined && Array.isArray(rawFrontmatter.skills) && rawFrontmatter.skills.length === 0) {
        return [{
    ...
    check: ({ shape, filePath }) => {
      if (shape.body && shape.prompt) {
        return [{
          ...
          layer: 'syntax',
          severity: 'info',
          message: 'Both `body` and `prompt` are set — `prompt` is an alias for `body`',
          path: filePath,
