...
export async function evaluateCommand(options: EvaluateOptions): Promise<void> {
  ...
  // Find project directory via harness config
  ...
  const goalPatterns = [
    '**/.harness/goals/*/GOAL.md',
    '**/.harness/playbooks/*/goals/*/GOAL.md',
  ];
  ...
  if (goalFiles.length === 0) {
    console.log('\nNo goals found. Create GOAL.md files in .harness/goals/ or .harness/playbooks/*/goals/ to define project goals.\n');
    return;
  }
  ...
  if (isDry) {
    ...
    for (const r of evalResult.needsPlanning) {
      const implSkills = (r.goal.skills ?? []).filter(s => s !== 'harness-planning');
