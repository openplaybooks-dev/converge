 ...
 * Order matches what `harness run --step` actually executes.
...
export async function buildTaskTree(
  ...
  for (const epic of sortedEpics) {
    ...
    for (const task of epicTasks) {
      ...
      try {
        ...
        if (process.env.HARNESS_DEBUG_DEPS === 'true') {
  ...
  for (const task of orphans) {
    ...
    // Try to infer epic from path pattern .harness/epics/{epicId}/...
    // This handles dynamically spawned WBS subtasks that may not be matched by the epic filter above
    const epicMatch = task.filePath.match(/\.harness[/\\]epics[/\\]([^/\\]+)/);
    ...
    try {
      ...
      if (process.env.HARNESS_DEBUG_DEPS === 'true' && dependencies) {
 ...
 *   - CheckpointManager.completedTasks  (.harness/journal/.checkpoint.json)
 *   - journal status.json               (.harness/journal/tasks/{epicId}/tasks/{taskId}/status.json)
 */
...
export async function getTaskStates(
  ...
  // When artifacts exist outside the harness flow (e.g. batch operations),
  ...
  for (const epicId of sortedEpicIds) {
    ...
    for (const task of sortedTasks) {
      ...
      if (process.env.HARNESS_DEBUG_DEPS === 'true' && failed.has(task.journalTaskId)) {
