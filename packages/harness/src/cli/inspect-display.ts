 ...
 */
function renderGroupedEvents(groups: EventGroup[], options: InspectOptions): void {
  for (const group of groups) {
    if (group.type === 'event') {
      ...
      for (const event of group.events) {
        ...
        console.log(`${color(timestamp, COLORS.GRAY)} ${icon} ${event.eventType}`);
      }
 ...
 * Render task group
 */
function renderTask(task: EventGroup, prefix: string, options: InspectOptions): void {
  ...
  if (task.epicId && task.taskId) {
    ...
    const leafId = task.taskId.startsWith(`${task.epicId}/`)
      ...
      : task.taskId;
    const attemptsDir = join(getEpicsDir(projectRoot), task.epicId, 'tasks', leafId, 'attempts');
    if (existsSync(attemptsDir)) {
      try {
        stallRetries = readdirSync(attemptsDir).filter(e => /^\d+$/.test(e)).length;
      } catch { /* ignore */ }
    }
  }
  const stallSuffix = stallRetries > 0 ? color(` (${stallRetries} stall retries)`, COLORS.GRAY) : '';
  const attempt = task.attempt ? color(`#${task.attempt}`, COLORS.GRAY) + stallSuffix : '';
  ...
  const durationText = task.duration ? color(formatDuration(task.duration), COLORS.GRAY) : '';

  console.log(`${prefix} ${resultIcon} ${taskName} ${attempt} ${resultText} ${durationText}`);
  ...
  if (!options.compact && task.epicId && task.taskId) {
    ...
    const leafTaskId = task.taskId.startsWith(`${task.epicId}/`)
      ? task.taskId.slice(task.epicId.length + 1)
      : task.taskId;
    const journalPath = `.harness/journal/default/tasks/${task.epicId}/tasks/${leafTaskId}/attempts/${attemptNum}/`;
    console.log(`${indent}   ${color('📁', COLORS.BLUE)} ${color(journalPath, COLORS.GRAY)}`);
  }

  // Show important sub-events (gaps, strategies, convergence)
  if (!options.compact) {
    const indent = prefix === '└─' ? '   ' : '│  ';
    const importantEvents = task.events.filter(e =>
  ...
  if (options.verbose) {
    const indent = prefix === '└─' ? '   ' : '│  ';
    const toolEvents = task.events.filter(e =>
    ...
    if (toolEvents.length > 0) {
      console.log(`${indent}   ${color('Detailed execution log:', COLORS.GRAY)}`);
      for (const event of toolEvents) {
        ...
        if (event.eventType === 'TOOL_CALL') {
          console.log(`${indent}      ${color(time, COLORS.GRAY)} ${icon} ${msg}`);
        } else if (event.eventType === 'TOOL_RESULT') {
          const success = event.metadata?.success ? '✓' : '✗';
          const size = event.metadata?.size ? ` (${formatBytes(event.metadata.size as number)})` : '';
          console.log(`${indent}      ${color(time, COLORS.GRAY)} ${success}${size}`);
        } else if (event.eventType === 'AI_OUTPUT') {
          // Show truncated AI output
          const truncated = msg.length > 80 ? msg.substring(0, 80) + '...' : msg;
          console.log(`${indent}      ${color(time, COLORS.GRAY)} ${icon} ${color(truncated, COLORS.GRAY)}`);
        }
      }
    }
  }
...
export function renderSingleSessionTimeline(
  ...
  console.log(`   Project: ${metadata.projectName} | Duration: ${durationStr} | Status: ${metadata.status}`);
  ...
  if (metadata.config) {
    ...
    if (metadata.config.maxAttemptsPerTask) {
      console.log(`   Max Attempts Per Task: ${metadata.config.maxAttemptsPerTask}`);
    }
  }
  ...
  if (metadata.outcomes) {
    ...
    console.log(`   Tasks Completed: ${color(String(metadata.outcomes.tasksCompleted), COLORS.GREEN)}`);
    console.log(`   Tasks Failed: ${color(String(metadata.outcomes.tasksFailed), metadata.outcomes.tasksFailed > 0 ? COLORS.RED : COLORS.GRAY)}`);
    console.log(`   Gaps Resolved: ${metadata.outcomes.gapsResolved}`);
    console.log(`   Iterations: ${metadata.outcomes.totalIterations}`);
    console.log(`   Convergence: ${metadata.outcomes.convergenceAchieved ? color('Yes', COLORS.GREEN) : color('No', COLORS.YELLOW)}`);
  }
  ...
  if (metadata.status === 'complete') {
    console.log(`${statusIcon} Session completed successfully in ${durationStr}`);
  ...
  console.log(`   harness inspect --session=${session.sessionId} --dirs          Show directory structure`);
  console.log(`   harness inspect --session=${session.sessionId} --toolDetails   Show detailed tool calls`);
  console.log(`   harness inspect --session=${session.sessionId} --ai            Show AI activity`);
  console.log('');
}
 ...
 */
function formatEventDetails(event: SessionEvent, options: InspectOptions): string {
  ...
  if (typeof event.metadata.success === 'boolean') {
    const status = event.metadata.success ? color('SUCCESS', COLORS.GREEN) : color('FAILED', COLORS.RED);
  ...
  // Progress info
  if (event.metadata.tasksComplete !== undefined && event.metadata.tasksTotal !== undefined) {
    lines.push(`${indent}Progress: ${event.metadata.tasksComplete}/${event.metadata.tasksTotal}`);
  }
...
export function renderMultiSessionTimeline(
  ...
  for (const session of sessions) {
    ...
    const duration = metadata.duration ? formatDuration(metadata.duration) : 'running';
    const statusIcon = getStatusIcon(metadata.status);
  ...
  console.log('');
  console.log(color('📁 Session directories:', COLORS.GRAY) + ' .harness/journal/sessions/');
  console.log('');
  console.log(color('Usage:', COLORS.BOLD));
  console.log('  harness inspect --session=<id>     View session details');
  console.log('  harness inspect --dirs              Show directory structure');
  console.log('  harness inspect --last=N            Show last N sessions');
  console.log('');
}
 ...
 */
export function renderDirectoryTree(sessionDir: string, sessionId: string): void {
  console.log('');
  ...
  const listDir = (dirPath: string, prefix: string = '', isLast: boolean = true): void => {
    try {
      const entries = readdirSync(dirPath, { withFileTypes: true }).sort((a: any, b: any) => {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        if (entry.isDirectory()) {
          console.log(`${prefix}${connector}${color('📁 ' + entry.name + '/', COLORS.BLUE)}`);
          ...
          console.log(`${prefix}${connector}📄 ${entry.name} ${color(`(${size})`, COLORS.GRAY)}${descText}`);
        }
      }
    } catch (err) {
  ...
  console.log(`  harness inspect --session=${sessionId} --tools`);
  console.log('');
}
...
function humanizeNodeId(node: ConvergenceNode): string {
  ...
  if (base.startsWith('run-strategy:')) {
    const stratName = (node.data?.strategy as string) || base.slice('run-strategy:'.length);
 ...
 */
function shortenGapDesc(gap: { description: string; metadata?: Record<string, unknown> }): string {
...
export function renderConvergenceView(
  ...
  console.log(color(`🎯 ${taskPath}`, COLORS.BOLD) + color(`  ${headerParts.join(' · ')}`, COLORS.GRAY));
  ...
  if (edges.length > 0) {
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      if (!targetNode) {
        rows.push({
          ...
          icon: '?', name: edge.to, dur: '',
          action: '', reason: '', origin: '', gaps: [], isLast,
        });
        continue;
      }

      const icon = targetNode.status === 'done' ? '✅'
        : targetNode.status === 'failed' ? '❌'
        : targetNode.status === 'executing' ? '🔄'
        : targetNode.status === 'skipped' ? '⏭️'
        : '⏳';

      const dur = targetNode.result ? formatDuration(targetNode.result.durationMs) : '';
      ...
      rows.push({
        iteration: edge.iteration,
        ts: edge.ts ? formatTimestamp(edge.ts, true) : '',
        icon, name: humanizeNodeId(targetNode), dur,
        action, reason, origin, gaps, isLast,
      });
    }
    ...
    for (const row of rows) {
      ...
      if (row.iteration !== prevIteration) {
        console.log('');
        if (row.iteration > 1) {
          console.log(color(`  ── Iteration ${row.iteration} ${'─'.repeat(50)}`, COLORS.GRAY));
        }
      ...
      let decision = '';
      if (row.action) {
        const actionColor = row.action === 'done' ? COLORS.GREEN
          : row.action === 'bail' ? COLORS.RED
          : COLORS.GRAY;
      ...
      const originTag = row.origin ? color(` [${row.origin}]`, COLORS.BLUE) : '';
      ...
      if (row.gaps.length > 0) {
        ...
        for (const g of row.gaps) {
          const gIcon = g.resolved ? color('✓', COLORS.GREEN) : color('✗', COLORS.RED);
          const kind = g.kind ? color(` (${g.kind})`, COLORS.GRAY) : '';
          console.log(color(`            ${gapConnector}`, COLORS.GRAY) + `     ${gIcon} ${color(g.desc, COLORS.GRAY)}${kind}`);
        }
      }
    }
  }
  ...
  if (bufferedNodes.length > 0 && !options.compact) {
    console.log('');
    console.log(color(`  Queued  ${bufferedNodes.length} nodes not executed`, COLORS.GRAY));
    const sorted = [...bufferedNodes].sort((a, b) =>
      ((b.data?.priority as number) || 0) - ((a.data?.priority as number) || 0)
    );
    for (const bn of sorted) {
      const cond = bn.data?.applicable ? ` requires: ${bn.data.applicable}` : '';
      console.log(color(`          ⏳ ${bn.id}${cond}`, COLORS.GRAY));
    }
  }

  // --- Outcome ---
  const verifyNode = [...nodes].reverse().find(n => n.handler === 'verify' && n.result);
  const lastFailedNode = [...nodes].reverse().find(n => n.status === 'failed' && n.result);
  const lastDoneNode = [...nodes].reverse().find(n => n.status === 'done' && n.result);
...
export async function renderAggregatedInspection(
  ...
  console.log('');
  console.log(color('🔍 Execution Timeline & Workflow Inspection', COLORS.BOLD));
  console.log(color('   Development tool - showing detailed session progression and issues', COLORS.GRAY));
  console.log('');
  ...
  for (let i = 0; i < timeline.length; i++) {
    ...
    console.log(color(`${statusIcon} Session ${i + 1}/${timeline.length}`, COLORS.BOLD) + color(` - ${session.sessionId}`, COLORS.GRAY));
    console.log(color(`   ${dateStr} ${timeStr}`, COLORS.GRAY) +
      (metadata.duration ? color(` • Duration: ${formatDuration(metadata.duration)}`, COLORS.GRAY) : ''));
    console.log('═'.repeat(80));
    ...
    if (existsSync(eventsPath)) {
      try {
        ...
        for (const event of events) {
          if (event.eventType === 'TASK_SELECTED' || event.eventType === 'TASK_ATTEMPT_START') {
        ...
        if (taskGroups.size > 0) {
          ...
          for (const [taskId, taskEvents] of taskGroups) {
            renderTaskWorkflow(taskId, taskEvents, taskNum++, taskIssues.get(taskId));
        ...
        console.log(color('   ⚠️  Error reading session events', COLORS.YELLOW));
      }
    }
    ...
    if (metadata.outcomes) {
      ...
      console.log(`   Iterations: ${metadata.outcomes.totalIterations}`);
      console.log(`   Completed: ${color(String(metadata.outcomes.tasksCompleted), COLORS.GREEN)} tasks`);
      if (metadata.outcomes.tasksFailed > 0) {
        console.log(`   Failed: ${color(String(metadata.outcomes.tasksFailed), COLORS.RED)} tasks`);
      }
      console.log(`   Convergence: ${metadata.outcomes.convergenceAchieved ? color('✓ Achieved', COLORS.GREEN) : color('✗ Not achieved', COLORS.RED)}`);
    }
    ...
    if (existsSync(sessionLogPath)) {
      ...
      console.log(`   harness inspect --session=${session.sessionId}`);
    }
  }
  ...
  if (journalOnlyTasks.length > 0) {
    console.log('');
    console.log('═'.repeat(80));
    console.log(color('📋 Tasks with journal data (no session events emitted):', COLORS.BOLD));
    console.log('');
  ...
  console.log(`   harness inspect --session=<id>    Deep dive into specific session`);
  console.log(`   harness tree                       View task dependency tree`);
  console.log(`   harness gantt                      Visualize execution order`);
  console.log('');
}
...
function renderTaskWorkflow(
  ...
  const completeEvent = events.find(e => e.eventType === 'TASK_ATTEMPT_COMPLETE');
  ...
  console.log(`      ${statusText} ${duration > 0 ? color(`(${formatDuration(duration)})`, COLORS.GRAY) : ''}`);
  ...
  if (issueData?.attempts && issueData.attempts.length > 0) {
    ...
    for (const attempt of issueData.attempts) {
      const resultIcon = attempt.hasCheckResult
        ? (attempt.checkPassed ? color('PASS', COLORS.GREEN) : color('FAIL', COLORS.RED))
        : color('incomplete', COLORS.YELLOW);
      const taskResult = attempt.hasTaskResult ? '' : color(' (no result)', COLORS.YELLOW);
  ...
  if (issueData?.outputs) {
    ...
    if (expectedOutputs && expectedOutputs.length > 0) {
      ...
      for (const out of expectedOutputs) {
        ...
        const icon = exists ? color('exists', COLORS.GREEN) : color('MISSING', COLORS.RED);
    ...
    if (inputs && inputs.length > 0) {
      ...
      for (const inp of inputs) {
        ...
        const icon = exists ? color('ok', COLORS.GREEN) : color('MISSING', COLORS.RED);
        console.log(`         ${pattern} [${icon}]`);
      }
    }
    if (blocked) {
      console.log(`      ${color('BLOCKED', COLORS.RED)} — inputs not satisfied`);
    }
  }
  ...
  if (issueData?.history) {
    const { totalAttempts, strategySummary, attempts: stratAttempts, failedApproaches } = issueData.history;
    if (totalAttempts > 0 || Object.keys(strategySummary).length > 0 || stratAttempts.length > 0) {
      console.log('');
      ...
      for (const attempt of stratAttempts) {
        ...
        const err = attempt.error ? color(` — ${attempt.error}`, COLORS.GRAY) : '';
        console.log(`         ${sIcon} ${attempt.strategy}${err}`);
      }
      if (failedApproaches.length > 0) {
        console.log(`         Failed approaches: ${failedApproaches.join(', ')}`);
      }
    } else if (cpStatus === 'interrupted' || !success) {
      console.log('');
      console.log(color('      Repair History:', COLORS.BLUE));
      console.log(`         ${color('No repair strategies were attempted', COLORS.YELLOW)}`);
      console.log(`         ${color('(HarnessController loop retried without invoking GapResolutionPipeline)', COLORS.GRAY)}`);
    }
  }
  ...
  if (gapEvents.length > 0) {
    ...
    for (const gap of gapEvents) {
      const msg = gap.message || gap.metadata?.gapType as string || 'Unknown gap';
      if (!seen.has(msg)) {
        seen.add(msg);
        console.log(`         ⚠️  ${msg}`);
      }
    ...
    if (gapEvents.length > seen.size) {
      console.log(color(`         (${gapEvents.length - seen.size} duplicate gap events omitted)`, COLORS.GRAY));
    }
  }

  // --- Session event: strategies attempted ---
  const strategyEvents = events.filter(e => e.eventType === 'STRATEGY_ATTEMPTED');
  if (strategyEvents.length > 0) {
    ...
    for (const stratEvent of strategyEvents) {
      ...
      const stratSuccess = events.some(e =>
        e.eventType === 'CONVERGENCE_ACHIEVED' &&
  ...
  if (toolCalls.length > 0) {
    ...
    const toolTypes = new Set(toolCalls.map(e => e.metadata?.tool).filter(Boolean));
    console.log('');
    console.log(color(`      Tools: ${toolCount} calls`, COLORS.GRAY) +
  ...
  if (issueData && issueData.issues.length > 0) {
    ...
    if (issueData.issues.length > 5) {
      console.log(color(`         ... and ${issueData.issues.length - 5} more`, COLORS.GRAY));
    }
  }

  // --- Convergence status ---
  const convergenceEvent = events.find(e =>
    e.eventType === 'CONVERGENCE_ACHIEVED' || e.eventType === 'CONVERGENCE_STALLED'
  );
  if (convergenceEvent) {
    ...
    console.log('');
    console.log(color(`      ${icon} ${text}`, converged ? COLORS.GREEN : COLORS.YELLOW));
  }
...
interface OutputSpec {
  inputs: Array<string | { pattern: string; count?: number; samples?: string[] }>;
  outputs: string[];
  blocked: boolean;
}
 ...
 */
async function loadTaskIssues(journalDir: string): Promise<Map<string, TaskIssueData>> {
  ...
  try {
    const epics = readdirSync(journalDir, { withFileTypes: true })
    ...
    for (const epic of epics) {
      ...
      const children = readdirSync(epicDir, { withFileTypes: true })
        .filter(d => d.isDirectory() && d.name !== 'attempts' && d.name !== 'logs');
...
function loadTaskJournalData(
    ...
    for (const attemptEntry of attemptDirs) {
      ...
      if (existsSync(checkResultPath)) {
        ...
        try {
          ...
          info.checkPassed = !content.includes('FAILED') && !content.includes('ERROR');
          // Extract a one-line summary
          const lines = content.split('\n').filter(Boolean);
          const summaryLine = lines.find(l => l.includes('✅') || l.includes('❌') || l.includes('FAILED') || l.includes('SUCCESS'));
  ...
  result.set(taskId, { issues, attempts, checkpoint, history, outputs, checks, feedback });
}
