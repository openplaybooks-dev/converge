...
export async function treeCommand(options: TreeCommandOptions = {}): Promise<void> {
  try {
    ...
    // Resolve harness config (provides discovery patterns)
    ...
    const harnessConfig = resolved
      ...
      : { name: path.basename(projectDir) } as import('../config/types.ts').HarnessConfig;
    ...
    const scanner = createDiscoveryScanner(
      harnessConfig.discovery || { epics: [], tasks: [] },
      projectDir
    );
    ...
    const taskTree = await TaskTree.load(projectDir, harnessConfig, discoveryResult);
    ...
    if (process.env.HARNESS_DEBUG || process.env.HARNESS_DEBUG_DEPS) {
