 ...
 *
 * Commands:
 *   harness run         - Run autonomous agent loop
...
function showHelp(): void {
  console.log(`
🤖 Harness - AI-First Autonomous Framework
...
COMMANDS:
  ...
  init                  Initialize new harness project
  ...
  skills list           List available skills in the harness
...
EXAMPLES:
  ...
  harness run 02-ux-ui-design-screen-generation

  # Run only a specific task within an epic
  harness run 02-ux-ui-design-screen-generation/003-generate-all-screens
  ...
  harness run --max-iterations=50 --verbose

  # Re-seed WBS (errors if already seeded)
  harness run --wbs 03-build-screens

  # Incremental WBS re-seed (preserves child progress)
  harness run --wbs --inc 03-build-screens

  # Run without auto-fix (manual mode)
  harness run --auto-fix=false
  ...
  harness reset 001-create-ux-overview 002-generate-design-system --all
  ...
  harness skills install --skill=harness-control

  # Install to a custom directory
  harness skills install --target=custom-skills-dir
  ...
  harness inspect --last-session

  # Show last 5 sessions
  harness inspect --last=5

  # Inspect specific session
  harness inspect --session=2026-04-05T05-07-19-abc123

  # Show directory structure
  harness inspect --dirs

  # Filter by task
  harness inspect --session=<id> --task=001-gather-idea-generate-ux

  # Show detailed tool calls with parameters
  harness inspect --session=<id> --toolDetails

  # Show AI activity
  harness inspect --session=<id> --ai

  # Export to JSON
  harness inspect --session=<id> --json > session.json

AUTONOMOUS LOOP:
  The 'harness run' command starts an autonomous agent loop that:
  ...
  6. 📊 Auto-writes to journals at .harness/journal/
...
For more: https://github.com/your-org/harness
`);
}
...
async function main(): Promise<void> {
  ...
  // When --playbook is set on ANY command, set HARNESS_PLAYBOOK so
  ...
  if (options.playbook && command !== 'run' && command !== 'plan') {
    process.env.HARNESS_PLAYBOOK = String(options.playbook);
  }
  ...
  if (!process.env.HARNESS_PLAYBOOK) {
    ...
    // set HARNESS_PLAYBOOK so path resolution matches the existing structure.
    if (!process.env.HARNESS_PLAYBOOK) {
      const { existsSync, readdirSync, statSync } = await import('node:fs');
      const { join } = await import('node:path');
      const journalDir = join(autoSearchDir, '.harness', 'journal');
      if (existsSync(journalDir)) {
        for (const entry of readdirSync(journalDir)) {
          ...
          if (existsSync(tasksDir) && statSync(tasksDir).isDirectory()) {
            process.env.HARNESS_PLAYBOOK = entry;
            break;
          }
        }
      }
  ...
  try {
    switch (command) {
      case 'run': {
        ...
        const resolved = await resolveHarnessConfig(searchDir);

        let hookRegistry: HookRegistry | undefined;
        let harnessConfig = resolved?.config;
        ...
        if (resolved) {
          ...
          // Validate config
          const validated = validateHarnessConfig(config, configPath);
          harnessConfig = validated;
          ...
          if (pb) {
            ...
            // Set HARNESS_PLAYBOOK so journal paths route correctly
            // through journal/{playbook}/tasks/ even without --playbook flag
            process.env.HARNESS_PLAYBOOK = pbName;
          }
        }
        ...
        if (options.playbook) {
          ...
          process.env.HARNESS_PLAYBOOK = playbookName;
        ...
        try {
          await runAutonomousCommand({
            dir: harnessConfig?.dir || options.dir,
            filter: runFilter,
            ...
            maxIterations: options['max-iterations'] || options.maxIterations || playbookRunCfg?.maxIterations || harnessConfig?.runtime?.maxIterations,
          ...
          delete process.env.HARNESS_PLAYBOOK;
        }
        break;
      }
      ...
      case 'init': {
        if (!options.name && positional.length === 0) {
          ...
          console.error('Usage: harness init --name="Project Name"');
          process.exit(1);
        }
      ...
      case 'reset': {
        if (!options.all && positional.length === 0) {
          console.error('❌ Usage: harness reset <taskId> [taskId...] [--outputs] [--wbs] [--all]');
          console.error('   Example: harness reset 003-generate-html-designs --all');
          console.error('   Example: harness reset --all  # reset entire project');
          process.exit(1);
        }
      ...
      case 'plan': {
        ...
        if (!prompt) {
          ...
          console.error('   Usage: harness plan --prompt "Your plan description"');
          console.error('      or: harness plan "Your plan description"\n');
          process.exit(1);
        }
        ...
        const planResolved = await resolveHarnessConfig(planSearchDir);

        let planHookRegistry: HookRegistry | undefined;
        let planHarnessConfig = planResolved?.config;

        if (planResolved) {
          const { config, configPath } = planResolved;
          console.log(`\n📋 Loaded config: ${configPath}`);
          const validated = validateHarnessConfig(config, configPath);
          planHarnessConfig = validated;
        ...
        const planTaskDir = join(planSearchDir, '.harness', 'playbooks', 'plan', 'tasks', '001-plan');
        const planPlaybookDir = join(planSearchDir, '.harness', 'playbooks', 'plan');
        ...
        process.env.HARNESS_PLAYBOOK = planPlaybookName;
        console.log(`   Mode: autonomous\n`);

        const planRunStartTime = Date.now();
        try {
          await runAutonomousCommand({
            dir: planHarnessConfig?.dir || options.dir,
            filter: '001-plan',
            force: options.force || false,
            ...
            selfPlan: false,
            verbose: options.verbose || options.v,
            harnessConfig: planHarnessConfig,
          ...
          // Show next steps
          const generatedDir = join(planSearchDir, '.harness', 'playbooks', planName);
          if (existsSync(generatedDir)) {
            console.log(`\n✅ Playbook generated: .harness/playbooks/${planName}/`);
            console.log(`\n   To run it:`);
            console.log(`   harness run --playbook=${planName}\n`);
          }
        } catch (err: any) {
          await appendTrend(planSearchDir, planPlaybookName, {
            sessionId: `run-${new Date().toISOString()}`,
      ...
      case 'skills': {
        ...
        switch (subcommand) {
          ...
          default: {
            ...
            console.error('Usage: harness skills <list|install> [options]');
            process.exit(1);
          }
        }
        break;
      ...
      case 'playbook': {
        ...
        switch (subcommand) {
          ...
          case 'info': {
            ...
            if (!playbookInfoName) {
              console.error('   Usage: harness playbook info <name>');
              process.exit(1);
            }
          ...
          case 'history': {
            ...
            if (!playbookHistName) {
              console.error('   Usage: harness playbook history <name>');
              process.exit(1);
            }
          ...
          default: {
            ...
            console.error('   Usage: harness playbook <list|info|history> [options]');
            process.exit(1);
          }
        }
        break;
      ...
      case 'shims': {
        ...
        if (!shimName) {
          console.error('❌ Usage: harness shims <name> [args...]');
          console.error('   Available shims: grep, wc, jq, find');
          console.error('   Example: harness shims grep -q "export default" src/App.tsx');
          process.exit(1);
        }
...
checks:
  - id: playbook-yml-exists
    cmd: test -f .harness/playbooks/${name}/playbook.yml
    description: playbook.yml created
  - id: has-tasks-dir
    cmd: test -d .harness/playbooks/${name}/tasks
    description: Tasks directory created
  - id: has-task-files
    cmd: test $(find .harness/playbooks/${name}/tasks -name "TASK.md" 2>/dev/null | wc -l) -ge 2
    description: At least 2 TASK.md files generated
---

# ${verb} Playbook: ${prompt}

You are ${isUpdate ? 'updating an existing' : 'generating a new'} harness playbook from a user's prompt.
...
1. Load the \`harness-planning\` skill
...
4. Target output directory: \`.harness/playbooks/${name}/\`
`;
}
