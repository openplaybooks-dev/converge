...
/**
 * Usage:
 *   harness run                # Full autonomous loop (snap → execute → snap)
 *   harness run --dry          # Preview full queue without executing
 *   harness run --step         # Execute the next single task
 *   harness run --step --dry   # Preview next task without executing
 */
export async function runAutonomousCommand(options: AutoRunOptions = {}): Promise<void> {
  try {
    if (!options.harnessConfig) {
      throw new Error('No .harness/PROJECT.md or .harness/project.yaml found. Please create a .harness/PROJECT.md or .harness/project.yaml file.');
    }
    ...
    const taskTree = await TaskTree.load(projectDir, options.harnessConfig);
      ...
      if (typeof unblockResult.retryMode === 'object' && unblockResult.retryMode.type === 'backoff') {
        ...
        if (producers.length > 0) {
          ...
          const sessionLogger = new SessionLogger(projectDir, sessionId,
            options.harnessConfig!.name || 'Unknown Project', { maxIterations: 1, maxAttemptsPerTask: 2 });
    ...
    const sessionId = generateSessionId();
    const projectName = options.harnessConfig.name || 'Unknown Project';
...
async function readNeedsJson(projectDir: string, node: TaskNode): Promise<any | null> {
  ...
  const p = join(
    projectDir, '.harness', 'journal', 'epics', node.epicId,
...
function buildStrayCheckpointPath(projectDir: string, node: TaskNode): string | null {
  ...
  return join(
    projectDir, '.harness', 'journal', 'epics', wrongEpicId,
...
async function runWbsOnly(options: AutoRunOptions): Promise<void> {
  ...
  const wbsNodes = tree.filter(n => {
    ...
    // Users may pass a full path like ".harness/epics/06-wire-screens/tasks/002-wire-navigation".
  ...
  for (const node of wbsNodes) {
    ...
    const sessionId = generateSessionId();
    const sessionLogger = new SessionLogger(projectDir, sessionId,
      options.harnessConfig?.name || 'Project', { maxIterations: 1, maxAttemptsPerTask: 1 });
    await sessionLogger.writeSessionStart();
