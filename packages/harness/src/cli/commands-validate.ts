 ...
 *
 * Usage:
 *   pnpm harness verify               # Full verification
 *   pnpm harness verify --rules       # Rule-based validation only
 *   pnpm harness verify --fix         # Auto-fix checkpoint inconsistencies
 *   pnpm harness verify --task 003-build  # Verify single task
 */
...
import { validateProject, validateTaskMdFile } from '../validation/validate.ts';
import type { ValidationIssue as RuleIssue } from '../validation/types.ts';
...
interface CheckpointIssue {
  taskId: string;
  epicId?: string;
  type: 'completed-missing-outputs' | 'failed-has-outputs' | 'checkpoint-mismatch';
...
async function runRuleValidation(projectDir: string): Promise<void> {
  ...
  if (result.projectMd) {
    if (result.projectMd.issues.length === 0) {
      ...
      for (const issue of result.projectMd.issues) {
        console.log(`      [${issue.layer}/${issue.severity}] ${issue.ruleId}: ${issue.message}`);
      }
    }
    console.log('');
  }
  ...
  if (result.tasks.length > 0) scopeParts.push(`${result.tasks.length} task${result.tasks.length !== 1 ? 's' : ''}`);
...
async function runSingleTaskValidation(projectDir: string, taskId: string): Promise<void> {
  console.log(`\n🔍 Validating task "${taskId}"...\n`);

  // Find the TASK.md file by scanning for matching task folder
  const { glob } = await import('glob');
  const patterns = [
    `.harness/epics/**/${taskId}/TASK.md`,
    `.harness/epics/**/tasks/${taskId}/TASK.md`,
    `.harness/playbooks/*/tasks/${taskId}/TASK.md`,
    `.harness/playbooks/*/tasks/**/tasks/${taskId}/TASK.md`,
  ];
...
async function runCheckpointValidation(
  ...
  resolved?: Awaited<ReturnType<typeof resolveHarnessConfig>>,
): Promise<void> {
  // Resolve harness config
  if (!resolved) {
    resolved = await resolveHarnessConfig(projectDir);
  }
  if (!resolved) {
    throw new Error('No .harness/PROJECT.md or .harness/project.yaml found. Run from within a harness project.');
  }
  const harnessConfig = validateHarnessConfig(resolved.config, resolved.configPath);

  // Scan filesystem for epics and tasks
  const scanner = createDiscoveryScanner(harnessConfig.discovery ?? {}, projectDir);
  ...
  console.log('');
  if (issues.length === 0) {
    console.log('✅ No inconsistencies found. Checkpoint is consistent with filesystem state.');
    return;
  }

  console.log(`❌ Found ${issues.length} inconsistenc${issues.length === 1 ? 'y' : 'ies'}:\n`);
  ...
  if (options.fix) {
    ...
    for (const issue of issues) {
      if (issue.type === 'failed-has-outputs') {
        ...
        await checkpointMgr.reconcileTask(issue.taskId, issue.message, issue.epicId);
        fixedCount++;
      }
    }
    ...
    console.log('💡 Run `pnpm harness verify --fix` to auto-fix reconcilable issues.');
  }
