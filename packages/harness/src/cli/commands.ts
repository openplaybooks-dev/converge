...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  const harnessDir = `${projectDir}/.harness`;

  console.log('🚀 Initializing Harness V2 project...\n');

  // Check if already initialized
  if (existsSync(harnessDir)) {
    console.error('❌ Error: Project already initialized (.harness/ exists)');
    process.exit(1);
  }

  // Create storage
  const storage = createFilesystemStorage(harnessDir);
  storage.init();
  ...
  console.log('\nNext steps:');
  console.log('  1. Define goals in .harness/project.yaml');
...
export async function runCommand(options: RunOptions = {}): Promise<void> {
  ...
  console.log('🚀 Running Harness V2 convergence loop...\n');
  ...
  if (!storage.isInitialized()) {
    console.error('❌ Error: Invalid .harness/ directory');
    process.exit(1);
  }
...
export async function statusCommand(options: CommonOptions = {}): Promise<void> {
  ...
  const projectYamlPath = `${harnessDir}/project.yaml`;
  if (!existsSync(projectYamlPath)) {
    ...
    const { resolveHarnessConfig } = await import('../config/loader.ts');
...
export async function checkpointCommand(options: CommonOptions = {}): Promise<void> {
  ...
  if (!checkpoint) {
    ...
    console.log('   Run: harness run --step');
    return;
  }
  ...
  console.log('📍 Harness Checkpoint\n');
  ...
  console.log('   💡 To reset failed tasks: harness reset <task-id>');
}
 ...
 *
 * Usage:
 *   harness reset 003-generate-all-screens
 *   harness reset 02-ux-ui-design-screen-generation/003-generate-all-screens
 *   harness reset 003-generate-all-screens --outputs   # also delete output files
 *
 * What it does:
...
export async function resetCommand(
  ...
  if (taskIds.length === 0) {
    console.error('❌ Usage: harness reset <taskId> [taskId...] [--outputs]');
    console.error('   Example: harness reset 003-generate-all-screens');
    process.exit(1);
  }
  ...
  for (const { filePath, status } of toReset) {
    ...
    console.log(`   ✔  ${displayId} reset — will re-run on next harness run`);
  }
  ...
  if (options.outputs) {
    ...
    console.log('\n💡 Run: harness run --step   to re-execute the reset task(s)');
