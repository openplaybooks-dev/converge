/**
 * Shared task-tree renderer used by both `harness run --step` and `harness tree`.
 */
 ...
 * Supports both the playbook layout (`.harness/playbooks/<name>/tasks/`) and
 * the legacy layout (`.harness/epics/`). Falls back to `.harness/epics/` when
 * nothing matches (e.g. empty tree).
 */
function deriveTreeRoot(tree: TaskNode[]): string {
  if (tree.length === 0) return '.harness/epics/';
  const sample = tree[0].filePath.replace(/\\/g, '/');
  const playbookMatch = sample.match(/(\.harness\/playbooks\/[^/]+\/tasks\/)/);
  if (playbookMatch) return playbookMatch[1];
  const legacyMatch = sample.match(/(\.harness\/epics\/)/);
  if (legacyMatch) return legacyMatch[1];
  return '.harness/epics/';
}
...
export function printTaskTree(
  ...
  epicIds.forEach((epicId, epicIdx) => {
    ...
    if (epicRootNode) {
      ...
      // No epic-root task node (epic has EPIC.md but no TASK.md).
