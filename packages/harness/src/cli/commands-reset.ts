...
import { getJournalRoot, getEpicsDir } from '../journal/structure.ts';
 ...
 *   harness reset 003-generate-html-designs --all  # outputs + wbs
 *   harness reset --all  # reset entire project
 */
export async function resetCommand(
  ...
  if (taskIds.length === 0) {
    ...
    console.error('  harness reset 003-generate-html-designs');
    console.error('  harness reset 003-generate-html-designs --all');
    console.error('  harness reset --all  # reset entire project');
    process.exit(1);
  }
  ...
  for (const taskId of taskIds) {
    await resetSingleTask(taskId, projectDir, checkpoint, checkpointMgr, options);
  }
  ...
  if (options.wbs) {
    console.log('💡 WBS-generated task files deleted. Parent will re-spawn children.');
  }
  if (!options.outputs && !options.wbs) {
    console.log('💡 Run: harness run --step   to re-execute the reset task(s)');
    ...
    console.log('💡 Run: harness run   to re-execute from scratch');
  }
}
...
async function resetSingleTask(
  ...
  const epicsDir = join(projectDir, '.harness', 'epics');
  ...
  const epicId = taskJournalDir ? await getEpicIdFromJournalPath(taskJournalDir) : null;
  ...
  if (checkpoint.version === 1) {
    ...
    for (const id of toRemove) {
      if (checkpoint.completedTasks.includes(id)) {
        checkpoint.completedTasks = checkpoint.completedTasks.filter((t: string) => t !== id);
        removed = true;
      }
      if (checkpoint.failedTasks && checkpoint.failedTasks.includes(id)) {
        checkpoint.failedTasks = checkpoint.failedTasks.filter((t: string) => t !== id);
        removed = true;
      }
      if (checkpoint.seededTasks && checkpoint.seededTasks.includes(id)) {
        checkpoint.seededTasks = checkpoint.seededTasks.filter((t: string) => t !== id);
        removed = true;
      }
      if (checkpoint.lockedTasks.includes(id)) {
        checkpoint.lockedTasks = checkpoint.lockedTasks.filter((t: string) => t !== id);
        removed = true;
      }
    }
  ...
  if (options.wbs && epicId) {
    ...
    if (existsSync(wbsTaskDir)) {
      ...
      console.log(`   ✅ Deleted WBS task directory (${wbsFiles.length} files)`);
    } else {
  ...
  async function search(dir: string): Promise<string | null> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === taskId || entry.name.endsWith(`-${taskId}`) || entry.name.includes(`/${taskId}`)) {
          return fullPath;
        }
 ...
 * Path format: .harness/journal/tasks/{epicId}/tasks/{taskId}
 */
async function getEpicIdFromJournalPath(journalPath: string): Promise<string | null> {
...
async function resetProjectCommand(options: ResetOptions): Promise<void> {
    ...
    for (const epicEntry of epicDirs) {
      ...
      for (const taskEntry of taskDirs) {
        ...
        if (existsSync(wbsDir)) {
          ...
          console.log(`✅ Deleted WBS tasks: ${epicEntry.name}/${taskEntry.name}/task/`);
        }
      }
    }
  }
  ...
  console.log('💡 Run: harness run   to start fresh execution');
}
