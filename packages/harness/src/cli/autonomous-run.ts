...
import type { ProgressSnapshot, GapInfo, SessionMetadata, SessionStatus } from '../journal/session-types.ts';
...
async function snapTree(config: AutonomousRunConfig): Promise<TreeSnap> {
  const scanner = createDiscoveryScanner(
    config.harnessConfig.discovery || { epics: [], tasks: [] },
    config.projectDir,
    config.hookRegistry,
  );
 ...
 */
export async function detectStuckTasks(projectDir: string, tree: TaskTree): Promise<StuckTask[]> {
  ...
  for (const epicEntry of epicEntries) {
    ...
    for (const ckptPath of checkpointPaths) {
      try {
        ...
        const taskId = (!rel || rel === '.') ? epicId : rel.replace(/(^|\/)tasks\//g, '$1');
        ...
        stuck.push({ taskId, epicId, status: raw.status, checkpointPath: ckptPath, lastActivity });
      } catch {
 ...
 */
export async function recoverStuckTasks(projectDir: string, tree: TaskTree, stuckTasks: StuckTask[]): Promise<void> {
  let recovered = 0;
  let reset = 0;
  ...
  for (const stuck of stuckTasks) {
    try {
      ...
      const node = tree.getNode(stuck.taskId) ?? tree.getNode(`${stuck.epicId}/${stuck.taskId}`);
      ...
      if (!node) {
        ...
        if (existsSync(wipDir)) {
          const unitCkptForAttempt = new UnitCheckpointManager(projectDir, 'task', stuck.epicId, stuck.taskId);
          const ckpt = await unitCkptForAttempt.load();
          await generateInterruptedMd(wipDir, projectDir, ckpt?.currentAttempt ?? 1, [], []);
        }

        const unitCkpt = new UnitCheckpointManager(projectDir, 'task', stuck.epicId, stuck.taskId);
      ...
      if ((unit.outputs?.length ?? 0) > 0) {
        ...
        if (actionableGaps.length === 0) {
          ...
          if (existsSync(wipDir)) {
            ...
            const missingOutputPaths = actionableGaps
              ...
              .map(g => g.description);
            const existingOutputs = allOutputs.filter(o => !missingOutputPaths.includes(o));
            const checkpoint = await unitCkpt.load();
            await generateInterruptedMd(wipDir, projectDir, checkpoint?.currentAttempt ?? 1, existingOutputs, missingOutputPaths);
          }
          ...
          console.log(`   → Reset to pending (${actionableGaps.length} output gap(s) remain)`);
          reset++;
        }
      } else {
        // Write INTERRUPTED.md into wip/ so the task runner continues in-place
        ...
        if (existsSync(wipDir)) {
          const checkpoint = await unitCkpt.load();
          await generateInterruptedMd(wipDir, projectDir, checkpoint?.currentAttempt ?? 1, [], []);
        }
 ...
 */
export async function resetAllTasks(projectDir: string, tree: TaskTree): Promise<void> {
  const journalEpicsDir = getEpicsDir(projectDir);
  if (!existsSync(journalEpicsDir)) return;
  ...
  for (const epicEntry of epicEntries) {
    ...
    for (const ckptPath of checkpointPaths) {
      try {
        ...
        const unitCkpt = new UnitCheckpointManager(projectDir, 'task', epicId, taskId);
        const checkpoint = await unitCkpt.load();
        if (checkpoint) {
          checkpoint.status = 'pending';
 ...
 */
const DIRTY_SESSION_STATUSES: Set<SessionStatus> = new Set(['error', 'cancelled', 'running']);
 ...
 */
async function getLastSessionMetadata(projectDir: string): Promise<SessionMetadata | null> {
 ...
 */
export async function guardDirtySession(projectDir: string, resume?: boolean, restart?: boolean): Promise<void> {
  ...
  if (outcomes) {
    console.error(`   Progress:   ${outcomes.tasksCompleted} completed, ${outcomes.tasksFailed} failed (${outcomes.totalIterations} iterations)`);
  }
  console.error(`\nTo continue, use one of:`);
  console.error(`   harness run --resume    # recover and continue from where it stopped`);
  console.error(`   harness run --restart   # reset non-complete tasks and start fresh\n`);
  process.exit(1);
}
 ...
 */
export async function autonomousRun(config: AutonomousRunConfig): Promise<AutonomousRunResult> {
  ...
  const projectName = config.harnessConfig.name || 'Unknown Project';
  ...
  let tree = await TaskTree.load(projectDir, config.harnessConfig);
  ...
  if (stuckTasks.length > 0) {
    if (config.resume) {
      ...
      console.error(`\nTo continue, use one of:`);
      console.error(`   harness run --resume    # recover interrupted tasks`);
      console.error(`   harness run --restart   # reset all tasks to pending\n`);
      process.exit(1);
    }
  } else if (config.restart) {
  ...
  const cleanupSession = async (signal: string) => {
    ...
    try {
      ...
      const currentTask = (global as any).__HARNESS_CURRENT_TASK__;
      if (currentTask) {
        try {
  ...
  try {
    ...
    if (elapsed > maxRunDurationMs) {
      ...
      return { completed: false, tasksCompleted, tasksFailed, iterations: iteration, stoppedReason: 'timeout' as const };
    }

    // ── 2. FIND NEXT TASK (hierarchical tree traversal) ──────────────
    const nextTaskResult = await tree.findNextTask(config.filter, config.force);
    ...
    if (!nextTaskResult) {
      ...
      return { completed: true, tasksCompleted, tasksFailed, iterations: iteration };
    }
    ...
    const node = {
      ...
      parentTaskId: nodeData.id.includes('/') ? nodeData.id.split('/')[0] : undefined,
    ...
    if (currentTaskId === lastSelectedTaskId) {
      ...
      if (consecutiveSelections >= MAX_SAME_TASK_SELECTIONS) {
        ...
        await sessionLogger.writeSessionEnd(
          { totalIterations: iteration, tasksCompleted, tasksFailed, gapsResolved, convergenceAchieved: false },
          'error'
        );
        return { completed: false, tasksCompleted, tasksFailed, iterations: iteration, stoppedReason: 'consecutive-failures' as const };
      }
    } else {
    ...
    // Log task selection
    await sessionLogger.logTaskSelected(node.journalTaskId, node.epicId, currentAttempt);

    // ── 3. EXECUTE ───────────────────────────────────────────────────
    const taskStartTime = Date.now();
    await sessionLogger.logTaskAttemptStart(node.journalTaskId, currentAttempt);
    ...
    try {
      ...
      console.error(`   ❌ Failed to load unit from ${node.filePath}: ${err.message}`);
      ...
      await sessionLogger.logTaskAttemptComplete(node.journalTaskId, currentAttempt, execResult.success, taskDuration);
    ...
    if (treeNode && treeNode.children.length > 0 && !unit.wbsFn) {
      console.log(`   ⏩ Container task (${treeNode.children.length} children) — skipping direct execution`);
      await tree.markSeeded(treeNode, treeNode.children.map(c => c.id));
      taskAttempts.delete(node.journalTaskId);
      consecutiveFailures = 0;
      tasksCompleted++;
      await sessionLogger.logConvergence(node.journalTaskId, true);
    ...
    if (execResult.success) {
      ...
      if (config.force) {
        await sessionLogger.writeSessionEnd(
          { totalIterations: iteration, tasksCompleted, tasksFailed, gapsResolved, convergenceAchieved: true },
          'complete'
        );
        return { completed: true, tasksCompleted, tasksFailed, iterations: iteration };
      }
      ...
      console.log(`: ${node.taskId} (attempt ${attempts}/${maxTaskAttempts})`);
      ...
      if (execResult.isBlocking) {
        ...
        console.error(`   ↳ Tasks without dependencies will continue executing.\n`);
      }

      if (attempts >= maxTaskAttempts) {
        // Task exhausted its attempts — permanently skip and unblock downstream
        console.log(`   ⛔ Max attempts (${maxTaskAttempts}) reached for ${node.taskId} — permanently skipping.`);
        console.log(`   ↳  Retry manually with: harness reset ${node.taskId}`);
        consecutiveFailures++;
        tasksFailed++;
        await sessionLogger.logConvergence(node.journalTaskId, false);
        ...
        console.log(`   ⚠️  Will retry on next iteration (${attempts}/${maxTaskAttempts} attempts used).`);
    ...
    return { completed: false, tasksCompleted, tasksFailed, iterations: iteration, stoppedReason: 'max-iterations' as const };
  } catch (error: any) {
