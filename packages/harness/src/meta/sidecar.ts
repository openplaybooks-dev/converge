 ...
 *
 * This is the recommended way to integrate Meta-Harness-style self-optimization
 * into the harness — it uses the existing sidecar infrastructure to hook into
 * task:complete and task:fail events without modifying the convergence loop.
 *
 * Usage:
 *   // In harness.ts or epic setup:
 ...
 *
 * Output:
 *   .harness/meta/proposals/{timestamp}-proposal.md   — human-readable
 *   .harness/meta/proposals/{timestamp}-proposal.json  — machine-readable
 *   .harness/meta/latest-stats.json                    — running statistics
 */
...
export class MetaOptimizationSidecar {
        ...
        try {
          ...
          if (proposal && proposal.proposals.length > 0) {
            ctx.log.warn(
              ...
              `Check .harness/meta/proposals/ for details.`
            );
          }
        } catch (err: any) {
          ctx.log.warn(`Meta-analysis failed: ${err.message}`);
  ...
  /**
   * Write running statistics to .harness/meta/latest-stats.json.
  ...
  private async writeRunningStats(ctx: SidecarContext): Promise<void> {
    try {
      const statsDir = join(this.projectDir, '.harness', 'meta');
   ...
   *
   * Created skills go to: .harness/skills/repair/repair-custom-{slug}/SKILL.md
   *
  ...
  private async maybeCreateRepairSkills(proposal: ImprovementProposal): Promise<void> {
    ...
    const repairSkillsDir = join(this.projectDir, '.harness', 'skills', 'repair');
