...
export async function evaluateGoals(projectDir: string): Promise<EvalResult> {
  ...
  const goalFiles = await glob('**/.harness/goals/*/GOAL.md', {
    cwd: projectDir,
    absolute: true,
    ignore: ['**/.harness/goals/*/goals/**'],  // Skip sub-goals (parsed by parent)
  });
...
const PLANNING_SKILLS = new Set(['harness-planning']);
...
export async function planFromGoals(
  ...
  const epicDir = join(projectDir, '.harness', 'epics', epicId);
  ...
  const logDir = join(projectDir, '.harness', 'journal', 'converge-planning', epicId);
  mkdirSync(logDir, { recursive: true });

  const skillsRoot = join(projectDir, '.harness', 'skills');
...
/**
 * Build the planning prompt sent to the harness-planning agent.
...
function buildPlanningPrompt(
  ...
  sections.push(`5. IMPORTANT: Do NOT include 'harness-planning' in any generated task's skills field.`);
 ...
 * - Strip planning skills (harness-planning) from skills fields
...
async function runWbs(wbsPath: string, result: GoalEvalResult, projectDir: string): Promise<{ tasks: TaskMdShape[], goalsSpawned: number }> {
  ...
  const logDir = join(projectDir, '.harness', 'journal', 'wbs-logs', goalId);
  mkdirSync(logDir, { recursive: true });
  ...
  const ctx: WbsContext = {
    ...
    plan: {
      getPlanPath: (_relativePath: string) => {
        ...
        return join(projectDir, '.harness', 'journal', 'wbs-logs', goalId, 'plan.md');
      },
    },
...
function goalTaskStatus(goal: GoalDefinition, projectDir: string): 'none' | 'all-failed' | 'active' {
  ...
  try {
    ...
    for (const epic of epics) {
      const epicMdPath = join(epicsDir, epic, 'EPIC.md');
 ...
 * Each GoalDef becomes a .harness/goals/{NNN}-{id}/GOAL.md file.
...
export function writeGoalDefs(projectDir: string, goalDefs: GoalDef[], parentGoalDir?: string): number {
  ...
  const goalsDir = join(projectDir, '.harness', 'goals');
  let written = 0;
