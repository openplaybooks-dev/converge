/**
 * Converge Runner — `harness run --converge`
 *
 ...
 *
 * Crash-safety model (matches harness core):
...
export async function convergeRun(config: ConvergeRunConfig): Promise<ConvergeResult> {
  ...
  const tree = await TaskTree.load(projectDir, harnessConfig);
  ...
  while (wave < maxWaves) {
    ...
    const postTree = await TaskTree.load(projectDir, harnessConfig);
  ...
  const finalTree = await TaskTree.load(projectDir, harnessConfig);
