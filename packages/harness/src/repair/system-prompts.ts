...
Execute all instructions in the skill file to completion.`;
    ...
    if (taskPrompt) {
      return `You are acting as a ${taskAgent} agent executing a harness task.
...
- If a check is marked **BROKEN COMMAND**, fix the \`cmd\` in the source TASK.md (in \`.harness/epics/\`)
...
2. Stop (framework will analyze)`;
  ...
  static buildDepsRepairPrompt(
    ...
    return `You are a harness repair agent. A task is blocked by missing required inputs.
...
Do NOT stop until each missing file is verified to exist on disk.`;
  ...
  /* ── Filesystem-based repair prompt (Meta-Harness optimization) ──── */
   ...
   *
   * Meta-Harness insight: Give the agent filesystem access to context files
