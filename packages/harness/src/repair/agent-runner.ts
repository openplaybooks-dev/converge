...
import type { AgentFnOptions, AgentFnResult } from '@crew/agentfn';
...
import { loadHarnessConfig } from '../config/loader.ts';
...
export function getAgentLogDir(projectDir: string, ctx?: JournalContext): string {
  if (ctx) {
    const structure = getJournalStructure(projectDir, ctx.epicId, ctx.taskId);
    return join(structure.attempt ?? structure.task!, 'logs');
  }
  return join(projectDir, '.harness', 'logs', 'claudefn');
}
 ...
 */
async function readLogTailAll(logDir: string, maxLines = 4): Promise<LogLine[]> {
  ...
  for (const logPath of logFiles) {
    try {
      ...
      for (const raw of content.split('\n')) {
        ...
        if (line.level === 'INFO' && line.message.startsWith('Starting claudefn')) continue;
        if (line.level === 'INFO' && line.message.startsWith('Timeout=')) continue;
        allLines.push(line);
      }
...
export function classifyAgentError(err: Error): {
  type: 'crash' | 'timeout' | 'api-error' | 'config-error' | 'mcp-error' | 'unknown';
  summary: string;
  hint: string;
} {
  ...
  if (exitMatch) {
    ...
    const codeName = codeNames[signed] ?? `0x${(code >>> 0).toString(16).toUpperCase()}`;
  ...
  if (msg.includes('startup timeout') || msg.includes('hung on startup') || msg.includes('MCP server connection')) {
    return {
      type: 'mcp-error',
  ...
  // Idle timeout
  if (msg.includes('timed out') || msg.includes('idle') || msg.includes('inactivity')) {
    return {
      type: 'timeout',
...
export { resolveAIConfig, listAIProviders, type ResolvedAIConfig } from '../ai/factory.ts';
...
async function printHeartbeat(opts: {
  ...
  if (lines.length === 0) {
    ...
    const lastExit = lines.slice().reverse().find((l: typeof lines[number]) => l.level === 'EXIT');
...
export async function logAgentStart(
  ...
  if (!ctx) return;
  await logTaskEvent(projectDir, ctx.epicId, ctx.taskId, 'CLAUDEFN_START', `claudefn ${phase}: ${description}`, { phase });
}
...
export async function logAgentComplete(
  ...
  if (!ctx) return;
  await logTaskEvent(
    projectDir, ctx.epicId, ctx.taskId, 'CLAUDEFN_COMPLETE',
    `claudefn ${phase} completed in ${result.durationMs}ms`,
    { phase, durationMs: result.durationMs, sessionId: result.sessionId, outputSnippet: tailString(result.raw, 500) },
  );
}
...
export async function logAgentFailed(
  ...
  await logTaskEvent(
    projectDir, ctx.epicId, ctx.taskId, 'CLAUDEFN_FAILED',
 ...
 */
export async function checkMcpToolsAvailable(requiredPrefixes: string[], projectDir?: string): Promise<string[]> {
  ...
  const missing = requiredPrefixes.filter(prefix => {
    return !Array.from(availableServers).some(server => server === prefix || server.startsWith(prefix + '-'));
  });
 ...
 */
export async function runAgent<T = unknown>(opts: AgentRunOptions): Promise<AgentFnResult<T>> {
  const {
    phase, prompt, agentOptions, projectDir, journalCtx,
    label, skillName, agentName, heartbeatIntervalMs = 60_000,
  } = opts;
  ...
  try {
    ...
    console.log(`   🔍 skillDirs: ${skillDirsDebug ? Object.keys(skillDirsDebug).join(', ') || '(empty)' : '(not set)'}`);
    ...
    try {
      ...
      const projectYamlPath = join(harnessDir, 'project.yaml');
      
      if (existsSync(projectYamlPath)) {
        const storage = new FilesystemStorage(harnessDir);
        ...
        if (projectConfig.ai) {
          ...
          console.log(`   📋 AI config type: ${aiConfigDebug.default ? 'multi-provider' : 'single-provider'}`);
          console.log(`   📋 AI config keys: ${Object.keys(aiConfigDebug).join(', ')}`);
          if (aiConfigDebug.providers) {
            console.log(`   📋 Providers: ${Object.keys(aiConfigDebug.providers).join(', ')}`);
          }
      ...
      if (!resolvedAI) {
        const projectMdPath = join(harnessDir, 'PROJECT.md');
        if (existsSync(projectMdPath)) {
          try {
            const harnessConfig = await loadHarnessConfig(projectMdPath);
            
            if (harnessConfig.ai) {
              aiConfigSource = 'PROJECT.md';
              availableProviders = listAIProviders(harnessConfig.ai);
              const preferredProvider = agentOptions.provider as string | undefined;
              resolvedAI = resolveAIConfig(harnessConfig.ai, preferredProvider);
            ...
            if (!loadError) loadError = `Failed to load PROJECT.md: ${mdErr.message}`;
          }
        } else if (!loadError) {
      ...
      if (resolvedAI) {
        console.log(`   🤖 AI Provider: ${resolvedAI.name} (${resolvedAI.resolvedProvider})${resolvedAI.baseUrl ? ` → ${resolvedAI.baseUrl}` : ''}`);
        console.log(`      Config source: ${aiConfigSource}`);
        if (availableProviders.length > 1) {
          const preferredProvider = agentOptions.provider as string | undefined;
          console.log(`      Available providers: ${availableProviders.join(', ')}${!preferredProvider ? ' (using default)' : ''}`);
        }
      } else {
        console.warn(`   ⚠️  ${loadError || 'AI configuration missing provider field'}`);
      }
    } catch (err: any) {
    ...
    const agentOpts: Record<string, unknown> = {
      ...
      ...(resolvedAI?.resolvedProvider && !agentOptions.provider ? { provider: resolvedAI.resolvedProvider } : {}),
      ...(resolvedAI?.apiKey && !agentOptions.apiKey ? { apiKey: resolvedAI.apiKey } : {}),
      ...(resolvedAI?.baseUrl && !agentOptions.baseUrl ? { baseUrl: resolvedAI.baseUrl } : {}),
      ...(resolvedAI?.model && !agentOptions.model ? { model: resolvedAI.model } : {}),
      ...(resolvedAI?.timeoutMs && !agentOptions.timeoutMs ? { timeoutMs: resolvedAI.timeoutMs } : {}),
      ...(resolvedAI?.maxRetries && !agentOptions.maxRetries ? { maxRetries: resolvedAI.maxRetries } : {}),
      // Hook to register process with AgentManager (disabled for kimi)
      ...(agentOptions.provider !== 'kimi' ? {
        onProcessSpawned: (proc: import('node:child_process').ChildProcess, logPath: string) => {
          if (proc.pid) {
    ...
    try {
      ...
      const logPath = await findLatestLog(logDir);
      await logAgentFailed(projectDir, journalCtx, phase, err, logPath ?? undefined);
      throw err;
    } finally {
