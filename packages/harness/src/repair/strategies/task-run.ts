...
export class TaskRunStrategy implements FixStrategy {
  ...
  async tryFix(gap: Gap, ctx: StrategyContext): Promise<StrategyOutcome> {
    ...
    // Resolve skills using the skill path resolver (supports .skill/, .claude/skills/, .harness/skills/)
    ...
    if (unitPath && basename(unitPath) === 'SKILL.md') {
      ...
      if (primarySkill) {
        ...
        if (skillDir) {
          ...
          console.log(`   ⚠️  Skill not found: ${primarySkill} (searched in .skill/, .claude/skills/, .harness/skills/)`);
        }
      }
    }
    ...
    if (attemptDir) {
      ...
      if (!filesExist) {
        ...
        const attemptNumber = Number(process.env.HARNESS_TASK_ATTEMPT ?? '1');
    ...
    const taskAgent = (gap.metadata?.taskAgent as string | undefined) ?? 'Harness';
    ...
    try {
      await runAgent({
        ...
        label: taskTitle,
        skillName,
        agentName: taskAgent !== 'Harness' ? taskAgent : undefined,
      });
  ...
  async preTask(_gap: Gap, _ctx: StrategyContext, _prevAttemptDirs: string[]): Promise<void> {
    ...
    const isResume  = process.env.HARNESS_RESUME  === '1' || process.argv.includes('--resume');
    const isRestart = process.env.HARNESS_RESTART === '1' || process.argv.includes('--restart');
