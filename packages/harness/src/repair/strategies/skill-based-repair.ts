 ...
 *   1. Builtin: {package}/src/repair/skill-templates/  (shipped with harness)
 *   2. Custom:  {project}/.harness/skills/repair/       (user/MetaSidecar created)
 *
...
import { parseTaskMd, type TaskMdDef } from '../../config/task-md-definition.ts';
...
export class SkillBasedRepairStrategy implements FixStrategy {
  ...
  async tryFix(gap: Gap, ctx: StrategyContext): Promise<StrategyOutcome> {
    ...
    const selected = await this.selectSkill(skills, gap, projectDir, journalCtx);
    if (!selected) {
      return { success: false, reason: 'AI could not select a repair skill' };
    }

    console.log(`   🔧 Skill: ${selected.name} — ${selected.description.slice(0, 70)}`);

    try {
      // 3. Gather context declared by the skill
      const attemptDir = process.env.HARNESS_TASK_ATTEMPT_DIR
        ?? getTaskAttemptDir(projectDir, journalCtx.epicId, journalCtx.taskId, 'wip');
      ...
      const contextFiles = await gatherAndWriteContext(contextSteps, vars, gap, projectDir, contextDir, journalCtx);
      ...
      // 6. Run
      const allowedTools = selected.def['allowed-tools'] ?? ['Read', 'Write', 'Edit', 'Bash', 'Glob', 'Grep'];
      await runAgent({
        phase: 'skill_repair',
        prompt,
        agentOptions: { skillDirs, timeoutMs: 300_000, maxRetries: 1, allowedTools },
        projectDir,
        journalCtx,
      ...
      return { success: true, reason: `Skill '${selected.name}' completed`, retryMode: 'validate' };
    } catch (err: any) {
      return { success: false, reason: `Skill '${selected.name}' failed: ${err.message}` };
    }
  }
  ...
  private async loadSkills(projectDir: string): Promise<LoadedSkill[]> {
    ...
    const customDir = join(projectDir, '.harness', 'skills', 'repair');
  ...
  private async loadFromDir(dir: string, source: 'builtin' | 'custom', out: Map<string, LoadedSkill>): Promise<void> {
...
Return: {"skill": "<name>", "reason": "<why>"}`;
    ...
    try {
      ...
      const jsonMatch = text.match(/\{[\s\S]*?"skill"\s*:\s*"([^"]+)"[\s\S]*?\}/);
      if (jsonMatch) {
...
function buildTemplateVars(
  ...
  const vars: TemplateVars = {
    ...
    attemptNumber: process.env.HARNESS_TASK_ATTEMPT ?? '1',
    gapId: gap.id,
...
async function gatherAndWriteContext(
  ...
  for (const step of steps) {
    try {
      switch (step.type) {
        case 'gap': {
          ...
          for (const field of step.fields) {
            ...
            if (v !== undefined) parts.push(`**${field}**: ${typeof v === 'string' ? v : JSON.stringify(v)}`);
          }
          if (parts.length > 0) {
            await writeFile(join(contextDir, 'gap-metadata.md'), parts.join('\n'));
        ...
        case 'cmd': {
          ...
          await writeFile(join(contextDir, fn), r.ok ? r.output : `Exit ${r.exitCode}:\n${r.output}`);
          files.push(fn);
          break;
        }
        case 'file': {
          ...
          if (!existsSync(full)) { if (!step.optional) { const fn = `${step.label}.txt`; await writeFile(join(contextDir, fn), `(not found: ${p})`); files.push(fn); } break; }
          const c = await readFile(full, 'utf-8');
          const fn = `${step.label}.md`;
          await writeFile(join(contextDir, fn), c.length > 3000 ? c.slice(0, 3000) + '\n...[truncated]' : c);
          files.push(fn);
          break;
        }
        case 'files': {
          ...
          const listing = matches.slice(0, max).map(f => `- ${f}`).join('\n');
          const extra = matches.length > max ? `\n... and ${matches.length - max} more` : '';
          const fn = `${step.label}.md`;
          await writeFile(join(contextDir, fn), listing + extra || '(no matches)');
          files.push(fn);
          break;
        }
        case 'ai': {
          ...
          const resp = await aiCtx.ask(prompt, {
            phase: 'context-gathering', label: step.label,
      ...
      console.log(`   ⚠️  Context '${(step as any).label ?? step.type}': ${err.message}`);
    }
  }
  return files;
}
...
async function writeGapSummary(gap: Gap, contextDir: string): Promise<void> {
  ...
  await writeFile(join(contextDir, 'gap.md'), [
    `# Gap: ${c.kind}`, `**Target**: ${c.target}`, `**Status**: ${c.status}`,
    `**Description**: ${gap.description}`,
    gap.metadata?.checkCmd ? `**Check**: \`${gap.metadata.checkCmd}\`` : '',
...
async function writeHistory(projectDir: string, journalCtx: { epicId: string; taskId: string }, contextDir: string): Promise<void> {
  try {
...
function buildPrompt(skillName: string, relContext: string, contextFiles: string[]): string {
  const list = [...contextFiles.map(f => `  - \`${relContext}/${f}\``),
    `  - \`${relContext}/gap.md\``, `  - \`${relContext}/history.md\` (if exists)`].join('\n');
