/**
 * WBSGeneratorRepairStrategy
 *
 * Fixes root cause in WBS generators when multiple tasks fail with the same pattern.
 * This strategy detects systemic issues and repairs the generator code directly.
 *
 * Use case:
 *   - Multiple tasks spawned by same WBS have identical gap types
...
export class WBSGeneratorRepairStrategy implements FixStrategy {
  ...
  async tryFix(gap: Gap, ctx: StrategyContext): Promise<StrategyOutcome> {
    ...
    console.log(`   🔍 Analyzing WBS generator for systemic issue...`);
    ...
    try {
      ...
      const generatorPath = gap.metadata?.wbsGeneratorPath as string;
      if (!generatorPath) {
        return {
          success: false,
      ...
      if (!existsSync(fullGeneratorPath)) {
        return {
          success: false,
          reason: `WBS generator not found: ${generatorPath}`,
          shouldRetry: false,
        };
      }
      ...
      // Path is like: .harness/epics/03-implement-app/002-generate-react-pages/task.ts
...
**Question:**
Is this gap caused by a bug in the WBS generator itself?
...
**Task:**
Generate the corrected WBS generator code that fixes: ${diagnosis.suggestedFix}
  ...
  private async regenerateAffectedTasks(
    ...
    for (const taskId of taskIds) {
      try {
        ...
        // Also delete the generated task file so WBS can regenerate it
        // Pattern: .harness/epics/{epicId}/{parentTaskId}/task/{childTaskId}/task.ts
        ...
        // This is usually in .harness/epics/.../task/{taskId}/task.ts
