...
🔧 Auto-fixed pattern in: .harness/epics/03-implement-app/003-generate-svg-assets/task.ts
...
```bash
cat .harness/journal/tasks/*/tasks/*/logs/events.jsonl | grep PATTERN
```
...
1. Verify the file was modified:
   ```bash
   git diff .harness/epics/*/*/task.ts
   ```

2. Check if pattern is now correct:
   ```bash
   grep "inputs" .harness/epics/*/*/task.ts
   ```
