...
import type { Snapshot, WalkResult, ActionHandler, Graph, GraphNode } from './types.ts';
...
function getEventWriter(): TaskEventWriter | null {
  return (global as any).__HARNESS_EVENT_WRITER__ || null;
}

function getSessionLogger(): any | null {
  return (global as any).__HARNESS_SESSION_LOGGER__ || null;
}

/**
 * Auto-install a skill from the harness package into the project's .harness/skills/.
 * Returns true if the skill was found and installed.
 */
async function autoInstallPackageSkill(skillName: string, projectDir: string): Promise<boolean> {
  const { existsSync } = await import('node:fs');
  const { join, dirname, resolve } = await import('node:path');
  const { mkdir, readdir, copyFile: fsCopyFile } = await import('node:fs/promises');
  const { fileURLToPath } = await import('node:url');

  // Resolve the harness package's skills directory
  ...
  const destDir = join(projectDir, '.harness', 'skills', skillName);
...
function pickRepresentative(group: Gap[]): Gap {
  const primary = group.find(g => g.metadata?.gapKind === 'output') ?? group[0];
  return {
    ...primary,
    metadata: {
      ...primary.metadata,
...
const checkWbsSeeded: ActionHandler = async (snap) => {
  ...
  const structure = getJournalStructure(snap.projectDir, snap.epicId, snap.unit.id);
  ...
  try {
    ...
    if (wbsData.spawnCount > 0) {
      ...
      const anyChildExists = subtasks.length > 0 && subtasks.some(
        (t) => existsSync(join(snap.unit.path, 'tasks', t.id, 'TASK.md'))
      );
    ...
    try { unlinkSync(wbsJsonPath); } catch { /* ignore */ }
...
const checkOutputsExist: ActionHandler = async (snap) => {
  ...
  const hasLearnMd = attemptDir ? existsSync(join(attemptDir, 'LEARN.md')) : false;
  const attemptNumber = parseInt(process.env.HARNESS_TASK_ATTEMPT || '1', 10);
  ...
  const actionableGaps = preflightGaps.filter(g =>
  ...
  if (actionableGaps.length === 0) {
    ...
    if (eventWriter) {
      eventWriter.aiReasoning('Pre-flight check: outputs already exist, skipping execution', {
        taskId: unit.id,
...
const resolvePlan: ActionHandler = async (snap) => {
  ...
  const { resolvePrompt, createTaskContext } = await import('../../unit/resolve.ts');
...
const resolveWbs: ActionHandler = async (snap) => {
  ...
  if (result.error || result.spawnCount === 0) {
    ...
    return { action: 'bail', success: false, reason: 'WBS failed to seed tasks' };
  }
...
const resolveBlockers: ActionHandler = async (snap) => {
  ...
  for (const b of blockers) {
    if (eventWriter) {
      eventWriter.gapDetected(b.id, b.description, (b.metadata?.gapKind as string) || 'blocker');
    }
    if (sessionLogger && snap.unit.id) {
      await sessionLogger.logGapDetected(snap.unit.id, (b.metadata?.gapKind as string) || 'blocker', b.description);
    }
  }
  ...
  if (stillBlocked.length > 0) {
    const label = snap.unit.wbsFn ? 'WBS task cannot seed' : 'Task cannot execute';
    console.log(`\n❌ ${label}: ${stillBlocked.length} blocker(s) still unresolved`);
    for (const b of stillBlocked) {
      console.log(`  - ${b.description}`);
    }
    return { action: 'bail', success: false, reason: `${stillBlocked.length} unresolvable blockers` };
  }
...
const bailBlockers: ActionHandler = async (snap) => {
  ...
  return { action: 'bail', success: false, reason: `${blockers.length} unresolvable blockers` };
};
...
const runExecutor: ActionHandler = async (snap, graph) => {
  ...
  if (unit.executorFn) {
    graph.addNode({ id: `run-executor-fn${suffix}`, handler: 'run-executor-fn', status: 'buffered', origin: 'reactive', data: { priority: 64 } });
    return { action: 'continue' };
  }
  if (unit.loopFn) {
    graph.addNode({ id: `run-loop-fn${suffix}`, handler: 'run-loop-fn', status: 'buffered', origin: 'reactive', data: { priority: 64 } });
    return { action: 'continue' };
  }
  if (unit.wbsFn) {
    return { action: 'done', success: true, reason: 'WBS already seeded' };
  ...
  if (unit.children.length > 0) {
    graph.addNode({ id: `run-children${suffix}`, handler: 'run-children', status: 'buffered', origin: 'reactive', data: { priority: 64 } });
    return { action: 'continue' };
  }

  const { resolveSkill } = await import('../../unit/resolve.ts');
  if (resolveSkill(unit)) {
    graph.addNode({ id: `run-skill${suffix}`, handler: 'run-skill', status: 'buffered', origin: 'reactive', data: { priority: 64 } });
    return { action: 'continue' };
  }
...
const runExecutorFn: ActionHandler = async (snap) => {
  ...
  const unit = snap.unit;
  const jCtx = { epicId: snap.epicId, taskId: unit.id };
  const meta = { id: unit.id, title: unit.title, outputs: unit.outputs, checks: await resolveChecks(unit) };
...
const runLoopFn: ActionHandler = async (snap) => {
  const { LoopFunctionExecutor } = await import('../../executor/loop-executor.ts');
  const unit = snap.unit;
  const jCtx = { epicId: snap.epicId, taskId: unit.id };
  const executor = new LoopFunctionExecutor(snap.projectDir, jCtx);
  const maxIterations = (unit.vars?.maxLoopIterations as number | undefined) ?? unit.config.maxIterations;
...
const runSkill: ActionHandler = async (snap) => {
  ...
  const { getJournalStructure } = await import('../../journal/structure.ts');
  const { resolveSkill } = await import('../../unit/resolve.ts');
  const { resolveSkillPath, resolveSkillsRoot } = await import('../../config/skill-path-resolver.ts');
  ...
  const attemptNumber = process.env.HARNESS_TASK_ATTEMPT || 'wip';
  const logDir = structure.attempt
    ? join(structure.attempt, 'logs')
    : join(structure.task!, 'attempts', attemptNumber, 'logs');

  const spawnRunner = new SpawnRunner(projectDir, jCtx, () => logDir,
    async (opts) => {
      await writeTaskStatus(projectDir, jCtx.epicId, jCtx.taskId, {
        taskId: jCtx.taskId, epicId: jCtx.epicId,
        focusPath: `${jCtx.epicId}/${jCtx.taskId}`,
        status: opts.status, startedAt: opts.startedAt, completedAt: opts.completedAt,
        attempt: 1, gapsResolved: 0, gapsFailed: 0,
        checklist: opts.checklist, error: opts.error,
      });
    }, unit);

  const state = { counter: 0, checklist: [] as any[], startedAt: new Date().toISOString() };

  for (const name of skillNames) {
    // Resolve skill path using the new resolver (supports .skill/, .claude/skills/, .harness/skills/)
    ...
    if (!skillDir) {
      ...
      if (!installed) {
        console.warn(`   ⚠️  Skill not found: ${name} (searched in .skill/, .claude/skills/, .harness/skills/)`);
        continue;
      }
    ...
    const relativeSkillPath = relative(projectDir, skillMdPath).replace(/\\/g, '/');
...
const repairLoop: ActionHandler = async (snap, graph) => {
  ...
  const repairDir = join(snap.projectDir, '.harness', 'skills', 'repair');
  const count = await reg.scanAndRegisterSkills(repairDir);
  if (count > 0) console.log(`   📦 ${count} repair skill(s) loaded`);

  const { fileURLToPath } = await import('node:url');
  const builtinDir = join(dirname(fileURLToPath(import.meta.url)), '..', 'skill-templates');
...
const runStrategyHandler: ActionHandler = async (snap, graph) => {
  ...
  if (!strategyName) return { action: 'bail', reason: 'run-strategy: no strategy name' };
  ...
  for (const [, group] of groups) {
    ...
    if (descriptor.type === 'builtin' && descriptor.strategy) {
      ...
      const gathered = await gatherContext(descriptor.contextSteps, gap, snap.projectDir);
...
async function buildTsStrategies(): Promise<import('../types.ts').FixStrategy[]> {
  const { TaskRunStrategy } = await import('../strategies/task-run.ts');
  const { WBSGeneratorRepairStrategy } = await import('../strategies/wbs-generator-repair.ts');
  const { WbsScriptRepairStrategy } = await import('../strategies/wbs-script-repair.ts');
  const { DependencyBackoffStrategy } = await import('../strategies/dependency-backoff.ts');
  const { MissingInputPatternRepairStrategy } = await import('../strategies/missing-input-pattern.ts');
  const { UserQuestionResumeStrategy } = await import('../strategies/user-question-resume.ts');
  const { ToolEnvironmentRepairStrategy } = await import('../strategies/tool-environment-repair.ts');
  return [
...
async function executeSkillStrategy(
  ...
  const { existsSync: fsExists, readdirSync: fsReaddir } = await import('node:fs');
  ...
  await writeFile(join(contextDir, 'gap.md'), [
    `# Gap: ${compact.kind}`, `**Target**: ${compact.target}`,
    `**Status**: ${compact.status}`, `**Description**: ${gap.description}`,
    gap.metadata?.checkCmd ? `**Check**: \`${gap.metadata.checkCmd}\`` : '',
    gap.metadata?.checkOutput ? `**Error**:\n\`\`\`\n${(gap.metadata.checkOutput as string).slice(0, 800)}\n\`\`\`` : '',
  ].filter(Boolean).join('\n'));
  ...
  const fileList = [...contextFiles.map(f => `  - \`${relCtx}/${f}\``),
    `  - \`${relCtx}/gap.md\``, `  - \`${relCtx}/history.md\``].join('\n');
...
**Read history.md first. Do NOT repeat failed approaches.**`;
  ...
  await runAgent({
    phase: 'skill_repair', prompt,
    agentOptions: { skillDirs, timeoutMs: 300_000, maxRetries: 1, allowedTools: ['Read', 'Write', 'Edit', 'Bash', 'Glob', 'Grep'] },
    projectDir: snap.projectDir, journalCtx: jCtx,
    label: descriptor.name, skillName: descriptor.name, agentName: 'repair',
  });
}

const strategyWbsGeneratorRepair: ActionHandler = async (snap) => {
  const { WBSGeneratorRepairStrategy } = await import('../strategies/wbs-generator-repair.ts');
  return runStrategy('wbs-generator-repair', () => new WBSGeneratorRepairStrategy(), snap);
};

const strategyWbsScriptRepair: ActionHandler = async (snap) => {
  const { WbsScriptRepairStrategy } = await import('../strategies/wbs-script-repair.ts');
  return runStrategy('wbs-script-repair', () => new WbsScriptRepairStrategy(), snap);
};

const strategyUserQuestionResume: ActionHandler = async (snap) => {
  const { UserQuestionResumeStrategy } = await import('../strategies/user-question-resume.ts');
  return runStrategy('user-question-resume', () => new UserQuestionResumeStrategy(), snap);
};
...
const verify: ActionHandler = async (snap) => {
  ...
  if (postGaps.length === 0) {
    ...
    return { action: 'done', success: true, reason: 'All gaps resolved', gaps: postGaps };
  }
  ...
  if (resolved >= 0) {
    ...
    console.log(`Verified: 0/${before} gap(s) resolved (${postGaps.length - before} new gap(s) found)`);
  }
...
const checkStall: ActionHandler = async (snap) => {
  ...
  if (actualResolved === 0 && hasStalled([...snap.gaps], [...snap.previousGaps])) {
    ...
    if (newStallCount >= 3) {
      ...
      return { action: 'bail', success: false, reason: 'Max stalls reached', stallCount: newStallCount };
    }

    console.log(`   Retrying... (attempt ${newStallCount + 1})`);
    return { action: 'continue', stallCount: newStallCount, previousGaps: [...snap.gaps] };
  }
...
const advanceAttempt: ActionHandler = async (snap) => {
  ...
  const currentDir = process.env.HARNESS_TASK_ATTEMPT_DIR;
  if (!currentDir || !existsSync(currentDir)) return { action: 'continue' };

  const logsDir = join(currentDir, 'logs');
  if (!existsSync(logsDir) || !readdirSync(logsDir).some(f => f.endsWith('.log'))) {
    return { action: 'continue' };
  }
  ...
  process.env.HARNESS_TASK_ATTEMPT_DIR = nextDir;
