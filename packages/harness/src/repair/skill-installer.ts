 ...
 *
 * Copies bundled repair skill templates to .harness/skills/repair/
 * if they don't already exist. Called during harness initialization
 ...
 * at .harness/skills/repair/repair-custom-{pattern}/SKILL.md
...
/**
 * Install repair skill templates to .harness/skills/repair/.
...
export async function installRepairSkills(
  ...
  const targetDir = join(projectDir, '.harness', 'skills', 'repair');
...
export function hasRepairSkills(projectDir: string): boolean {
  const repairDir = join(projectDir, '.harness', 'skills', 'repair');
