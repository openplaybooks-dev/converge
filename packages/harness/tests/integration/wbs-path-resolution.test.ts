...
import { extractJournalTaskId, extractLeafTaskId, extractParentTaskId } from '../../src/unit/path-utils.ts';
import { createTaskContext } from '../../src/unit/task-context.ts';
import * as path from 'node:path';

describe('WBS Path Resolution Integration', () => {
  const exampleProjectPath = '/Users/minh/Documents/sheetsrun/artifacts/claude-reactjs/example';

  describe('Real WBS Task: 002-001-page-home-dashboard', () => {
    const taskPath = path.join(
      exampleProjectPath,
      '.harness/epics/03-implement-app/002-generate-react-pages/tasks/002-001-page-home-dashboard'
    );

    it('should extract hierarchical journal task ID', () => {
      const journalTaskId = extractJournalTaskId(taskPath);
      expect(journalTaskId).toBe('002-generate-react-pages/002-001-page-home-dashboard');
    });
    ...
    it('should create correct task context', () => {
      ...
      expect(context.fullTaskId).toBe('002-generate-react-pages/002-001-page-home-dashboard');
      // Journal path should mirror epic structure exactly
      expect(context.journalPath).toContain('journal/default/tasks/03-implement-app/002-generate-react-pages/tasks/002-001-page-home-dashboard');
    });
  });
  ...
  describe('Real WBS Parent: 002-generate-react-pages', () => {
    const taskPath = path.join(
      exampleProjectPath,
      '.harness/epics/03-implement-app/002-generate-react-pages'
    );
  ...
  describe('Real WBS Task: 003-001-asset-logo', () => {
    const taskPath = path.join(
      exampleProjectPath,
      '.harness/epics/03-implement-app/003-generate-svg-assets/tasks/003-001-asset-logo'
    );
