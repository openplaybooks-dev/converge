...
describe('Session Logger Smoke Test', () => {
  ...
  beforeEach(async () => {
    testDir = await mkdtemp(join(tmpdir(), 'harness-smoke-'));
  });
  ...
  it('should create a complete session from start to end', async () => {
    ...
    const snapshot1: ProgressSnapshot = {
      ...
      gaps: [
        { type: 'output', task: 'task-001', count: 1 }
      ],
    };
    ...
    expect(events.some(e => e.eventType === 'CONVERGENCE_ACHIEVED')).toBe(true);
    expect(events.some(e => e.eventType === 'CONVERGENCE_STALLED')).toBe(true);

    // Verify progress snapshots
    const progress = (await readFile(join(sessionDir, 'progress.jsonl'), 'utf-8'))
      .trim()
      .split('\n')
      .map(line => JSON.parse(line));
