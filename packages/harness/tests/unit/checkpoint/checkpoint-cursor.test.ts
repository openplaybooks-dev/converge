...
import type { Checkpoint, Cursor, CheckpointV1 } from '../../../src/storage/types.ts';
import {
  hashTaskTree,
...
describe('Checkpoint Cursor System', () => {
  describe('Tree Hashing', () => {
    it('should generate deterministic hash for task paths', () => {
      const paths = [
        '.harness/epics/01-epic/001-task/TASK.md',
        '.harness/epics/01-epic/002-task/TASK.md',
        '.harness/epics/01-epic/003-task/TASK.md',
      ];
    ...
    it('should generate different hash when paths change', () => {
      ...
      const paths2 = [
        '.harness/epics/01-epic/001-task/TASK.md',
        '.harness/epics/01-epic/003-task/TASK.md', // Different task
      ];
  ...
  describe('Task ID Extraction', () => {
    it('should extract task ID from TASK.md path', () => {
      const path = '.harness/epics/01-epic/003-task/TASK.md';
      const taskId = extractTaskIdFromPath(path);
      expect(taskId).toBe('003-task');
    });

    it('should extract subtask ID from nested path', () => {
      const path = '.harness/epics/01-epic/003-task/tasks/002-subtask/TASK.md';
      const taskId = extractTaskIdFromPath(path);
  ...
  describe('Task Depth Calculation', () => {
    it('should calculate depth 0 for epic-level task', () => {
      const taskPath = '.harness/epics/01-epic/001-task/TASK.md';
      const epicPath = '.harness/epics/01-epic';
      const depth = calculateTaskDepth(taskPath, epicPath);
      expect(depth).toBe(0);
    });

    it('should calculate depth 1 for subtask', () => {
      const taskPath = '.harness/epics/01-epic/001-task/tasks/002-subtask/TASK.md';
      const epicPath = '.harness/epics/01-epic';
      const depth = calculateTaskDepth(taskPath, epicPath);
      expect(depth).toBe(1);
    });
    ...
    it('should calculate depth 2 for nested subtask', () => {
      const taskPath = '.harness/epics/01-epic/001-task/tasks/002-subtask/tasks/003-nested/TASK.md';
      const epicPath = '.harness/epics/01-epic';
      const depth = calculateTaskDepth(taskPath, epicPath);
      expect(depth).toBe(2);
    });
