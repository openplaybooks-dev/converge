...
describe('Tree Traversal', () => {
  ...
  describe('Cursor Tracking', () => {
    ...
    it('should maintain breadcrumbs in cursor', async () => {
      const root = new MockUnit('root', 1, '.harness/root');
      const task = new MockUnit('task', 1, '.harness/task');
      const subtask = new MockUnit('subtask', 1, '.harness/subtask');
  ...
  describe('Checkpoint Creation', () => {
    ...
    it('should include execution context in checkpoint', async () => {
      ...
      expect(lastCheckpoint?.context.rootPath).toBe('.harness/root');
  ...
  describe('Resume from Checkpoint', () => {
    it('should resume from cursor position', async () => {
      ...
      const resumeCursor: Cursor = {
        path: ['root', 'child2'],
        breadcrumbs: [
          { id: 'root', type: 'epic', filePath: '.harness/root', depth: 0 },
          { id: 'child2', type: 'task', filePath: '.harness/child2', depth: 1 },
        ],
        depth: 1,
      };
