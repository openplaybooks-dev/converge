...
import { generateEpicFromPlaybook, mergeRunConfig } from '../../../src/playbook/executor.ts';
import { resolvePlaybookPaths } from '../../../src/playbook/paths.ts';
import type { PlaybookSource, PlaybookRunConfig } from '../../../src/playbook/types.ts';
...
beforeEach(async () => {
  tmpDir = join(tmpdir(), `harness-pb-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
...
describe('parseDuration', () => {
  ...
  it('returns undefined for null', () => expect(parseDuration(null)).toBeUndefined());
});
...
`,
  ...
  it('throws on missing playbook.yml', async () => {
    ...
    await expect(parsePlaybookYml(join(tmpDir, 'empty'))).rejects.toThrow('No playbook.yml');
  });
  ...
  it('throws when no tasks/ directory', async () => {
    ...
    await expect(parsePlaybookYml(pbDir)).rejects.toThrow('no tasks/ directory');
  });
});
...
describe('resolvePlaybook', () => {
  it('generates epicId from key', () => {
    const source: PlaybookSource = {
      def: { name: 'fix-issue', inputs: { issue: { required: true } }, key: 'issue' },
...
describe('generateEpicFromPlaybook', () => {
  ...
  it('throws when epic already exists', async () => {
    ...
    await expect(generateEpicFromPlaybook(resolved, projectDir)).rejects.toThrow('already exists');
  });
});
...
describe('discoverPlaybooks', () => {
  it('discovers from .harness/playbooks/', async () => {
    await createPlaybook(join(tmpDir, '.harness', 'playbooks'), 'my-pb', {
      yml: 'name: my-pb\n',
      tasks: { '001': '---\ntitle: Step\n---\n' },
    });
...
describe('resolvePlaybookPaths', () => {
  it('named playbook', () => {
    ...
    expect(p.tasks).toBe(join('/project', '.harness', 'playbooks', 'fix-issue', 'tasks'));
    expect(p.journalTasks).toBe(join('/project', '.harness', 'journal', 'fix-issue', 'tasks'));
    expect(p.config).toBe(join('/project', '.harness', 'playbooks', 'fix-issue', 'playbook.yml'));
  });

  it('default playbook', () => {
    const p = resolvePlaybookPaths('/project');
    expect(p.playbook).toBe('default');
    expect(p.tasks).toBe(join('/project', '.harness', 'playbooks', 'default', 'tasks'));
    expect(p.config).toBe(join('/project', '.harness', 'playbooks', 'default', 'playbook.yml'));
  });

  it('default === no arg', () => {
    expect(resolvePlaybookPaths('/p')).toEqual(resolvePlaybookPaths('/p', 'default'));
  });
});
