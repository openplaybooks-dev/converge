...
beforeEach(async () => {
  tmpDir = join(tmpdir(), `harness-pj-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
...
describe('path helpers', () => {
  it('getPlaybookJournalDir returns journal/{playbook}', () => {
    ...
    expect(dir).toBe(join('/project', '.harness', 'journal', 'react-app'));
  });

  it('default playbook uses "default" as name', () => {
    const dir = getPlaybookJournalDir('/project', 'default');
    expect(dir).toBe(join('/project', '.harness', 'journal', 'default'));
  });
});
...
describe('initPlaybookJournal', () => {
  ...
  it('creates playbook.json', async () => {
    ...
    const playbookJsonPath = join(getPlaybookJournalDir(tmpDir, 'react-app'), 'playbook.json');
  ...
  it('does not overwrite existing playbook.json', async () => {
    await initPlaybookJournal(tmpDir, 'react-app');
    const path = join(getPlaybookJournalDir(tmpDir, 'react-app'), 'playbook.json');
...
describe('appendTrend', () => {
  it('creates trends.jsonl and appends entry', async () => {
    ...
    const trendsPath = join(getPlaybookJournalDir(tmpDir, 'react-app'), 'trends.jsonl');
