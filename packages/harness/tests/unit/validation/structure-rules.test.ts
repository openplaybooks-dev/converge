...
function makeInput(
  ...
  return {
    ...
    rawFrontmatter: {},
    filePath: '/project/.harness/epics/01/001-test/TASK.md',
    taskDir: '/project/.harness/epics/01/001-test',
    projectDir: '/project',
    ...overrides,
  };
}
...
describe('structure rules', () => {
  describe('folder-name-matches-id', () => {
    it('flags mismatch between folder name and id', () => {
      const issues = runRule('folder-name-matches-id', makeInput(
        { id: '002-other' },
        { taskDir: '/project/.harness/epics/01/001-test' },
      ));
  ...
  describe('wbs-script-exists', () => {
    it('flags missing wbs script', () => {
      ...
      const issues = runRule('wbs-script-exists', makeInput({
        id: '001-test',
    ...
    it('skips when no wbs declared', () => {
      const issues = runRule('wbs-script-exists', makeInput({ id: '001-test' }));
      expect(issues).toHaveLength(0);
    });
  });
  ...
  describe('executor-script-exists', () => {
    it('flags missing executor script', () => {
      mockExistsSync.mockReturnValue(false);
      const issues = runRule('executor-script-exists', makeInput({
        id: '001-test',
  ...
  describe('materials-exist', () => {
    it('flags missing materials', () => {
      mockExistsSync.mockReturnValue(false);
      const issues = runRule('materials-exist', makeInput({
        id: '001-test',
  ...
  describe('inputs-not-self-referential', () => {
    it('flags overlapping input/output paths', () => {
      const issues = runRule('inputs-not-self-referential', makeInput({
        id: '001-test',
  ...
  describe('body-or-skill-required', () => {
    it('flags task with nothing executable', () => {
      const issues = runRule('body-or-skill-required', makeInput({
        id: '001-test',
      }));
      expect(issues).toHaveLength(1);
      expect(issues[0].severity).toBe('warning');
  ...
  describe('plan-output-path', () => {
    it('flags path outside project', () => {
      const issues = runRule('plan-output-path', makeInput({
        id: '001-test',
  ...
  describe('check-cmd-syntax', () => {
    it('flags dangling operator', () => {
      const issues = runRule('check-cmd-syntax', makeInput({
        id: '001-test',
