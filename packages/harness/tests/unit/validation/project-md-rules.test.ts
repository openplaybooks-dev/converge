...
function makeInput(
  ...
  filePath = '/project/.harness/PROJECT.md',
...
describe('PROJECT.md rules', () => {
  describe('project-name-required', () => {
    ...
    it('passes with valid name', () => {
      const issues = runRule('project-name-required', makeInput({ name: 'My App' }));
  ...
  describe('project-unknown-key', () => {
    it('flags unknown keys', () => {
      const issues = runRule('project-unknown-key', makeInput({ name: 'X', foo: 'bar' }));
      expect(issues).toHaveLength(1);
    ...
    it('passes with only known keys', () => {
      const issues = runRule('project-unknown-key', makeInput({
        name: 'X', description: 'Y', discovery: {}, runtime: {},
      }));
      expect(issues).toHaveLength(0);
    });
  });
  ...
  describe('project-discovery-shape', () => {
    it('flags non-object discovery', () => {
      const issues = runRule('project-discovery-shape', makeInput({ discovery: 'bad' }));
      expect(issues).toHaveLength(1);
      expect(issues[0].severity).toBe('error');
    });
  ...
  describe('project-runtime-shape', () => {
    it('flags non-object runtime', () => {
      const issues = runRule('project-runtime-shape', makeInput({ runtime: [] }));
      expect(issues).toHaveLength(1);
    });

    it('flags negative maxIterations', () => {
      const issues = runRule('project-runtime-shape', makeInput({
        runtime: { maxIterations: -1 },
      }));
      expect(issues.some(i => i.field === 'runtime.maxIterations')).toBe(true);
    });
    ...
    it('flags non-boolean parallelExecution', () => {
      ...
      expect(issues.some(i => i.field === 'runtime.parallelExecution')).toBe(true);
    });
    ...
    it('flags unknown runtime keys', () => {
      ...
      expect(issues.some(i => i.message.includes('unknownOption'))).toBe(true);
    });

    it('passes with valid runtime', () => {
      const issues = runRule('project-runtime-shape', makeInput({
        runtime: {
          maxIterations: 50, parallelExecution: true,
          logLevel: 'info', milestone: 'v1',
        },
      }));
      expect(issues).toHaveLength(0);
    });
  ...
  describe('project-hook-event-valid', () => {
    ...
    it('passes with valid hook events', () => {
      const issues = runRule('project-hook-event-valid', makeInput({
        hooks: { 'task:fail': './on-fail.sh', 'project:complete': './done.sh' },
      }));
      expect(issues).toHaveLength(0);
    });
  ...
  describe('project-hook-value-string', () => {
    it('flags non-string hook value', () => {
      const issues = runRule('project-hook-value-string', makeInput({
        hooks: { 'task:fail': 42 },
      }));
      expect(issues).toHaveLength(1);
      expect(issues[0].severity).toBe('error');
  ...
  describe('project-plugins-array', () => {
    it('flags non-array plugins', () => {
      const issues = runRule('project-plugins-array', makeInput({ plugins: 'typescript' }));
      expect(issues).toHaveLength(1);
    });
  ...
  describe('project-string-map-shape', () => {
    it('flags non-object agents', () => {
      const issues = runRule('project-string-map-shape', makeInput({ agents: ['main'] }));
      expect(issues).toHaveLength(1);
    });

    it('flags non-string agent value', () => {
      const issues = runRule('project-string-map-shape', makeInput({
        agents: { default: 42 },
      }));
      expect(issues).toHaveLength(1);
    ...
    it('passes with valid agents map', () => {
      const issues = runRule('project-string-map-shape', makeInput({
        agents: { default: '.harness/agents/main.md' },
      }));
      expect(issues).toHaveLength(0);
    });
  ...
  describe('project-variables-object', () => {
    it('flags non-object variables', () => {
      const issues = runRule('project-variables-object', makeInput({ variables: 'bad' }));
      expect(issues).toHaveLength(1);
    });
    ...
    it('passes with object variables', () => {
      const issues = runRule('project-variables-object', makeInput({ variables: { env: 'prod' } }));
      expect(issues).toHaveLength(0);
    });
  });
});
