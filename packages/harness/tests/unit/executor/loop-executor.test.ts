...
vi.mock('../../../src/journal/structure.ts', () => ({
  getJournalStructure: vi.fn(() => ({
    root: '/project/.harness/journal',
    project: '/project/.harness/journal/project',
    epic: '/project/.harness/journal/tasks/02-epic',
...
function simpleLoopFn(skillPath = '.harness/skills/stitch-loop/TASK.md'): LoopFn {
  return async (ctx) => {
...
describe('LoopFunctionExecutor', () => {
  ...
  describe('ctx.loop.spawn()', () => {
    it('returns not-done when skill file does not exist', async () => {
      ...
      const fn: LoopFn = async (ctx) => {
        await ctx.loop.spawn('.harness/skills/stitch-loop/TASK.md');
        return ctx.loop.done();
      };
    ...
    it('returns SpawnResult with success=true on success', async () => {
      let result: any;
      const fn: LoopFn = async (ctx) => {
        result = await ctx.loop.spawn('.harness/skills/stitch-loop/TASK.md');
        return ctx.loop.done();
      };
  ...
  describe('ctx.loop.spawn() — extended forms', () => {
    ...
    it('path-ref form calls Unit.fromPath with the resolved absolute path', async () => {
      ...
      const fn: LoopFn = async (ctx) => {
        await ctx.loop.spawn(taskDef().fromPath('.harness/tasks/my-task/TASK.md'));
        return ctx.loop.done();
      };
    ...
    it('path-ref form throws when file does not exist', async () => {
      ...
      const fn: LoopFn = async (ctx) => {
        await ctx.loop.spawn(taskDef().fromPath('.harness/tasks/missing.ts'));
        return ctx.loop.done();
      };
    ...
    it('taskDef().fromPath() returns a PathRefTarget synchronously without I/O', () => {
      const ref = taskDef().fromPath('.harness/tasks/my-task/TASK.md');
      expect(ref).toEqual({ _type: 'path-ref', path: '.harness/tasks/my-task/TASK.md' });
    });
