...
import type { ExecutorFn, ExecutorContext, AiFnOpts } from '../../../src/config/task-definition.ts';
import { z } from 'zod';
...
vi.mock('@crew/agentfn', () => ({
  agentfn: vi.fn((opts: any) => {
    ...
    return async () => {
      ...
      if (isStructured) {
        const data = structuredResponses.shift() ?? { answer: false, reasoning: 'default' };
        return { data, raw: '', durationMs: 30, sessionId: 'structured-session', provider: 'claude' };
      }
      return { data: 'output', raw: 'output', durationMs: 100, sessionId: 'skill-session', provider: 'claude' };
    };
  }),
}));
...
vi.mock('../../../src/unit/unit.ts', () => ({
  Unit: {
    fromDefinition: vi.fn((_taskDef: any, _parent: any, _virtualPath?: any) => ({
...
vi.mock('../../../src/journal/structure.ts', () => ({
  getJournalStructure: vi.fn(() => ({
    task: '/project/.harness/journal/tasks/02-epic/tasks/003-task',
  })),
  getTaskCorrectionsDir: vi.fn(() => '/project/.harness/journal/tasks/02-epic/tasks/003-task/corrections'),
}));
...
const TASK_META = { id: '003-generate-all-screens', title: 'Generate All Screens' };
const PROJECT_DIR = '/project';

function makeExecutor(parentUnit?: any) {
  return new TaskExecutor(PROJECT_DIR, JOURNAL_CTX, TASK_META, parentUnit ?? null);
}
...
describe('TaskExecutor', () => {
  ...
  describe('ctx.task', () => {
    it('exposes task id and title', async () => {
      ...
      const fn: ExecutorFn = async (ctx) => { capturedCtx = ctx; };
    ...
    it('exposes iteration from harness', async () => {
      const iterations: number[] = [];
      const fn: ExecutorFn = async (ctx) => { iterations.push(ctx.task.iteration); };
    ...
    it('exposes projectDir', async () => {
      ...
      const fn: ExecutorFn = async (ctx) => { dir = ctx.projectDir; };
  ...
  describe('ctx.ai.fn()', () => {
    ...
    it('multiple calls to same fn are independent and journaled separately', async () => {
      ...
      const starts = journalEvents.filter(e => e.type === 'CLAUDEFN_START' && e.message?.includes('checkDone'));
    ...
    it('agentfn constructed once per fn() call, not per invocation', async () => {
      const { agentfn } = await import('@crew/agentfn');
    ...
    it('defaults to unrestricted allowedTools (undefined)', async () => {
      ...
      const fn: ExecutorFn = async (ctx) => {
        const f = ctx.ai.fn({ prompt: 'Q', schema: z.object({ answer: z.boolean() }) });
        await f();
      };
    ...
    it('logs CLAUDEFN_FAILED on error and re-throws', async () => {
      ...
      const fn: ExecutorFn = async (ctx) => {
        const f = ctx.ai.fn({ prompt: 'Q', schema: z.object({ done: z.boolean() }) });
        try {
          await f();
        } catch (e: any) {
          thrown = e;
      ...
      expect(journalEvents.some(e => e.type === 'CLAUDEFN_FAILED')).toBe(true);
    });
  });
  ...
  describe('ctx.ai.ask().asJson()', () => {
    it('returns structured data matching the schema', async () => {
      structuredResponses = [{ remaining: 3, names: ['home', 'about', 'contact'] }];
      let result: any;

      const fn: ExecutorFn = async (ctx) => {
        result = await ctx.ai.ask('How many screens are remaining?').asJson(
      ...
      expect(result).toEqual({ remaining: 3, names: ['home', 'about', 'contact'] });
    });
  ...
  describe('ctx.spawn(string)', () => {
    it('throws when skill file does not exist', async () => {
      ...
      const fn: ExecutorFn = async (ctx) => {
        try {
          await ctx.spawn('.harness/skills/stitch-loop/TASK.md');
        } catch (e: any) {
          thrown = e;
        }
      };
    ...
    it('returns SpawnResult with success=true', async () => {
      let result: any;
      const fn: ExecutorFn = async (ctx) => {
        result = await ctx.spawn('.harness/skills/stitch-loop/TASK.md');
      };
    ...
    it('journals as subtask:spawn-N checklist item', async () => {
      const fn: ExecutorFn = async (ctx) => {
        await ctx.spawn('.harness/skills/stitch-loop/TASK.md', { label: 'My spawn' });
      };
  ...
  describe('ctx.spawn(InlineTaskTarget)', () => {
    it('creates a virtual child Unit via Unit.fromDefinition', async () => {
      ...
      const fn: ExecutorFn = async (ctx) => {
        await ctx.spawn({
          definition: { id: 'child-task', outputs: ['out/file.html'], vars: {} },
        });
      };
    ...
    it('journals as subtask checklist item', async () => {
      const fn: ExecutorFn = async (ctx) => {
        await ctx.spawn({
          definition: { id: 'child-task', title: 'My Child', outputs: [], vars: {} },
        });
      };
    ...
    it('writes .ts file when writeToPath is specified', async () => {
      const fn: ExecutorFn = async (ctx) => {
        await ctx.spawn({
          ...
          writeToPath: '.harness/epics/02-ux/003-screens/child-task/TASK.md',
        });
      };
    ...
    it('passes absolute path to Unit.fromDefinition when writeToPath set', async () => {
      ...
      const fn: ExecutorFn = async (ctx) => {
        await ctx.spawn({
          definition: { id: 'child-task', outputs: [], vars: {} },
          writeToPath: '.harness/epics/child-task/TASK.md',
        });
      };
  ...
  describe('TaskExecutorResult', () => {
    it('returns success=true when executor completes without error', async () => {
      const fn: ExecutorFn = async (_ctx) => { /* no-op */ };
    ...
    it('returns success=false when executor throws', async () => {
      const fn: ExecutorFn = async (_ctx) => { throw new Error('executor error'); };
      const result = await makeExecutor().run(fn, 1);
