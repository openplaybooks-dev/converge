...
describe('Parent Facts Loading', () => {
  ...
  it('should have context with parent reference', async () => {
    // Create parent task
    const parentPath = path.join(projectDir, '.harness/epics/01-epic/001-parent');
    ...
    const parentJournalPath = path.join(
      projectDir,
      '.harness/journal/tasks/01-epic/001-parent/attempts/wip/logs'
    );
    ...
    expect(parentUnit.context?.journalPath).toBe(
      path.join(projectDir, '.harness/journal/tasks/01-epic/001-parent')
    );
  ...
  it('should return empty object when parent has no facts', async () => {
    ...
    const parentPath = path.join(projectDir, '.harness/epics/02-epic/002-parent');
  ...
  it('should handle units without parent', async () => {
    ...
    const taskPath = path.join(projectDir, '.harness/epics/03-epic/003-task');
