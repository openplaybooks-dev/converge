 ...
 *
 *   000-setup      → install harness-planning skill to .harness/skills
...
export async function run(ctx) {
  ...
  // Install the harness-planning skill into .harness/skills so that
  // all subsequent tasks can load playbooks and preferences from it.
  await ctx.spawn({
    id: '000-setup-skills',
    title: 'Install harness-planning skill',
    outputs: ['.harness/skills/harness-planning/SKILL.md'],
    checks: [
      { id: 'skill-installed', cmd: 'test -f .harness/skills/harness-planning/SKILL.md', description: 'harness-planning skill installed' },
      { id: 'playbooks-exist', cmd: 'test -d .harness/skills/harness-planning/playbooks', description: 'Skill playbooks directory exists' },
      { id: 'preferences-exist', cmd: 'test -d .harness/skills/harness-planning/preferences', description: 'Skill preferences directory exists' },
    ],
    body: `Install the harness-planning skill into \`.harness/skills/\`.

Run:
\`\`\`bash
harness skills install --skill=harness-planning --target=.harness/skills --force
\`\`\`

If the \`harness\` CLI is not available, manually copy the skill:
1. Find the harness-planning skill in the harness package's \`skills/\` directory
2. Copy the entire \`harness-planning/\` directory to \`.harness/skills/harness-planning/\`

After installation, verify the skill structure:
\`\`\`
.harness/skills/harness-planning/
├── SKILL.md
├── README.md
├── playbooks/
│   ├── analyze.md
...
This skill provides the reference material (playbooks, schemas, patterns) that
...
Reference: load the harness-planning skill, then read \`playbooks/analyze.md\` for the full analysis protocol.
...
Reference: load the harness-planning skill, then read \`playbooks/discovery.md\` for the discovery framework.
...
Reference: load the harness-planning skill, then read:
- \`playbooks/architect.md\` — Step 3.1 (Define Epics) only
- \`preferences/project-patterns.md\` for common project templates
...
Reference: the harness-planning skill's playbooks/architect.md (Steps 3.2-3.6)
and preferences/plan-schema.md for task format.`,
    },
...
Reference: load the harness-planning skill, then read \`playbooks/validate.md\` for the full validation checklist.
...
Do NOT proceed to emit until validation passes.`,
  ...
  await ctx.spawn({
    ...
    outputs: [`.harness/playbooks/${name}/playbook.yml`],
    checks: [
      { id: 'playbook-yml-exists', cmd: `test -f .harness/playbooks/${name}/playbook.yml`, description: 'playbook.yml created' },
      { id: 'has-tasks', cmd: `test -d .harness/playbooks/${name}/tasks`, description: 'Tasks directory created' },
    ],
...
${update ? `UPDATE MODE:
- Read existing playbook at .harness/playbooks/${name}/
...
- Playbook location
- How to run: \`harness run --playbook=${name}\``,
  });
}
