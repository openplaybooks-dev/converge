...
- **Three-layer data model:** see §7.1. The runner reads from `inventory/`. Inventory has two sync sources — `compile` (from `playbooks/`) and runtime `spawn` (from `journal/`). Never hand-edit inventory or journal files; fix the source or the loader/compiler.
...
- A `handoff:` block on any task (task or gateway) holds the task as `awaiting-review` after execution succeeds — the handoff IS the review; there is no separate `review:` key. The runner stays alive and polls the verdict file at `inventory/<playbook>/reports/<taskId>.jsonl`.
