...
`,
      ...
      const logFile = (await import("node:fs/promises"))
        .readdir(logs)
        .then((files) => files.find((file) => file.endsWith(".log"))!);
      await expect(
        readFile(join(logs, await logFile), "utf8"),
      ).resolves.toContain("no model activity");
    } finally {
