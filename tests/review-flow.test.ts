...
function spawnCli(
  cwd: string,
  args: string[],
): { child: ChildProcess; out: () => string; exit: Promise<ProcResult> } {
  const child = spawn("node", [CLI, ...args], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
  });
...
describe("review flow (CLI-only, no HTTP)", { sequential: true }, () => {
  ...
  it("runner stays alive on awaiting-review; approve via CLI resumes it", async () => {
    await cp(FIXTURE, join(ws, ".converge", "playbooks", "handoff-review"), {
      recursive: true,
    });
    ...
    await waitFor(
      () => out().includes("awaiting-review"),
      30_000,
      "awaiting-review line",
    );
  ...
  it("revise records the verdict; the next approve clears the gate", async () => {
    ...
    const rev1 = await runCli(ws, [
      ...
      "manager-report",
      "--revise",
      "Add concrete risk metrics.",
      "--playbook=handoff-review",
    ]);
    expect(rev1.code).toBeFalsy();
    await waitFor(
      () => out().includes("received revise"),
      15_000,
      "revise echo",
    );
  ...
  it("reject records the verdict and the runner does not mark the task done", async () => {
    ...
    const entries = readFileSync(reports, "utf8")
      .trim()
      .split("\n")
      .map((l) => JSON.parse(l));
  ...
  it("any writer (not just `converge review`) can submit a verdict; runner picks it up", async () => {
    ...
    mkdirSync(join(ws, ".converge", "inventory", "handoff-review", "reports"), {
      recursive: true,
    });
    const entry = {
  ...
  it("a missing handoff artifact invalidates the cache and regenerates", async () => {
    ...
    rmSync(
      join(ws, ".converge", "journal", "handoff-review", "runstate.json"),
      { force: true },
    );
