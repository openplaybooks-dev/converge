...
describe("playbook run lock interruption recovery", () => {
  it("keeps stale run-lock state recoverable by replacing the interrupted lock file", async () => {
    ...
    try {
      await writeFile(
        lockPath,
        JSON.stringify({ pid: 1, startedAt: "interrupted" }),
        { flag: "wx" },
      );
      ...
      await writeFile(
        lockPath,
        JSON.stringify({ pid: process.pid, recoveredFrom: "interrupted" }),
        { flag: "wx" },
      );

      await expect(
        writeFile(lockPath, "second concurrent run", { flag: "wx" }),
  ...
  it("does not accept cached completion when a declared output is missing", async () => {
    ...
    try {
      ...
      await writeFile(
        join(root, ".converge", "project.yaml"),
...
`,
      ...
      const runstate = JSON.parse(
        await readFile(
          join(root, ".converge", "journal", "default", "runstate.json"),
          "utf8",
        ),
      );
