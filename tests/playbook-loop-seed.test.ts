...
describe("loop CLI seed driver", () => {
  ...
  beforeAll(() => {
    ...
    writeFileSync(
      join(playbookDir, "tasks/improve/TASK.md"),
  ...
  it("compiles an incremental loop driver that uses CLI seed mode", () => {
    ...
    const manifest = JSON.parse(
      readFileSync(
        join(projectDir, ".converge/journal/default/manifest.json"),
        "utf8",
      ),
    );
    expect(manifest.nodes.improve).toBeDefined();
    expect(
      existsSync(join(projectDir, ".converge/journal/default/manifest.json")),
    ).toBe(true);
  });
});
