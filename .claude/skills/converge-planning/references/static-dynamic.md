...
**Dynamic subtasks** use runtime templates. The parent task body emits `converge spawn template --path <template-path> --id <child-id>` (or `converge spawn task --id <child-id> --task-file <pre-rendered.md>`) for each child. Two sub-cases:
