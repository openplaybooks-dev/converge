...
**The TASK.md body is the converge prompt.** Decomposition is handled either by static children under `tasks/` or by a `mode: spawner` body that writes `$CONVERGE_TASK_DIR/spawn.plan.jsonl`. For the converging parent, the body contains only convergence instructions — what to read, how to integrate, what to validate. It runs after children complete.
