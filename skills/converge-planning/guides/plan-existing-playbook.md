...
Add tasks to, modify, or extend an existing converge playbook based on the user's prompt. Preserve existing structure and progress.
...
**Outputs:**
- Updated or new TASK.md files in `.converge/playbooks/{name}/tasks/`
...
find .converge/playbooks/{name}/tasks -name "TASK.md" -exec head -1 {} \;

# Verify no duplicate task IDs
find .converge/playbooks/{name}/tasks -name "TASK.md" -exec grep "^id:" {} \; | sort | uniq -d
```
