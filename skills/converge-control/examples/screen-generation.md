...
```yaml
# .converge/epics/03-build-screens/TASK.md
---
title: Build Screens
dependencies:
...
```js
// .converge/epics/03-build-screens/wbs.js
...
1. Converge reads the parent TASK.md, sees `wbs:` config
2. Runs `wbs.js` — script calls `ctx.spawn()` per screen
3. Converge writes child TASK.md files to disk:
   ```
   03-build-screens/
   ├── TASK.md              ← parent
   ├── wbs.js               ← script
...
- Converge writes child TASK.md files to disk
...
cat .converge/journal/.checkpoint.json | jq '.failedTasks'

# Fix failed subtask
converge reset 003-002
converge run --step 003-002
```
...
cat .converge/journal/epics/02-ux-ui-design/tasks/003-generate-screen-prompts/attempts/wip/log.log

# Look for spawn errors
grep -i "spawn" log.log

# Verify subtask files created
ls -la .converge/epics/02-ux-ui-design/003-generate-screen-prompts/
```
...
converge status | grep 001-create-ux-overview

# Check if file exists
ls -la .stitch/screens-plan.json

# If missing, check upstream logs
cat .converge/journal/epics/02-ux-ui-design/tasks/001-create-ux-overview/attempts/wip/log.log
```
...
export async function run(ctx) {
  ...
  for (let idx = 0; idx < screens.length; idx++) {
    ...
    const basePath = `.converge/epics/03-build-screens/tasks/${screenTaskId}`;
...
4. Converge writes child TASK.md files and executes them
