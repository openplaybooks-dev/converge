...
```
.converge/epics/03-build-screens/
...
The converge writes child TASK.md files automatically — the script does NOT write files itself.
...
Throw errors with clear messages. The converge captures them and can attempt self-repair:
...
1. Converge reads parent TASK.md, sees `wbs:` config
2. Runs `wbs.js` — script calls `ctx.spawn()` per child
3. Converge writes child TASK.md files to disk
4. Converge records spawn results in `wbs.json` in the journal
