...
> **For comprehensive upfront planning** (fresh project, full analysis + discovery + plan creation), use the **`converge-planning`** skill instead. This playbook covers the mechanics of creating individual epics and tasks within an existing plan.
...
```yaml
# .converge/epics/{epic}/tasks/001-create-ux/TASK.md
---
...
### After Planning
- [ ] Visualize with `converge tree`
- [ ] Check for circular deps with `converge verify`
...
```bash
converge tree                # See task tree
converge verify              # Check for issues
converge tree 02-design      # View specific epic
converge run --step --dry    # Preview execution order
```
