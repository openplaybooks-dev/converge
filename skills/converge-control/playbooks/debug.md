...
| "It works on my machine" | Converge runs checks from project root. Check your working directory. |
...
|---------|---------------------------|-------------|--------------|
| "Task not found" | File path wrong | `find .converge/epics -name "TASK.md"` | Check path matches `{epic}/{task}/TASK.md` |
...
| "Blocked by deps" | Upstream task incomplete/failed | `converge tree \| grep {taskId}` | Fix upstream first |
| "Skill invocation failed" | Skill doesn't exist OR outputs not created | `ls .converge/skills/{name}/SKILL.md` | Verify skill outputs |
| "Nothing runs" | All complete OR all blocked OR all failed | `converge status` + `converge tree` | Check checkpoint.json |
...
```bash
# 1. What failed?
converge status --failed

# 2. Why did it fail? (READ THIS)
cat .converge/journal/epics/{epic}/tasks/{task}/attempts/wip/LEARN.md

# 3. What was the actual error?
cat .converge/journal/epics/{epic}/tasks/{task}/attempts/wip/log.log | tail -50

# 4. Which check failed?
cat .converge/journal/epics/{epic}/tasks/{task}/attempts/wip/CHECK.result.md

# 5. What was the task trying to do?
cat .converge/epics/{epic}/{task}/TASK.md
```
...
# Run from project root (where converge runs it)
cd /absolute/path/to/project
$check_cmd && echo "✅ PASS" || echo "❌ FAIL (exit code: $?)"

# If fails but file exists → check command is wrong
# If passes manually but fails in converge → working dir issue
```
...
diff .converge/journal/epics/{epic}/tasks/{task}/attempts/01/LEARN.md \
     .converge/journal/epics/{epic}/tasks/{task}/attempts/wip/LEARN.md
...
# 2. If check passes manually but fails in converge:
...
grep -r "\.outputs.*{input-file}" .converge/epics/*/*/TASK.md

# Is that task complete?
cat .converge/journal/.checkpoint.json | jq '.completedTasks' | grep {upstream-task}

# If not complete → run upstream first
converge run --step {upstream-epic}/{upstream-task}
...
converge tree | grep -A5 {taskId}

# Find root tasks (no dependencies)
grep -L "dependencies" .converge/epics/*/tasks/*/TASK.md
```
...
grep "Spawning subtask" .converge/journal/epics/{epic}/tasks/{parent}/attempts/wip/log.log

# If yes → check if children were created
find .converge/epics/{epic}/{parent} -name "TASK.md" -type f
...
cat .converge/journal/epics/{epic}/tasks/{task}/attempts/wip/TASK.md
...
diff .converge/journal/epics/{epic}/tasks/{task}/attempts/01/TASK.md \
     .converge/journal/epics/{epic}/tasks/{task}/attempts/wip/TASK.md
...
vim .converge/epics/{epic}/{task}/TASK.md  # Fix .checks([...])
converge reset {taskId} && converge run --step
```
...
vim .converge/epics/{epic}/{task}/TASK.md
converge reset {taskId} --outputs  # Clean old outputs
converge run --step
```
...
find .converge/journal/epics -name "wip" -type d -exec rm -rf {} + 2>/dev/null
converge run --step  # Start completely fresh
```
...
```bash
# 1. Verify converge can find tasks
find .converge/epics -name "TASK.md" -type f | wc -l  # Should show N tasks

# 2. Verify checkpoint is readable
cat .converge/journal/.checkpoint.json | jq .  # Should show valid JSON

# 3. Verify task definition is valid TypeScript
cd .converge/epics/{epic}/{task}
node -c TASK.md  # Check for syntax errors

# 4. Check file permissions
ls -la .converge/journal/  # Should be writable
...
❌ **Wrong:** `converge reset` → run again → fails again
...
- [ ] Task runs successfully: `converge run --step {taskId}` completes
