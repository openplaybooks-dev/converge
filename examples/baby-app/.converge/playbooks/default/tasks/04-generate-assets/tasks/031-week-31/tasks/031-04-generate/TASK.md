---
id: 031-04-generate
title: "Generate — baby-size: Week 31"
...
tags:
  ...
  - baby-size
inputs:
  - .converge/playbooks/default/tasks/04-generate-assets/tasks/031-week-31/SPEC.md
...
checks:
  - id: svg-exists
    ...
    cmd: test -f assets/illustrations/baby-sizes/week-31.svg
  - id: svg-valid
    description: File contains valid SVG markup
    cmd: "head -5 assets/illustrations/baby-sizes/week-31.svg | grep -q '<svg'"
  - id: svg-size-reasonable
    description: "SVG file size is reasonable (not empty, not huge)"
    cmd: "stat -f%z assets/illustrations/baby-sizes/week-31.svg 2>/dev/null | awk '{if ($1 > 100 && $1 < 500000) exit 0; exit 1}'"
vars:
...
Create `assets/illustrations/baby-sizes/week-31.svg`:
