...
checks:
  - id: no-empty-handlers
    cmd: "node .converge/playbooks/default/tasks/06-wire-screens/check-all-handlers.mjs"
...
2. **Analyze Navigations** — Scan every screen, extract all interactive elements, produce `navigations.json` manifest
3. **Wire Per Screen** — WBS: one parent per screen → one child per broken handler, each with node-based check
4. **Verify** — End-to-end check with node script that scans all files for empty handlers

Checks use `check-handler.mjs` (per-element) and `check-all-handlers.mjs` (global) instead of grep to avoid macOS compatibility issues and catch multiline empty handlers.
