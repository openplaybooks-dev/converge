---
id: 006-01-spec
title: "Spec: Due Date Picker"
description: Generate Due Date Picker overlay specification
tags:
  - spec
  - overlay-due-date-picker
inputs:
  - .stitch/system/DESIGN.md
  - .stitch/UX.md
  - .stitch/screens.json
  - lib/screens/settings/settings_screen.dart
outputs:
  - .stitch/designs/due-date-picker/SPEC.md
checks:
  - id: spec-exists
    description: SPEC.md exists for due-date-picker
    cmd: test -f .stitch/designs/due-date-picker/SPEC.md
  - id: spec-has-content
    description: "SPEC.md has >30 lines"
    cmd: test $(wc -l < .stitch/designs/due-date-picker/SPEC.md) -gt 30
plan:
vars:
  references: ["flutter-building-layouts"]
  prefix: 006
  overlayId: due-date-picker
  title: Due Date Picker
  widgetName: DueDatePicker
  snakeName: due_date_picker
  overlayTaskId: 006-due-date-picker
  parentScreenId: settings
  parentScreenPath: lib/screens/settings/settings_screen.dart
  overlayType: dialog
  specPath: .stitch/designs/due-date-picker/SPEC.md
  metaPath: .stitch/designs/due-date-picker/META.md
  designPath: .stitch/designs/due-date-picker/design.html
  widgetPath: lib/widgets/overlays/due_date_picker/due_date_picker.dart
---

# Spec: Due Date Picker

Generate the overlay specification for **Due Date Picker** (`due-date-picker`).

## Inputs
- `.stitch/system/DESIGN.md` — Design system
- `.stitch/UX.md` — UX overview
- `.stitch/screens.json` — Screen and overlay definitions
- `lib/screens/settings/settings_screen.dart` — Parent screen (check for existing trigger placeholders)

## Context

This is an **overlay** (dialog), not a full screen. It is triggered from the **settings** screen. Overlays are presented using `showModalBottomSheet()`, `showDialog()`, or persistent bar patterns — never via GoRouter navigation.

**IMPORTANT:** Read the parent screen file (`lib/screens/settings/settings_screen.dart`) — the `03-build-screens` pipeline typically generates screens with placeholder triggers (e.g., `Placeholder()` builder, `SnackBar` stub, `debugPrint` stub) where overlays should be wired. Document the existing trigger location and placeholder pattern in the spec so the `05-mount` step knows exactly what to replace.

## Task

Read inputs and produce `.stitch/designs/due-date-picker/SPEC.md` containing:

1. **Overlay Title** — Due Date Picker
2. **Overlay Type** — dialog (bottom-sheet | dialog | persistent-bar)
3. **Parent Screen** — settings
4. **Trigger** — What user action opens this overlay (e.g., tap button, long press)
5. **Purpose** — What this overlay does and why it exists
6. **Widget Name** — `DueDatePicker`
7. **Design Tokens** — Colors, typography, spacing from DESIGN.md
8. **Layout** — Container structure:
   - For bottom-sheet: drag handle, content area, action buttons, max height
   - For dialog: title bar, content, action row, max width
   - For persistent-bar: height, background, content layout
9. **Sections** — Each visual section with:
   - Description of content
   - Widget type (ListView, Column, Wrap, etc.)
   - Interactive elements (buttons, toggles, inputs)
10. **Data** — Entities and fields displayed/edited
11. **Dismiss Behavior** — How the overlay closes (swipe down, tap outside, confirm button, cancel)
12. **Return Value** — What data (if any) the overlay passes back to the parent when dismissed
13. **Accessibility** — Semantics labels, focus trapping, screen reader announcements

## Success Criteria

- `.stitch/designs/due-date-picker/SPEC.md` exists and has >30 lines
- All required sections present
- Overlay type and parent screen clearly identified
- Trigger and dismiss behavior documented
- Design tokens reference DESIGN.md values
