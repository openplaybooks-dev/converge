---
id: 002-05-mount
title: "Mount: Weight Entry"
description: Mount Weight Entry overlay in parent screen and wire trigger
dependencies:
  - 002-04-connect
tags:
  - mount
...
checks:
  - id: parent-imports-overlay
    ...
    cmd: "grep -q 'weight_entry' lib/screens/weight_nutrition/weight_nutrition_screen.dart"
  - id: parent-shows-overlay
    description: Parent screen calls showModalBottomSheet or showDialog
    cmd: "grep -qE 'showModalBottomSheet|showDialog' lib/screens/weight_nutrition/weight_nutrition_screen.dart"
  - id: dart-valid
    description: Dart analysis passes for parent screen
    cmd: dart analyze lib/screens/weight_nutrition/weight_nutrition_screen.dart
vars:
  prefix: 002
...
Mount the **Weight Entry** overlay in its parent screen (**weight-nutrition**) and wire the trigger.

## Inputs
- `lib/widgets/overlays/weight_entry/weight_entry.dart` — The overlay widget file
- `.stitch/designs/weight-entry/SPEC.md` — Overlay specification (describes trigger and dismiss behavior)
- `lib/screens/weight_nutrition/weight_nutrition_screen.dart` — The parent screen file to modify
...
3. **Add import** — Add the overlay widget import to the parent screen (use the project's package name from `pubspec.yaml`):
   ```dart
   import 'package:<pkg>/widgets/overlays/weight_entry/weight_entry.dart';
   ```
...
5. **Verify** — Run `dart analyze lib/screens/weight_nutrition/weight_nutrition_screen.dart`
...
- `lib/screens/weight_nutrition/weight_nutrition_screen.dart` updated with overlay import
