---
id: 007-03-convert
title: "Convert: Delete Entry Confirmation"
description: Convert constrained HTML design to Flutter widget for Delete Entry Confirmation overlay
dependencies:
  - 007-02-design
tags:
  - convert
...
checks:
  - id: widget-exists
    ...
    cmd: test -f lib/widgets/overlays/delete_entry/delete_entry.dart
  - id: dart-valid
    description: Dart analysis passes
    cmd: dart analyze lib/widgets/overlays/delete_entry/delete_entry.dart
  - id: uses-theme
    description: Uses Theme.of(context) for styling
    cmd: "grep -q 'Theme.of(context)' lib/widgets/overlays/delete_entry/delete_entry.dart"
  - id: no-hardcoded-colors
    description: No hardcoded colors — uses colorScheme
    cmd: "! grep -qE 'Color\\(0x|Colors\\.' lib/widgets/overlays/delete_entry/delete_entry.dart"
  - id: no-router-registration
    description: Overlay does NOT register a GoRoute
    cmd: "! grep -q 'GoRoute' lib/widgets/overlays/delete_entry/delete_entry.dart"
vars:
...
# Convert: Delete Entry Confirmation

Convert the constrained HTML design to a Flutter widget using the **stitch-flutter** skill.

## Inputs
- `.stitch/designs/delete-entry/design.html` — HTML design mockup (written in Flutter HTML Glossary vocabulary)
...
1. **Read HTML** — Parse `.stitch/designs/delete-entry/design.html` and extract:
...
3. **Generate Dart** — Write `lib/widgets/overlays/delete_entry/delete_entry.dart`:
   ```dart
...
5. **Verify** — Run `dart analyze lib/widgets/overlays/delete_entry/delete_entry.dart`
...
- `lib/widgets/overlays/delete_entry/delete_entry.dart` exists with valid Dart
