/**
 * WBS: Scientific Research — Epoch Spawner
 *
...
export async function run(ctx) {
  ...
  const artifactsDir = join(ctx.projectDir, '.converge', 'artifacts', 'scientific-research', 'epochs', epochStr);
  ...
  await ctx.spawn(
    { _type: 'template-ref', path: templatePath, vars: {
      ...
      targetScore: ctx.inputs?.targetScore ?? '70',
    }},
    { id: epochId },
  );
}
