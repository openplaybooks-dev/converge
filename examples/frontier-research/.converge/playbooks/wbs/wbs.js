/**
 * WBS: Frontier Research — Epoch Spawner
 *
 * Finds the next epoch number by scanning existing epoch task directories,
 * then spawns a new epoch from the template. Each loop iteration runs
 * one epoch (research iteration).
 */
...
export async function run(ctx) {
  ...
  const artifactsDir = join(ctx.projectDir, '.converge', 'artifacts', 'frontier-research', 'epochs', epochStr);
  ...
  await ctx.spawn(
    { _type: 'template-ref', path: templatePath, vars: {
      ...
      beamWidth: ctx.inputs?.beamWidth ?? '5',
      selectionWidth: ctx.inputs?.selectionWidth ?? '2',
      convergenceThreshold: ctx.inputs?.convergenceThreshold ?? '0.15',
    }},
    { id: epochId },
  );
}
