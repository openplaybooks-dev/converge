...
description: Generate constrained HTML design for Settings using Flutter HTML Glossary
dependencies:
  - 009-01-spec
tags:
  - design
...
checks:
  - id: design-exists
    ...
    cmd: test -f .stitch/designs/settings/design.html
  - id: meta-exists
    description: META.md exists for settings
    cmd: test -f .stitch/designs/settings/META.md
  - id: uses-glossary
    description: HTML uses Flutter HTML Glossary vocabulary
    cmd: "grep -q 'class=\"scaffold\"' .stitch/designs/settings/design.html"
  - id: has-data-attributes
    description: "HTML uses data-* attributes for Flutter conversion"
    cmd: "grep -q 'data-color=' .stitch/designs/settings/design.html"
vars:
...
Generate the HTML design mockup for **Settings** using the **Flutter HTML Glossary** constrained vocabulary.
...
Path from `screens.json` for this screen: **`.stitch/references/settings/code.html`**

- When **non-empty** and the file exists: that file is the **fidelity target**. `.stitch/designs/settings/design.html` MUST reproduce the same structure, section order, hierarchy, spacing rhythm, and visible text/icons as that reference **as closely as the Flutter HTML Glossary allows**. Do not invent alternate layouts. Where the glossary has no one-to-one element, pick the closest glossary mapping and note the compromise in `.stitch/designs/settings/META.md`.
- When **empty** or missing: infer structure from `.stitch/designs/settings/SPEC.md`, `ANALYSIS.md`, and example selection per below.

## Inputs
- `.stitch/designs/settings/SPEC.md` — Screen specification
- `.stitch/system/DESIGN.md` — Design system
- `.stitch/system/META.md` — Reference examples metadata
- **Linked HTML** — `.stitch/references/settings/code.html` when set (see above)

## Steps

1. **Read spec** — Load `.stitch/designs/settings/SPEC.md` to understand layout, sections, data
2. **Read glossary** — Load `stitch-flutter/references/flutter-html-glossary.md` to understand the constrained vocabulary
3. **Linked HTML first** — If `.stitch/references/settings/code.html` is non-empty and the file exists, read it completely. Treat it as the visual ground truth for `design.html` (subject to glossary-only elements). Skip browsing other `code.html` files for layout unless the linked file is unusable.
...
7. **Generate META.md** — Write `.stitch/designs/settings/META.md` with example selection, scoring table, and (when used) explicit note that `design.html` was matched to `.stitch/references/settings/code.html`
8. **Generate design.html** — Write `.stitch/designs/settings/design.html` following `stitch-generate` skill using ONLY glossary elements; **pixel-fidelity to the linked reference HTML** when `htmlReference` is present and valid
...
- `.stitch/designs/settings/META.md` — Example selection and scoring; document linked-HTML fidelity when applicable
- `.stitch/designs/settings/design.html` — Self-contained HTML mockup at 375px using glossary vocabulary; when `htmlReference` is set and valid, structure and content mirror that file within glossary constraints
