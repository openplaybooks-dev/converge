...
export async function run(ctx) {
  ...
  const shared = widgets.filter(c => c.shared);
  const toLift = shared.length > 0 ? shared : widgets;

  if (toLift.length === 0) return;

  const snakeName = (name) => name.replace(/([a-z])([A-Z])/g, '$1_$2').toLowerCase();

  for (let i = 0; i < toLift.length; i++) {
    const widget = toLift[i];
