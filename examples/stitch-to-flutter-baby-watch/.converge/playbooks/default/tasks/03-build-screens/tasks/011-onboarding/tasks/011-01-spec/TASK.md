...
description: Generate Onboarding screen specification
tags:
  - spec
  - screen-onboarding
inputs:
...
checks:
  - id: spec-exists
    ...
    cmd: test -f .stitch/designs/onboarding/SPEC.md
  - id: spec-has-content
    description: "SPEC.md has >50 lines"
    cmd: test $(wc -l < .stitch/designs/onboarding/SPEC.md) -gt 50
plan:
vars:
...
Generate the screen specification for **Onboarding** (`/onboarding`).

## Inputs
- `.stitch/system/DESIGN.md` — Design system
- `.stitch/UX.md` — UX overview
- `.stitch/screens.json` — Screen definitions (includes `htmlReference` for this screen: `.stitch/references/babyguard_onboarding_phase_2/code.html`)
...
Value from `.stitch/screens.json` for this screen: **`.stitch/references/babyguard_onboarding_phase_2/code.html`**
...
Read inputs and produce `.stitch/designs/onboarding/SPEC.md` containing:

1. **Screen Title** — Onboarding
2. **Purpose** — What this screen does and why
3. **Route** — `/onboarding`
4. **Widget Name** — `OnboardingScreen`
...
- `.stitch/designs/onboarding/SPEC.md` exists and has >50 lines
