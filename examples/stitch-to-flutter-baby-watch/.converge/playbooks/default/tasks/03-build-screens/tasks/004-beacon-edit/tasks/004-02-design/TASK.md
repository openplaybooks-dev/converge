---
id: 004-02-design
title: "Design: Edit Beacon"
description: Generate constrained HTML design for Edit Beacon using Flutter HTML Glossary
dependencies:
  - 004-01-spec
tags:
  - design
...
outputs:
  - .stitch/designs/beacon-edit/META.md
  - .stitch/designs/beacon-edit/design.html
checks:
  - id: design-exists
    description: design.html exists for beacon-edit
    cmd: test -f .stitch/designs/beacon-edit/design.html
  - id: meta-exists
    description: META.md exists for beacon-edit
    cmd: test -f .stitch/designs/beacon-edit/META.md
  - id: uses-glossary
    description: HTML uses Flutter HTML Glossary vocabulary
    cmd: "grep -q 'class=\"scaffold\"' .stitch/designs/beacon-edit/design.html"
  - id: has-data-attributes
    description: "HTML uses data-* attributes for Flutter conversion"
    cmd: "grep -q 'data-color=' .stitch/designs/beacon-edit/design.html"
vars:
...
Generate the HTML design mockup for **Edit Beacon** using the **Flutter HTML Glossary** constrained vocabulary.
...
- When **non-empty** and the file exists: that file is the **fidelity target**. `.stitch/designs/beacon-edit/design.html` MUST reproduce the same structure, section order, hierarchy, spacing rhythm, and visible text/icons as that reference **as closely as the Flutter HTML Glossary allows**. Do not invent alternate layouts. Where the glossary has no one-to-one element, pick the closest glossary mapping and note the compromise in `.stitch/designs/beacon-edit/META.md`.
- When **empty** or missing: infer structure from `.stitch/designs/beacon-edit/SPEC.md`, `ANALYSIS.md`, and example selection per below.
...
1. **Read spec** — Load `.stitch/designs/beacon-edit/SPEC.md` to understand layout, sections, data
...
7. **Generate META.md** — Write `.stitch/designs/beacon-edit/META.md` with example selection, scoring table, and (when used) explicit note that `design.html` was matched to ``
8. **Generate design.html** — Write `.stitch/designs/beacon-edit/design.html` following `stitch-generate` skill using ONLY glossary elements; **pixel-fidelity to the linked reference HTML** when `htmlReference` is present and valid
...
- `.stitch/designs/beacon-edit/META.md` — Example selection and scoring; document linked-HTML fidelity when applicable
- `.stitch/designs/beacon-edit/design.html` — Self-contained HTML mockup at 375px using glossary vocabulary; when `htmlReference` is set and valid, structure and content mirror that file within glossary constraints
