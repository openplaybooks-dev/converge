---
id: 006-02-design
title: "Design: Add Safe Zone"
description: Generate constrained HTML design for Add Safe Zone using Flutter HTML Glossary
dependencies:
  - 006-01-spec
tags:
  - design
...
checks:
  - id: design-exists
    ...
    cmd: test -f .stitch/designs/add-safe-zone/design.html
  - id: meta-exists
    description: META.md exists for add-safe-zone
    cmd: test -f .stitch/designs/add-safe-zone/META.md
  - id: uses-glossary
    description: HTML uses Flutter HTML Glossary vocabulary
    cmd: "grep -q 'class=\"scaffold\"' .stitch/designs/add-safe-zone/design.html"
  - id: has-data-attributes
    description: "HTML uses data-* attributes for Flutter conversion"
    cmd: "grep -q 'data-color=' .stitch/designs/add-safe-zone/design.html"
vars:
...
# Design: Add Safe Zone

Generate the HTML design mockup for **Add Safe Zone** using the **Flutter HTML Glossary** constrained vocabulary.
...
- When **non-empty** and the file exists: that file is the **fidelity target**. `.stitch/designs/add-safe-zone/design.html` MUST reproduce the same structure, section order, hierarchy, spacing rhythm, and visible text/icons as that reference **as closely as the Flutter HTML Glossary allows**. Do not invent alternate layouts. Where the glossary has no one-to-one element, pick the closest glossary mapping and note the compromise in `.stitch/designs/add-safe-zone/META.md`.
- When **empty** or missing: infer structure from `.stitch/designs/add-safe-zone/SPEC.md`, `ANALYSIS.md`, and example selection per below.

## Inputs
- `.stitch/designs/add-safe-zone/SPEC.md` — Screen specification
...
1. **Read spec** — Load `.stitch/designs/add-safe-zone/SPEC.md` to understand layout, sections, data
...
7. **Generate META.md** — Write `.stitch/designs/add-safe-zone/META.md` with example selection, scoring table, and (when used) explicit note that `design.html` was matched to ``
8. **Generate design.html** — Write `.stitch/designs/add-safe-zone/design.html` following `stitch-generate` skill using ONLY glossary elements; **pixel-fidelity to the linked reference HTML** when `htmlReference` is present and valid
...
- `.stitch/designs/add-safe-zone/META.md` — Example selection and scoring; document linked-HTML fidelity when applicable
- `.stitch/designs/add-safe-zone/design.html` — Self-contained HTML mockup at 375px using glossary vocabulary; when `htmlReference` is set and valid, structure and content mirror that file within glossary constraints
