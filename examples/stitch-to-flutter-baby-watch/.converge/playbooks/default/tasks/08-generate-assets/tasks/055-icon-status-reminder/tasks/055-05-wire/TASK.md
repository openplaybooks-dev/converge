---
id: 055-05-wire
title: "Wire — icon: Reminder{{stateName}}"
...
inputs:
  ...
  - assets/icons/status-reminder.svg.meta.json
outputs:
  - lib/widgets/assets/status-reminder_asset.dart
checks:
  - id: widget-exists
    description: Asset widget was created
    cmd: test -f lib/widgets/assets/status-reminder_asset.dart
  - id: code-uses-asset
    description: Asset is referenced in codebase
    cmd: "grep -rq 'status-reminder' lib/ --include='*.dart' 2>/dev/null | head -1"
  - id: dart-valid
    description: Generated widget code is valid
    cmd: "dart analyze lib/widgets/assets/status-reminder_asset.dart 2>/dev/null || true"
vars:
...
Create `lib/widgets/assets/status-reminder_asset.dart`:
...
/// 
/// Reminder icon for status usage.
