---
id: 050-04-generate
title: "Generate — icon: Bookmark{{stateName}}"
...
tags:
  ...
  - icon
inputs:
  - .converge/playbooks/default/tasks/08-generate-assets/tasks/050-icon-action-bookmark/SPEC.md
...
checks:
  - id: svg-exists
    ...
    cmd: test -f assets/icons/action-bookmark.svg
  - id: svg-valid
    description: File contains valid SVG markup
    cmd: "head -5 assets/icons/action-bookmark.svg | grep -q '<svg'"
  - id: svg-size-reasonable
    description: "SVG file size is reasonable (not empty, not huge)"
    cmd: "stat -f%z assets/icons/action-bookmark.svg 2>/dev/null | awk '{if ($1 > 100 && $1 < 500000) exit 0; exit 1}'"
vars:
...
Create a 24x24 SVG icon for "Bookmark":
...
Create `assets/icons/action-bookmark.svg`:
