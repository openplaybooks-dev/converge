---
id: 052-05-wire
title: "Wire — icon: Weight{{stateName}}"
...
tags:
  ...
  - icon
inputs:
  - assets/icons/status-weight.svg
  - assets/icons/status-weight.svg.meta.json
outputs:
  - lib/widgets/assets/status-weight_asset.dart
checks:
  - id: widget-exists
    description: Asset widget was created
    cmd: test -f lib/widgets/assets/status-weight_asset.dart
  - id: code-uses-asset
    description: Asset is referenced in codebase
    cmd: "grep -rq 'status-weight' lib/ --include='*.dart' 2>/dev/null | head -1"
  - id: dart-valid
    description: Generated widget code is valid
    cmd: "dart analyze lib/widgets/assets/status-weight_asset.dart 2>/dev/null || true"
vars:
...
Create `lib/widgets/assets/status-weight_asset.dart`:
