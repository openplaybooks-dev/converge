---
id: 044-05-wire
title: "Wire — icon: Wellness{{stateName}}"
...
tags:
  ...
  - icon
inputs:
  - assets/icons/nav-wellness.svg
  - assets/icons/nav-wellness.svg.meta.json
outputs:
  - lib/widgets/assets/nav-wellness_asset.dart
checks:
  - id: widget-exists
    description: Asset widget was created
    cmd: test -f lib/widgets/assets/nav-wellness_asset.dart
  - id: code-uses-asset
    description: Asset is referenced in codebase
    cmd: "grep -rq 'nav-wellness' lib/ --include='*.dart' 2>/dev/null | head -1"
  - id: dart-valid
    description: Generated widget code is valid
    cmd: "dart analyze lib/widgets/assets/nav-wellness_asset.dart 2>/dev/null || true"
vars:
...
Create `lib/widgets/assets/nav-wellness_asset.dart`:
...
/// 
/// Wellness icon for navigation usage.
