---
id: 018-04-generate
title: "Generate — baby-size: Week 18"
...
checks:
  - id: svg-exists
    ...
    cmd: test -f assets/illustrations/baby-sizes/week-18.svg
  - id: svg-valid
    description: File contains valid SVG markup
    cmd: "head -5 assets/illustrations/baby-sizes/week-18.svg | grep -q '<svg'"
  - id: svg-size-reasonable
    description: "SVG file size is reasonable (not empty, not huge)"
    cmd: "stat -f%z assets/illustrations/baby-sizes/week-18.svg 2>/dev/null | awk '{if ($1 > 100 && $1 < 500000) exit 0; exit 1}'"
vars:
...
Create `assets/illustrations/baby-sizes/week-18.svg`:
