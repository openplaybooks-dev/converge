---
id: 053-04-generate
title: "Generate — icon: Cycle{{stateName}}"
...
tags:
  ...
  - icon
inputs:
  - .converge/playbooks/default/tasks/08-generate-assets/tasks/053-icon-status-cycle/SPEC.md
...
checks:
  - id: svg-exists
    ...
    cmd: test -f assets/icons/status-cycle.svg
  - id: svg-valid
    description: File contains valid SVG markup
    cmd: "head -5 assets/icons/status-cycle.svg | grep -q '<svg'"
  - id: svg-size-reasonable
    description: "SVG file size is reasonable (not empty, not huge)"
    cmd: "stat -f%z assets/icons/status-cycle.svg 2>/dev/null | awk '{if ($1 > 100 && $1 < 500000) exit 0; exit 1}'"
vars:
...
Create a 24x24 SVG icon for "Cycle":
...
Create `assets/icons/status-cycle.svg`:
