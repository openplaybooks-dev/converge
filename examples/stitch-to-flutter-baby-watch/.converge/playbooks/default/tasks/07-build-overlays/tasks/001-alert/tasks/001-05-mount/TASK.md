---
id: 001-05-mount
title: "Mount: Alert Screen"
description: Mount Alert Screen overlay in parent screen and wire trigger
dependencies:
  - 001-04-connect
tags:
  - mount
...
checks:
  - id: parent-imports-overlay
    ...
    cmd: "grep -q 'alert' lib/screens/home/home_screen.dart"
...
Mount the **Alert Screen** overlay in its parent screen (****) and wire the trigger.

## Inputs
- `lib/widgets/overlays/alert/alert.dart` — The overlay widget file
- `.stitch/designs/alert/SPEC.md` — Overlay specification (describes trigger and dismiss behavior)
...
3. **Add import** — Add the overlay widget import to the parent screen (use the project's package name from `pubspec.yaml`):
   ```dart
   import 'package:<pkg>/widgets/overlays/alert/alert.dart';
   ```
