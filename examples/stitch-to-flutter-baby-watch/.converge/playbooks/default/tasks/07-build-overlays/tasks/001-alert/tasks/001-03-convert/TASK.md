---
id: 001-03-convert
title: "Convert: Alert Screen"
description: Convert constrained HTML design to Flutter widget for Alert Screen overlay
dependencies:
  - 001-02-design
tags:
  - convert
...
checks:
  - id: widget-exists
    ...
    cmd: test -f lib/widgets/overlays/alert/alert.dart
  - id: dart-valid
    description: Dart analysis passes
    cmd: dart analyze lib/widgets/overlays/alert/alert.dart
  - id: uses-theme
    description: Uses Theme.of(context) for styling
    cmd: "grep -q 'Theme.of(context)' lib/widgets/overlays/alert/alert.dart"
  - id: no-hardcoded-colors
    description: No hardcoded colors — uses colorScheme
    cmd: "! grep -qE 'Color\\(0x|Colors\\.' lib/widgets/overlays/alert/alert.dart"
  - id: no-router-registration
    description: Overlay does NOT register a GoRoute
    cmd: "! grep -q 'GoRoute' lib/widgets/overlays/alert/alert.dart"
vars:
...
# Convert: Alert Screen

Convert the constrained HTML design to a Flutter widget using the **stitch-flutter** skill.

## Inputs
- `.stitch/designs/alert/design.html` — HTML design mockup (written in Flutter HTML Glossary vocabulary)
...
1. **Read HTML** — Parse `.stitch/designs/alert/design.html` and extract:
...
3. **Generate Dart** — Write `lib/widgets/overlays/alert/alert.dart`:
   ```dart
...
5. **Verify** — Run `dart analyze lib/widgets/overlays/alert/alert.dart`
...
- `lib/widgets/overlays/alert/alert.dart` exists with valid Dart
