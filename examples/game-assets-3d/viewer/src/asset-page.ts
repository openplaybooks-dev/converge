...
const stripTex = params.get('stripTextures') !== '0'; // default true when faceted=1

const mod = getAsset(slug);
const meta = mod?.meta;

// Faceted toggle helpers — flip materials to flatShading, recompute normals,
// optionally drop texture maps. Re-apply each tick so async glTF meshes
// (loaded after build) get the treatment too.
