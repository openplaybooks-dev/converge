...
// Background overwritten by the scene's World/DayNight widget if it sets one.
scene.background = new THREE.Color(0x6a7a82);

const camera = new THREE.PerspectiveCamera(45, 1, 0.1, 2000);
camera.position.set(60, 35, 60);

// Phase B: lighting comes from the scene's World widget. We keep a tiny
// fallback ambient so a scene with no lighting widget isn't pitch black;
// World-using scenes overpower it via their own AmbientLightWidget.
scene.add(new THREE.AmbientLight(0xffffff, 0.05));

// Ground (always present so shadows have a surface to land on).
