...
/**
 * Preload-time hook: register every animated sheet in the atlas with Phaser.
 *
 * Two atlas shapes supported:
 *   - sheet mode (image-gen path): one packed PNG per state, frame rects
 *     inside it. Loaded as a Phaser spritesheet.
 *   - frames mode (video pipeline): N individual PNGs per state. Each frame
 *     loaded as its own image with key `<sheetKey>:<frameIndex>`.
 *
...
export function loadAtlasAssets(scene, atlas, rootUrl = '..') {
  for (const cat of CATEGORIES) {
    for (const slice of slicesByCategory(atlas, cat)) {
      ...
      if (mode === 'frames' && isAnimated(slice)) {
        slice.frames.forEach((f, i) => {
          scene.load.image(`${key}:${i}`, `${rootUrl}/${f.path}`);
        });
        continue;
      }
...
export function registerAnimations(scene, atlas, frameRate = 8, { yoyo = false } = {}) {
  ...
  for (const cat of CATEGORIES) {
    for (const slice of slicesByCategory(atlas, cat)) {
      ...
      const frameCount = slice.meta.frame_count ?? slice.frames.length ?? 16;
      ...
      const mode = slice.mode || slice.meta?.mode || 'sheet';
      const animFrames = mode === 'frames'
        ? Array.from({ length: frameCount }, (_, i) => ({ key: `${sheet}:${i}` }))
        : scene.anims.generateFrameNumbers(sheet, { start: 0, end: frameCount - 1 });
