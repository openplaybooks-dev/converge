...
The image's only job is to show painters what `tree-cluster-mid` looks like in
...
TOKEN SPEC:
  ...
  biome:        `grassland`
  category:     `background`
  footprint:    3×4 concept tiles (image aspect = 3:4)
  primary fill: `#3F7A4A`
...
- Honor the primary fill color #3F7A4A and the project palette. Use
