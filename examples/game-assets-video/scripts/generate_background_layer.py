...
def main() -> int:
    ...
    print(
        f"[{args.bg_id}] generating {bg.get('parallax_layer', 'mid')} parallax layer at {width}x{height} backend={backend} cost={cost}¢",
        file=sys.stderr,
    )
    with budget.charged(project_root, cost, f"image-{backend}",
                        note=f"{args.bg_id}/background-{bg.get('parallax_layer', 'mid')}"):
