...
def generate_text_from_image(
    ...
    """Send (image refs + prompt) to a text-out Gemini model. Returns the
    generated text. Used by the visual-target planner to derive an ASSETS.md
    table from the reference screenshot + idea brief.
    """
    try:
        from google import genai
        from google.genai import types
    ...
    out_chunks: list[str] = []
    for chunk in client.models.generate_content_stream(model=TEXT_MODEL, contents=contents, config=config):
        if chunk.parts is None:
            continue
        for part in chunk.parts:
