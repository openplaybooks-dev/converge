...
def build_variant_prompt(
    ...
    prompt_data = {
        ...
        "background": "PURE CHROMA GREEN #00FF00 — preserve the flat green background from the base reference exactly.",
        "critical_instructions": [
            "Maintain character identity and design",
            f"Only modify: {edit_description}",
            "Background is PURE FLAT CHROMA GREEN (#00FF00) — every non-character pixel is exactly #00FF00, matching the base reference. Do NOT introduce any other background color, gradient, shadow, ground plane, or environment. The character itself must NOT contain any pure-green areas; if the palette calls for greens, shift them toward yellow-green or blue-green so the chroma key downstream stays clean.",
    ...
    return f"""Edit this character reference to create a pose variation, on a pure chroma-green (#00FF00) background that matches the base image.
...
Background is PURE FLAT CHROMA GREEN (#00FF00) across the entire canvas — match the base reference exactly. Every non-character pixel is exactly #00FF00, with no gradients, shadows, ground plane, or environment. The character itself must NOT contain any pure-green areas; shift any greens in the palette toward yellow-green or blue-green so the chroma key downstream can isolate the background cleanly.
...
def ensure_variant_ref(
    ...
    *,
    ...
    print(f"[{char_id}] generating variant '{pose_name}' at {working_resolution}x{working_resolution} (base={base_ref_path.relative_to(project_root)}) backend={backend} cost={cost}¢", file=sys.stderr)
    with budget.charged(project_root, cost, f"image-{backend}",
                        note=f"{char_id}/variant-{pose_name}"):
