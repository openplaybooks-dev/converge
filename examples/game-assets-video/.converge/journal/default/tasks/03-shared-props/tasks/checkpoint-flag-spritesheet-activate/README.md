# Task Journal: 03-shared-props/checkpoint-flag-spritesheet-activate
...
```bash
  python -c "from PIL import Image; im=Image.open('assets/objects/checkpoint-flag/spritesheets/activate/activate.png'); w,h=im.size; assert w>=512 and h>=256, f'sheet too small: {im.size}'"
  ...
  test -s assets/objects/checkpoint-flag/spritesheets/activate/activate.prompt.txt
```