---
id: bounce-spring-spritesheet-idle
title: Generate Coil Spring idle sprite sheet
description: Idle animation sprite sheet for Coil Spring
tags:
  - prop
  - interactive
  - spritesheet
...
checks:
  - id: prop-spritesheet-png-exists-and-large
    ...
    cmd: "python -c \"from PIL import Image; im=Image.open('assets/objects/bounce-spring/spritesheets/idle/idle.png'); w,h=im.size; assert w>=512 and h>=256, f'sheet too small: {im.size}'\"\n"
  - id: prop-atlas-json-matches-png
    description: "Prop atlas JSON's grid + sheet_size match the PNG (auto-detected layout)"
    cmd: "python -c \"import json; from PIL import Image; a=json.load(open('assets/objects/bounce-spring/spritesheets/idle/idle.atlas.json')); im=Image.open('assets/objects/bounce-spring/spritesheets/idle/idle.png'); m=a['meta']; assert m['cols']>=2 and m['rows']>=1, f\\\"grid too small: {m}\\\"; assert len(a['frames'])==m['cols']*m['rows'], f\\\"frame count != cols*rows: {m}\\\"; assert m['sheet_size']['w']==im.size[0] and m['sheet_size']['h']==im.size[1], f\\\"atlas/sheet size mismatch: atlas={m['sheet_size']} png={im.size}\\\"\"\n"
  - id: prop-prompt-saved
    description: Sibling .prompt.txt exists for debugging
    cmd: test -s assets/objects/bounce-spring/spritesheets/idle/idle.prompt.txt
vars:
  ...
  states: "[\"idle\",\"bounce\"]"
  state_name: idle
  state_description: Idle animation sprite sheet for Coil Spring
---

# Coil Spring idle Sprite Sheet

Runs `scripts/generate_prop_spritesheet.py bounce-spring idle`. **One image-gen call** draws a 4x4 grid (16 frames) on one canvas, so the prop's identity stays consistent across frames.

Outputs land in `assets/objects/bounce-spring/spritesheets/idle/`:
...
To debug, `cat` the prompt or re-run with a different seed: `python scripts/generate_prop_spritesheet.py bounce-spring idle --seed N`.
