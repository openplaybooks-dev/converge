# Task Journal: 03-shared-props/ui-key-slot-spritesheet-idle
...
```bash
  python -c "from PIL import Image; im=Image.open('assets/objects/ui-key-slot/spritesheets/idle/idle.png'); w,h=im.size; assert w>=512 and h>=256, f'sheet too small: {im.size}'"
  ...
  test -s assets/objects/ui-key-slot/spritesheets/idle/idle.prompt.txt
```