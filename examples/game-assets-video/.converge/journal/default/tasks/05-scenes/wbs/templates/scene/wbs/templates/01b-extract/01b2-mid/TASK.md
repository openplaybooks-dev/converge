...
checks:
  ...
  - id: bg-mid-extracted-shape
    cmd: |
      ...
      h, _ = a.shape
      # Mid covers bottom-half-down: top half is mostly transparent
      # (chroma-keyed sky), bottom half is solid mid-distance content.
      top_transparent = (a[:h // 2] == 0).mean()
      bot_opaque = (a[h // 2:] == 255).mean()
      assert top_transparent > 0.40, f'top half of bg-mid only {top_transparent:.1%} transparent — should be mostly chroma so far shows through (got prompt to fill top half with #00FF00)'
      assert bot_opaque > 0.70, f'bottom half of bg-mid only {bot_opaque:.1%} opaque — should be solid mid-distance painting, no chroma gaps'
      # The horizon between chroma and content must be irregular (not a flat row).
      row_solid = ((a == 0).all(axis=1) | (a == 255).all(axis=1)).sum()
      assert row_solid / h <= 0.85, f'{row_solid}/{h} rows are entirely chroma or entirely solid — the horizon should be an irregular silhouette across multiple rows'
      "
    description: bg-mid follows the new stacking contract — top half mostly transparent (chroma keyed), bottom half solid painted content, horizon irregular
