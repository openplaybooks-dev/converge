...
outputs:
  ...
  - "assets/scenes/{{scene_id}}/bg-near.atlas.json"
checks:
  - id: bg-near-stitched-png-exists
    cmd: test -s assets/scenes/{{scene_id}}/bg-near.png
    description: stitched bg-near.png exists
  - id: bg-near-stitched-atlas-exists
    cmd: test -s assets/scenes/{{scene_id}}/bg-near.atlas.json
  ...
  - id: bg-near-stitched-width-matches-target
    cmd: |
      ...
      w, h = Image.open('assets/scenes/{{scene_id}}/bg-near.png').size
...
The stitcher reads every `assets/scenes/{{scene_id}}/bg-near/seg-*.png`, resizes each to a common per-segment width, and feather-blends them onto a wide canvas. Default overlap is 256px (heavier than mid because foreground content is busier).
