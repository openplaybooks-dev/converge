---
id: scene-forest-tutorial-02c-background-02c-bg-near-seg-04
title: "Scene `forest-tutorial` — bg-near segment 04/8"
...
checks:
  - id: bg-near-seg-png-exists
    ...
    cmd: test -s assets/scenes/forest-tutorial/bg-near/seg-003.png
...
vars:
  ...
  seg_ordinal: 04
  seg_count: 8
  seg_index_padded: 003
  seg_prev_padded: 002
  seg_x_lo_norm: 0.3279
  seg_x_hi_norm: 0.4508
  section_label: water-pit-crossing → first-jump-challenge
  section_kind: hazard → platform-up
  prev_input_path: assets/scenes/forest-tutorial/bg-near/seg-002.png
---

# Scene `forest-tutorial` — bg-near segment 04/8
...
```bash
python scripts/generate_bg_layer_segment.py forest-tutorial near 3 8 \
  --x-lo 0.3279 --x-hi 0.4508 \
  --section-label "water-pit-crossing → first-jump-challenge" --section-kind "hazard → platform-up"
```

This segment owns the **`water-pit-crossing → first-jump-challenge`** section (x_norm `[0.3279, 0.4508]`). Section boundaries come from `stage.json[beats[]]` so each segment is one beat-bounded slice of the gameplay rhythm, not an arbitrary fraction of total width.
