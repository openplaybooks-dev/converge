---
id: scene-forest-tutorial-02c-background-02c-bg-near-seg-05
title: "Scene `forest-tutorial` — bg-near segment 05/8"
...
vars:
  ...
  seg_ordinal: 05
  seg_count: 8
  seg_index_padded: 004
  seg_prev_padded: 003
  seg_x_lo_norm: 0.4508
  seg_x_hi_norm: 0.6148
  section_label: first-jump-challenge → optional-ledge-access
  section_kind: platform-up → platform-up
  prev_input_path: assets/scenes/forest-tutorial/bg-near/seg-003.png
---

# Scene `forest-tutorial` — bg-near segment 05/8
...
```bash
python scripts/generate_bg_layer_segment.py forest-tutorial near 4 8 \
  --x-lo 0.4508 --x-hi 0.6148 \
  --section-label "first-jump-challenge → optional-ledge-access" --section-kind "platform-up → platform-up"
```

This segment owns the **`first-jump-challenge → optional-ledge-access`** section (x_norm `[0.4508, 0.6148]`). Section boundaries come from `stage.json[beats[]]` so each segment is one beat-bounded slice of the gameplay rhythm, not an arbitrary fraction of total width.
