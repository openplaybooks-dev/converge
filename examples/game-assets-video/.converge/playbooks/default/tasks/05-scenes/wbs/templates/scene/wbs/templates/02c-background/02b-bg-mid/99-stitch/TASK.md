...
description: "Concatenate every bg-mid/seg-NNN.png into one wide bg-mid.png. Each seam between adjacent segments is filled by an AI-inpainted bridge that sees both sides at once — eliminates the visible seam jumps. Cost: 1 stitch + (N-1) inpaint calls; for an 8-segment layer, ~35¢ extra."
cost_cents: 35
inputs:
...
outputs:
  ...
  - "assets/scenes/{{scene_id}}/bg-mid/final.atlas.json"
checks:
  - id: bg-mid-stitched-png-exists
    cmd: test -s assets/scenes/{{scene_id}}/bg-mid/final.png
    description: stitched bg-mid.png exists
  - id: bg-mid-stitched-atlas-exists
    cmd: test -s assets/scenes/{{scene_id}}/bg-mid/final.atlas.json
  ...
  - id: bg-mid-stitched-width-matches-target
    cmd: |
      ...
      w, h = Image.open('assets/scenes/{{scene_id}}/bg-mid/final.png').size
...
The stitcher reads every `assets/scenes/{{scene_id}}/bg-mid/segments/seg-*.png`, resizes each to a common per-segment width, and feather-blends them onto a wide canvas. The output is one RGBA PNG sized to `bg.layers.mid.target_size`.
