---
title: Classify Game Type
description: Read idea.md (and visual-target.png if present) to classify the game into one of the supported types and write assets/game.json. Every downstream task reads game.json to decide which asset categories to produce or skip.
outputs:
  - "assets/game.json"
  - "assets/game.notes.md"
checks:
  - id: game-json-exists
    cmd: test -s assets/game.json
