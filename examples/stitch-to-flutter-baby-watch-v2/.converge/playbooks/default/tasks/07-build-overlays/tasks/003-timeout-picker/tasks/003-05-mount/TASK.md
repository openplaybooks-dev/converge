---
id: 003-05-mount
title: "Mount: Timeout Picker"
description: Mount Timeout Picker overlay in parent screen and wire trigger
dependencies:
  - 003-04-connect
tags:
  - mount
...
checks:
  - id: parent-imports-overlay
    ...
    cmd: "grep -q 'timeout_picker' lib/screens/settings/settings_screen.dart"
...
# Mount: Timeout Picker

Mount the **Timeout Picker** overlay in its parent screen (****) and wire the trigger.

## Inputs
- `lib/widgets/overlays/timeout_picker/timeout_picker.dart` — The overlay widget file
- `.stitch/designs/timeout-picker/SPEC.md` — Overlay specification (describes trigger and dismiss behavior)
...
3. **Add import** — Add the overlay widget import to the parent screen (use the project's package name from `pubspec.yaml`):
   ```dart
   import 'package:<pkg>/widgets/overlays/timeout_picker/timeout_picker.dart';
   ```
