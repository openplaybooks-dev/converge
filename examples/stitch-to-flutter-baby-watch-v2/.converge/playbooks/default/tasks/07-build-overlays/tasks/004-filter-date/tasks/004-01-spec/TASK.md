---
id: 004-01-spec
title: "Spec: Filter Date Range"
description: Generate Filter Date Range overlay specification
tags:
  - spec
  - overlay-filter-date
inputs:
...
checks:
  - id: spec-exists
    ...
    cmd: test -f .stitch/designs/filter-date/SPEC.md
  - id: spec-has-content
    description: "SPEC.md has >30 lines"
    cmd: test $(wc -l < .stitch/designs/filter-date/SPEC.md) -gt 30
plan:
vars:
...
# Spec: Filter Date Range

Generate the overlay specification for **Filter Date Range** (`filter-date`).
...
Read inputs and produce `.stitch/designs/filter-date/SPEC.md` containing:

1. **Overlay Title** — Filter Date Range
...
- `.stitch/designs/filter-date/SPEC.md` exists and has >30 lines
