---
id: 009-02-normalize-to-glossary
title: "Normalize to glossary: History"
description: Rewrite the reference HTML for history into the Flutter HTML Glossary vocabulary. Constrained transformation — no creative changes.
dependencies:
  - 009-01-link-reference
tags:
  - normalize
  - glossary
  - screen-history
...
checks:
  ...
  - id: meta-names-source
    ...
    cmd: "grep -qE 'Normalized from: `?\\.stitch/designs/history/code\\.html`?' .stitch/designs/history/META.md"
vars:
...
# Normalize to glossary: History

Rewrite the reference HTML at `.stitch/designs/history/code.html` into the constrained Flutter HTML Glossary vocabulary at `.stitch/designs/history/design.html`. This is a **mechanical transformation**, not creative generation.

## Inputs

- `.stitch/designs/history/code.html` — the reference HTML (Tailwind + raw HTML, pixel-truth)
...
Emit `.stitch/designs/history/META.md`:
...
- Leaving Tailwind utility classes in `.stitch/designs/history/design.html` — everything must be glossary classes + `data-*`.

## Success Criteria

- `.stitch/designs/history/design.html` exists, uses `class="scaffold"` wrapper
- Uses `data-color="..."` attributes (not hex)
- `.stitch/designs/history/META.md` exists, cites the source
