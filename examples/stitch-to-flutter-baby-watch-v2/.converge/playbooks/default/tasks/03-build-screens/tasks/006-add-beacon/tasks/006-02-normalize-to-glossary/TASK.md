---
id: 006-02-normalize-to-glossary
title: "Normalize to glossary: Add Beacon"
description: Rewrite the reference HTML for add-beacon into the Flutter HTML Glossary vocabulary. Constrained transformation — no creative changes.
dependencies:
  - 006-01-link-reference
tags:
  - normalize
  - glossary
  - screen-add-beacon
...
checks:
  - id: design-exists
    ...
    cmd: test -f .stitch/designs/add-beacon/design.html
  - id: meta-exists
    description: META.md exists
    cmd: test -f .stitch/designs/add-beacon/META.md
  - id: design-uses-glossary
    description: design.html uses Flutter HTML Glossary vocabulary
    cmd: "grep -q 'class=\"scaffold\"' .stitch/designs/add-beacon/design.html"
  - id: design-has-data-attrs
    description: "design.html uses data-* attributes for Flutter conversion"
    cmd: "grep -q 'data-color=' .stitch/designs/add-beacon/design.html"
  - id: meta-names-source
    description: META.md names the normalization source
    cmd: "grep -qE 'Normalized from: `?\\.stitch/designs/add-beacon/code\\.html`?' .stitch/designs/add-beacon/META.md"
vars:
...
# Normalize to glossary: Add Beacon

Rewrite the reference HTML at `.stitch/designs/add-beacon/code.html` into the constrained Flutter HTML Glossary vocabulary at `.stitch/designs/add-beacon/design.html`. This is a **mechanical transformation**, not creative generation.

## Inputs

- `.stitch/designs/add-beacon/code.html` — the reference HTML (Tailwind + raw HTML, pixel-truth)
...
Emit `.stitch/designs/add-beacon/META.md`:
...
- Leaving Tailwind utility classes in `.stitch/designs/add-beacon/design.html` — everything must be glossary classes + `data-*`.

## Success Criteria

- `.stitch/designs/add-beacon/design.html` exists, uses `class="scaffold"` wrapper
- Uses `data-color="..."` attributes (not hex)
- `.stitch/designs/add-beacon/META.md` exists, cites the source
