---
id: 005-03-convert
title: "Convert: Beacon Detail"
description: "Convert glossary HTML to Flutter widgets for Beacon Detail using stitch-flutter. Inject @converge:element markers for every interactive element."
dependencies:
  - 005-02-normalize-to-glossary
tags:
  - convert
  - flutter
...
checks:
  - id: screen-exists
    ...
    cmd: test -f lib/screens/beacon_detail/beacon_detail_screen.dart
  - id: dart-valid
    description: dart analysis passes
    cmd: dart analyze lib/screens/beacon_detail/beacon_detail_screen.dart
  - id: uses-theme
    description: uses Theme.of(context) for styling
    cmd: "grep -q 'Theme.of(context)' lib/screens/beacon_detail/beacon_detail_screen.dart"
  - id: no-hardcoded-colors
    description: no hardcoded colors — uses colorScheme / semantic theme extension
    cmd: "bash -c 'if grep -qE \"Color\\\\(0x|Colors\\\\.\" lib/screens/beacon_detail/beacon_detail_screen.dart; then exit 1; else exit 0; fi'"
  - id: has-converge-markers
    description: "screen contains at least one @converge:element marker"
    cmd: "grep -q '@converge:element' lib/screens/beacon_detail/beacon_detail_screen.dart"
  - id: markers-count-matches-handlers
    description: "number of markers >= number of data-handler attributes in glossary HTML"
    cmd: "bash -c 'handlers=$(grep -c \"data-handler=\" .stitch/designs/beacon-detail/design.html || echo 0); markers=$(grep -c \"@converge:element\" lib/screens/beacon_detail/beacon_detail_screen.dart || echo 0); test \"$markers\" -ge \"$handlers\"'"
vars:
...
# Convert: Beacon Detail

Convert the glossary HTML at `.stitch/designs/beacon-detail/design.html` into a Flutter widget at `lib/screens/beacon_detail/beacon_detail_screen.dart` using the **stitch-flutter** skill.

## Critical: marker injection

For every interactive element in `.stitch/designs/beacon-detail/design.html` with a `data-handler="..."` attribute, inject a comment marker on the line immediately preceding the widget:
...
- `.stitch/designs/beacon-detail/design.html` — glossary HTML (authoritative for structure and interactive elements)
- `.stitch/designs/beacon-detail/META.md` — includes the handler inventory
...
- `lib/screens/beacon_detail/beacon_detail_screen.dart` exists and passes `dart analyze`
- Uses `Theme.of(context)` throughout
- Zero hardcoded hex colors or `Colors.X`
- Marker count ≥ `data-handler` attribute count in `.stitch/designs/beacon-detail/design.html`
