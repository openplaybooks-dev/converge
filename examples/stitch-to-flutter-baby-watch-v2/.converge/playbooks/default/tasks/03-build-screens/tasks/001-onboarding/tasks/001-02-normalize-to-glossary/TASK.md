---
id: 001-02-normalize-to-glossary
title: "Normalize to glossary: Onboarding"
description: Rewrite the reference HTML for onboarding into the Flutter HTML Glossary vocabulary. Constrained transformation — no creative changes.
dependencies:
  - 001-01-link-reference
tags:
  - normalize
  - glossary
  - screen-onboarding
...
checks:
  ...
  - id: meta-names-source
    ...
    cmd: "grep -qE 'Normalized from: `?\\.stitch/designs/onboarding/code\\.html`?' .stitch/designs/onboarding/META.md"
vars:
...
# Normalize to glossary: Onboarding

Rewrite the reference HTML at `.stitch/designs/onboarding/code.html` into the constrained Flutter HTML Glossary vocabulary at `.stitch/designs/onboarding/design.html`. This is a **mechanical transformation**, not creative generation.

## Inputs

- `.stitch/designs/onboarding/code.html` — the reference HTML (Tailwind + raw HTML, pixel-truth)
...
Emit `.stitch/designs/onboarding/META.md`:
...
- Leaving Tailwind utility classes in `.stitch/designs/onboarding/design.html` — everything must be glossary classes + `data-*`.

## Success Criteria

- `.stitch/designs/onboarding/design.html` exists, uses `class="scaffold"` wrapper
- Uses `data-color="..."` attributes (not hex)
- `.stitch/designs/onboarding/META.md` exists, cites the source
