// WBS: Per-widget split subtasks
// Reads widgets.jsonl and spawns one subtask per widget using the shared subtask template.
...
export async function run(ctx) {
  const SUBTASK_TEMPLATE = relative(
    ctx.projectDir,
    join(
      __dirname,
      '..',
  ...
  const { localWidgetsDir, screenPath, screenId, screenTitle, title, widgetsJsonPath } = ctx.vars;
  ...
  const resolvedScreenTitle = screenTitle ?? title ?? '';
