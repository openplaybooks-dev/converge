...
description: "Convert glossary HTML to Flutter widgets for History using stitch-flutter. Inject @converge:element markers for every interactive element."
dependencies:
  - 009-02-normalize-to-glossary
tags:
  - convert
  - flutter
  - screen-history
...
checks:
  - id: screen-exists
    ...
    cmd: test -f lib/screens/history/history_screen.dart
  - id: dart-valid
    description: dart analysis passes
    cmd: dart analyze lib/screens/history/history_screen.dart
  - id: uses-theme
    description: uses Theme.of(context) for styling
    cmd: "grep -q 'Theme.of(context)' lib/screens/history/history_screen.dart"
  - id: no-hardcoded-colors
    description: no hardcoded colors — uses colorScheme / semantic theme extension
    cmd: "bash -c 'if grep -qE \"Color\\\\(0x|Colors\\\\.\" lib/screens/history/history_screen.dart; then exit 1; else exit 0; fi'"
  - id: has-converge-markers
    description: "screen contains at least one @converge:element marker"
    cmd: "grep -q '@converge:element' lib/screens/history/history_screen.dart"
  - id: markers-count-matches-handlers
    description: "number of markers >= number of data-handler attributes in glossary HTML"
    cmd: "bash -c 'handlers=$(grep -c \"data-handler=\" .stitch/designs/history/design.html || echo 0); markers=$(grep -c \"@converge:element\" lib/screens/history/history_screen.dart || echo 0); test \"$markers\" -ge \"$handlers\"'"
vars:
...
Convert the glossary HTML at `.stitch/designs/history/design.html` into a Flutter widget at `lib/screens/history/history_screen.dart` using the **stitch-flutter** skill.

## Critical: marker injection

For every interactive element in `.stitch/designs/history/design.html` with a `data-handler="..."` attribute, inject a comment marker on the line immediately preceding the widget:
...
- `.stitch/designs/history/design.html` — glossary HTML (authoritative for structure and interactive elements)
- `.stitch/designs/history/META.md` — includes the handler inventory
...
- `lib/screens/history/history_screen.dart` exists and passes `dart analyze`
- Uses `Theme.of(context)` throughout
- Zero hardcoded hex colors or `Colors.X`
- Marker count ≥ `data-handler` attribute count in `.stitch/designs/history/design.html`
