---
id: 004-02-normalize-to-glossary
title: "Normalize to glossary: Home — Alert"
description: Rewrite the reference HTML for home-alert into the Flutter HTML Glossary vocabulary. Constrained transformation — no creative changes.
dependencies:
  - 004-01-link-reference
tags:
  - normalize
  - glossary
  - screen-home-alert
...
checks:
  - id: design-exists
    ...
    cmd: test -f .stitch/designs/home-alert/design.html
  - id: meta-exists
    description: META.md exists
    cmd: test -f .stitch/designs/home-alert/META.md
  - id: design-uses-glossary
    description: design.html uses Flutter HTML Glossary vocabulary
    cmd: "grep -q 'class=\"scaffold\"' .stitch/designs/home-alert/design.html"
  - id: design-has-data-attrs
    description: "design.html uses data-* attributes for Flutter conversion"
    cmd: "grep -q 'data-color=' .stitch/designs/home-alert/design.html"
  - id: meta-names-source
    description: META.md names the normalization source
    cmd: "grep -qE 'Normalized from: `?\\.stitch/designs/home-alert/code\\.html`?' .stitch/designs/home-alert/META.md"
vars:
...
# Normalize to glossary: Home — Alert

Rewrite the reference HTML at `.stitch/designs/home-alert/code.html` into the constrained Flutter HTML Glossary vocabulary at `.stitch/designs/home-alert/design.html`. This is a **mechanical transformation**, not creative generation.

## Inputs

- `.stitch/designs/home-alert/code.html` — the reference HTML (Tailwind + raw HTML, pixel-truth)
...
Emit `.stitch/designs/home-alert/META.md`:
...
- Leaving Tailwind utility classes in `.stitch/designs/home-alert/design.html` — everything must be glossary classes + `data-*`.

## Success Criteria

- `.stitch/designs/home-alert/design.html` exists, uses `class="scaffold"` wrapper
- Uses `data-color="..."` attributes (not hex)
- `.stitch/designs/home-alert/META.md` exists, cites the source
