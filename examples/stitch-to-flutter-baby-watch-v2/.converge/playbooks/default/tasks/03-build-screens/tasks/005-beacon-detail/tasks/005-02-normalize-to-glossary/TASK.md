---
id: 005-02-normalize-to-glossary
title: "Normalize to glossary: Beacon Detail"
description: Rewrite the reference HTML for beacon-detail into the Flutter HTML Glossary vocabulary. Constrained transformation — no creative changes.
dependencies:
  - 005-01-link-reference
tags:
  - normalize
  - glossary
...
checks:
  ...
  - id: meta-names-source
    ...
    cmd: "grep -qE 'Normalized from: `?\\.stitch/designs/beacon-detail/code\\.html`?' .stitch/designs/beacon-detail/META.md"
vars:
...
# Normalize to glossary: Beacon Detail

Rewrite the reference HTML at `.stitch/designs/beacon-detail/code.html` into the constrained Flutter HTML Glossary vocabulary at `.stitch/designs/beacon-detail/design.html`. This is a **mechanical transformation**, not creative generation.

## Inputs

- `.stitch/designs/beacon-detail/code.html` — the reference HTML (Tailwind + raw HTML, pixel-truth)
...
Emit `.stitch/designs/beacon-detail/META.md`:
...
- Leaving Tailwind utility classes in `.stitch/designs/beacon-detail/design.html` — everything must be glossary classes + `data-*`.

## Success Criteria

- `.stitch/designs/beacon-detail/design.html` exists, uses `class="scaffold"` wrapper
- Uses `data-color="..."` attributes (not hex)
- `.stitch/designs/beacon-detail/META.md` exists, cites the source
