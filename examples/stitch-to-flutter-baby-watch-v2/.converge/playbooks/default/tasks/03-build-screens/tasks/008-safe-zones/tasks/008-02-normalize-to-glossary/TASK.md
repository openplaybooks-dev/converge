---
id: 008-02-normalize-to-glossary
title: "Normalize to glossary: Safe Zones"
description: Rewrite the reference HTML for safe-zones into the Flutter HTML Glossary vocabulary. Constrained transformation — no creative changes.
dependencies:
  - 008-01-link-reference
tags:
  - normalize
  - glossary
  - screen-safe-zones
...
checks:
  ...
  - id: meta-names-source
    ...
    cmd: "grep -qE 'Normalized from: `?\\.stitch/designs/safe-zones/code\\.html`?' .stitch/designs/safe-zones/META.md"
vars:
...
# Normalize to glossary: Safe Zones

Rewrite the reference HTML at `.stitch/designs/safe-zones/code.html` into the constrained Flutter HTML Glossary vocabulary at `.stitch/designs/safe-zones/design.html`. This is a **mechanical transformation**, not creative generation.

## Inputs

- `.stitch/designs/safe-zones/code.html` — the reference HTML (Tailwind + raw HTML, pixel-truth)
...
Emit `.stitch/designs/safe-zones/META.md`:
...
- Leaving Tailwind utility classes in `.stitch/designs/safe-zones/design.html` — everything must be glossary classes + `data-*`.

## Success Criteria

- `.stitch/designs/safe-zones/design.html` exists, uses `class="scaffold"` wrapper
- Uses `data-color="..."` attributes (not hex)
- `.stitch/designs/safe-zones/META.md` exists, cites the source
