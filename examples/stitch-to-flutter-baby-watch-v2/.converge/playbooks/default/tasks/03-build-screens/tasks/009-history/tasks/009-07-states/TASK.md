...
description: "Add empty, loading, and error variants for History as a sibling file imported by the screen."
dependencies:
  - 009-06-lift
tags:
  - states
...
outputs:
  - lib/screens/history/history_states.dart
  - lib/screens/history/history_screen.dart
checks:
  - id: states-file-exists
    description: states file exists
    cmd: test -f lib/screens/history/history_states.dart
  - id: states-dart-valid
    description: states file passes dart analyze
    cmd: dart analyze lib/screens/history/history_states.dart
  - id: screen-imports-states
    description: screen imports its states file
    cmd: "grep -q '_states.dart' lib/screens/history/history_screen.dart"
  - id: states-has-empty
    description: HistoryEmptyState widget defined
    cmd: "grep -qE 'class +HistoryEmptyState\\b' lib/screens/history/history_states.dart"
  - id: states-has-loading
    description: HistoryLoadingState widget defined
    cmd: "grep -qE 'class +HistoryLoadingState\\b' lib/screens/history/history_states.dart"
  - id: states-has-error
    description: HistoryErrorState widget defined
    cmd: "grep -qE 'class +HistoryErrorState\\b' lib/screens/history/history_states.dart"
vars:
...
Add empty, loading, and error variants to `history`. Emit them as three widgets in `lib/screens/history/history_states.dart` so the main screen can compose them based on provider `AsyncValue` state.
...
- `lib/screens/history/history_screen.dart` — the happy-path screen. Read it to learn which providers it will read (phase 05 will add those) and what layout to mirror in the states.
...
## Output: `lib/screens/history/history_states.dart`
...
After emitting `lib/screens/history/history_states.dart`, modify `lib/screens/history/history_screen.dart` to import it:
...
body: ref.watch(beaconsProvider).when(
      data: (beacons) => beacons.isEmpty
          ? HistoryEmptyState(onPrimaryAction: () => /* nav to scanner */)
          : /* existing happy-path body */,
      loading: () => const HistoryLoadingState(),
      error: (e, _) => HistoryErrorState(error: e, onRetry: () => ref.invalidate(beaconsProvider)),
    ),
```
...
- `lib/screens/history/history_states.dart` exists with three widget classes.
- `lib/screens/history/history_screen.dart` imports `history_states.dart`.
