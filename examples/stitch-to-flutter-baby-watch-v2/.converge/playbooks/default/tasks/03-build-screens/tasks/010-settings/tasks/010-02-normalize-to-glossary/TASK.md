---
id: 010-02-normalize-to-glossary
title: "Normalize to glossary: Settings"
description: Rewrite the reference HTML for settings into the Flutter HTML Glossary vocabulary. Constrained transformation — no creative changes.
dependencies:
  - 010-01-link-reference
tags:
  - normalize
  - glossary
  - screen-settings
...
checks:
  ...
  - id: meta-names-source
    ...
    cmd: "grep -qE 'Normalized from: `?\\.stitch/designs/settings/code\\.html`?' .stitch/designs/settings/META.md"
vars:
...
# Normalize to glossary: Settings

Rewrite the reference HTML at `.stitch/designs/settings/code.html` into the constrained Flutter HTML Glossary vocabulary at `.stitch/designs/settings/design.html`. This is a **mechanical transformation**, not creative generation.

## Inputs

- `.stitch/designs/settings/code.html` — the reference HTML (Tailwind + raw HTML, pixel-truth)
...
Emit `.stitch/designs/settings/META.md`:
...
- Leaving Tailwind utility classes in `.stitch/designs/settings/design.html` — everything must be glossary classes + `data-*`.

## Success Criteria

- `.stitch/designs/settings/design.html` exists, uses `class="scaffold"` wrapper
- Uses `data-color="..."` attributes (not hex)
- `.stitch/designs/settings/META.md` exists, cites the source
