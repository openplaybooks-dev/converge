...
description: "Convert glossary HTML to Flutter widgets for Onboarding using stitch-flutter. Inject @converge:element markers for every interactive element."
dependencies:
  - 001-02-normalize-to-glossary
tags:
  - convert
  - flutter
  - screen-onboarding
...
checks:
  ...
  - id: dart-valid
    ...
    cmd: dart analyze lib/screens/onboarding/onboarding_screen.dart
  - id: uses-theme
  ...
  - id: no-hardcoded-colors
    ...
    cmd: "bash -c 'if grep -qE \"Color\\\\(0x|Colors\\\\.\" lib/screens/onboarding/onboarding_screen.dart; then exit 1; else exit 0; fi'"
  - id: has-converge-markers
    description: "screen contains at least one @converge:element marker"
    cmd: "grep -q '@converge:element' lib/screens/onboarding/onboarding_screen.dart"
  - id: markers-count-matches-handlers
    description: "number of markers >= number of data-handler attributes in glossary HTML"
    cmd: "bash -c 'handlers=$(grep -c \"data-handler=\" .stitch/designs/onboarding/design.html || echo 0); markers=$(grep -c \"@converge:element\" lib/screens/onboarding/onboarding_screen.dart || echo 0); test \"$markers\" -ge \"$handlers\"'"
vars:
...
Convert the glossary HTML at `.stitch/designs/onboarding/design.html` into a Flutter widget at `lib/screens/onboarding/onboarding_screen.dart` using the **stitch-flutter** skill.

## Critical: marker injection

For every interactive element in `.stitch/designs/onboarding/design.html` with a `data-handler="..."` attribute, inject a comment marker on the line immediately preceding the widget:
...
- `.stitch/designs/onboarding/design.html` — glossary HTML (authoritative for structure and interactive elements)
- `.stitch/designs/onboarding/META.md` — includes the handler inventory
...
- `lib/screens/onboarding/onboarding_screen.dart` exists and passes `dart analyze`
- Uses `Theme.of(context)` throughout
- Zero hardcoded hex colors or `Colors.X`
- Marker count ≥ `data-handler` attribute count in `.stitch/designs/onboarding/design.html`
