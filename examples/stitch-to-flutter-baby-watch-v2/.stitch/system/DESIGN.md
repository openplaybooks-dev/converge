> Source: .stitch/references/serene_guardian/DESIGN.md (chosen in .stitch/system/DECISION.md).
...
- From `lullaby_minimal/DESIGN.md` §"Signature App-Specific Components" — **Sleep Waveform**: a custom visualization using the mint `tertiary` color with a soft linear gradient to transparent. Applies where chosen system is silent on app-specific data viz.
- From `lullaby_minimal/DESIGN.md` §"Signature App-Specific Components" — **Status Orb**: a floating 8px circle using a safe-state token for "Safe" and an error-container token for "Alert," with a 12px ambient glow of the same color. Applies where chosen system does not specify a floating status indicator.
- From `lullaby_minimal/DESIGN.md` §"Cards & Lists" — Use `xl` (3rem) radius for parent containers and `lg` (2rem) for nested cards as the upper-bound radius scale. Applies where chosen system is silent on parent-vs-nested radius differentiation (chosen system specifies only `md`/`lg`).
- From `lullaby_minimal/DESIGN.md` §"Buttons" — On press, scale the button to `0.98` rather than changing color drastically. Applies where chosen system does not specify a press interaction.
