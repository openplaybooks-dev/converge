...
outputs:
  - runs/run-2026-04-25T01-45/timeline.jsonl
  - "vault/runs/run-2026-04-25T01-45/actions/t3-p007-*.md"
checks:
  - id: action-appended
    description: Exactly one timeline row exists for tick=3 personaId=p007
    cmd: "python3 -c \"import json,sys; lines=[json.loads(l) for l in open('runs/run-2026-04-25T01-45/timeline.jsonl') if l.strip()]; hits=[l for l in lines if l.get('tick')==3 and l.get('personaId')=='p007']; sys.exit(0 if len(hits)==1 else 1)\"\n"
  - id: action-valid
    description: Recorded action is one of post/repost/reply/like/follow/nothing
    cmd: "python3 -c \"import json,sys; lines=[json.loads(l) for l in open('runs/run-2026-04-25T01-45/timeline.jsonl') if l.strip()]; hits=[l for l in lines if l.get('tick')==3 and l.get('personaId')=='p007']; r=hits[0] if hits else {}; sys.exit(0 if r.get('action') in ['post','repost','reply','like','follow','nothing'] else 1)\"\n"
  - id: vault-action-note
    description: Vault action note exists for this tick × persona
    cmd: "ls vault/runs/run-2026-04-25T01-45/actions/t3-p007-*.md 2>/dev/null | head -1 | grep -q .\n"
vars:
  taskId: p007
  tick: 3
  tickNum: 3
...
Write `vault/runs/run-2026-04-25T01-45/actions/t3-p007-<action>.md`
...
```markdown
---
tags: [action, run/run-2026-04-25T01-45, tick/3, persona/p007, action/<action>]
tick: 3
persona_id: p007
...
- If the timeline already has a row for `tick=3, personaId=p007`,
