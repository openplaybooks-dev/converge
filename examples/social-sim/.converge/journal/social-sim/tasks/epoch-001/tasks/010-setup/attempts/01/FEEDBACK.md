...
**Output**:
```
Command failed: test -f runs/run-001/personas.json
```
...
FileNotFoundError: [Errno 2] No such file or directory: 'runs/run-001/personas.json'
```
...
**Output**:
```
Command failed: test -f runs/run-001/graph.json
```
...
Traceback (most recent call last):
  File "<string>", line 1, in <module>
FileNotFoundError: [Errno 2] No such file or directory: 'runs/run-001/graph.json'
```
...
**Output**:
```
touch: runs/run-001/timeline.jsonl: No such file or directory
```
...
**Output**:
```
Command failed: test -f vault/runs/run-001/overview.md
```
...
**Output**:
```
Command failed: test "$(ls vault/runs/run-001/personas/*.md 2>/dev/null | wc -l | tr -d ' ')" = "10"
```
