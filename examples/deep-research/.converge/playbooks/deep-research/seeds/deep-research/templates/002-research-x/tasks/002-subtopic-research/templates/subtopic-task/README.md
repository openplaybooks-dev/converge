...
await ctx.spawn({
  _type: 'template-ref',
  path: 'templates/subtopic-task/TASK.md',  // Points back to parent!
  vars: { subtopicId, subtopicName, ... }
});
```
...
await ctx.spawn({
  ...
  vars: { subtopicId, subtopicName, subtopicDescription, ... }
});
```

The recursion happens automatically through the sub-subtopics spawner.
