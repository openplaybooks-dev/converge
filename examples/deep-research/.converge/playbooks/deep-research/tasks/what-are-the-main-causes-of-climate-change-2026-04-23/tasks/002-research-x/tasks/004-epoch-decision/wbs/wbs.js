...
export async function run(ctx) {
  ...
  // Try multiple possible paths for the decision file
  const possiblePaths = [
    join(artifactsDir, `epoch-${currentEpoch}-004-epoch-decision`, 'epoch-decision.json'),
    join(artifactsDir, '2-research', `epoch-${currentEpoch}-decision.json`),
    join(artifactsDir, '2-research', 'epoch-decision.json'),
  ];
  ...
  if (!decisionPath) {
    ctx.log.warn(`Decision file not found. Tried paths: ${possiblePaths.join(', ')}`);
    return;
  }
  ...
  await ctx.spawn(
    {
      _type: 'template-ref',
      path: relative(ctx.projectDir, join(templatesDir, '002-research-x', 'TASK.md')),
      vars: {
