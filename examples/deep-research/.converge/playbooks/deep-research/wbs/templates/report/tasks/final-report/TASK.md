...
checks:
  - id: report-written
    cmd: "test -f {{artifactsDir}}/final-report/final-report.md"
...
Read all research-x epoch aggregates:
- `{{artifactsDir}}/epoch-*-003-cross-topic-aggregate/epoch-aggregate.json`

Read initial phase aggregation:
- `{{artifactsDir}}/004-initial-aggregation/initial-aggregation.json`

## Process

1. **Cumulative Synthesis**: Combine all epoch findings into coherent picture
...
Write `{{artifactsDir}}/final-report/final-report.md`:
...
Also write `{{artifactsDir}}/final-report/report-summary.json`:
```json
{
